...
검사 축 (각 축은 모집단을 **문서 구조에서 파생**한다; 자리 열거를 하드코딩하지 않는다)::
  ...
  C1  TOS-CC-C1   「(4) 열거」를 참조하며 **기수 토큰**(아라비아 숫자 + 「개」/「원소」 ·
                  정본은 `CARDINALITY_TOKEN_RE`)을 담은 문장 ≡ 실제 열거 원소 수.
                  **«모든 기수 문장»이 아니다** — 한글 수사·다른 단위는 보지 못한다
                  (등재된 잔여 · `check_c1` docstring).
...
def iter_cardinality_statements(
    ...
    **«수량을 말하는 자리»가 아니다** — 참조 뒤 `CARDINALITY_WINDOW` 자 창에서
    `CARDINALITY_TOKEN_RE`(아라비아 숫자 + 「개」/「원소」)만 찾는다.  한글 수사·다른 단위는
    보지 못한다(등재된 잔여 · `check_c1` docstring 과 같은 술어).  **재심 #18 이 이 자리를
    C1 전칭의 «네 번째» 자리로 적발했다** — 56차가 「세 자리 일치」라 적은 것이 거짓이었다.
...
def check_c1(doc: ContractDoc) -> list[Violation]:
    ...
    **«모든 기수 문장»이 아니다.**  모집단은 `CARDINALITY_TOKEN_RE` = `(\\d+)\\s*(개|원소)`
    하나이므로 **아라비아 숫자 + 「개」/「원소」** 형태만 잡는다.  **등재된 잔여**: 한글
    수사(「여덟」)나 다른 단위(「항목」·「건」·「자리」)로 적은 거짓 기수 진술은 이 축이
    **보지 못한다** — 재심 #16 이 실측으로 냈다(「아래 9개」를 「아래 여덟 항목」으로 바꾸면
    위반 0건).  종래 이 문언은 「모든 문장」이라는 **전칭**이었고 그것이 능력보다 넓었다.
    """
...
def _cap_guard_sites(
    ...
    **표 행을 통째로 빼지 않는다**(43차 재심 F2) — 그러나 **떠다니는 표식 문자열로도 빼지
    ...
    면제는 **행 정체**로 한다: `_fixture_row_id` 가 **정규화한 첫 셀의 «완전일치»** 를
    요구한다 — **접두 일치가 아니다**(45차 재심 #5 F1: `startswith` 는 `| **T-84**-NORM |`
    같은 접두 네임스페이스를 같은 행으로 읽어 통째로 면제했다).  **재심 #18 실측**: 그
    접두 행에 guard 를 넣으면 모집단 4 → **5**(면제되지 «않는다»), 완전일치 행이면 **4**.
    ...
    **«바로 다음 행»은 필요조건이지 충분조건이 아니다**(재심 #18): 판정 창은
    `(prev + cur[:m.start()])[-GUARD_WINDOW:]` 이므로 **비교식이 창 밖으로 밀리면 결합하지
