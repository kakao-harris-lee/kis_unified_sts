...
def _cap_guard_sites(
    ...
    **코드펜스 «안»도 HTML 주석도 배제하지 «않는다» — 전 행을 훑는다**(면제는 픽스처 행
    정체 하나뿐이다).  44차가 펜스 배제를 **철회**했기 때문이다: 이 계약은 규범 술어를
    ```text 블록 «안»에 쓰므로 배제하면 모집단이 4 → 1 로 줄고 배터리가 그것을 «죽은 검사
    2건»으로 잡았다(그 판정 근거는 `GUARD_ARROW_RE` 위 주석에 있다).  귀결로 **펜스 안
    문서화 예시에 대한 위양성은 열려 있으며 «등재된 잔여»다** — 닫힌 능력이 아니다.
    표기 변형은 **NFKC 표준**에 위임한다.

    이 문언은 44차(`df72ae5b`)가 철회 선언과 **같은 커밋에서** 옛 주장을 남겨 둔 자리였고,
    **재심 #15 가 그 능력 과대서술을 material 로 냈다** — 구조 확인: 이 함수는
    `_fence_spans`/`_enclosing_fence` 를 호출하지 «않는다».
