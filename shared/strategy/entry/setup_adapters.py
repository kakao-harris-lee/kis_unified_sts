...
class SetupAEntryConfig(ServiceConfigBase):
    ...
    daily_regime_trend_filter: DailyRegimeTrendFilterConfig = Field(
        default_factory=DailyRegimeTrendFilterConfig,
        description="Daily trend-regime direction gate for futures entries",
    )
...
def _daily_regime_gate_allows(
    *,
    ...
    """Apply the optional daily trend-regime gate to a candidate signal."""

    decision = apply_daily_regime_trend_filter(
        config=config,
        indicators=context.indicators or {},
        signal_direction=str(decision_signal.direction),
    )
    if decision.allowed:
        if config.enabled:
            logger.debug(
                "%s daily regime filter allowed %s: bias=%s reason=%s",
                setup_name,
                decision_signal.direction,
                decision.bias,
                decision.reason,
            )
        return True
    ...
    logger.info(
        "%s daily regime filter blocked %s: bias=%s reason=%s",
        setup_name,
        decision_signal.direction,
        decision.bias,
        decision.reason,
...
class SetupAEntryAdapter(EntrySignalGenerator[SetupAEntryConfig]):
    ...
    async def generate(self, context: EntryContext) -> OrchestratorSignal | None:
        ...
        if decision_signal is None:
            return None
        if not _daily_regime_gate_allows(
            setup_name=self.name,
            config=self.config.daily_regime_trend_filter,
