...
from shared.kis.target_price import (
    calc_target_revision_pct,
    empty_target_price_summary,
    normalize_target_price_report,
    parse_kis_date,
    summarize_target_price_rows,
...
class KISClient(AsyncSessionMixin):
    ...
    async def summarize_target_price(
        ...
        *,
        ...
        return summarize_target_price_rows(
            rows,
    ...
    def _normalize_target_price_report(
        ...
        _ = cls
        return normalize_target_price_report(row, current_price)

    @staticmethod
    def _calc_target_revision_pct(
        reports: list[dict[str, Any]], recent_days: int
    ) -> float:
        return calc_target_revision_pct(reports, recent_days)

    @staticmethod
    def _target_revision_direction(revision_pct: float) -> str:
        return target_revision_direction(revision_pct)
