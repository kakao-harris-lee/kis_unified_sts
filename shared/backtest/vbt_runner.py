...
- **DailyBacktestAdapter(일봉 어댑터) 경로**: parity 미검증 — dual-run 증거가
  분봉 어댑터 프로토콜에만 존재하므로 정적으로 거부한다(legacy 폴백). 일봉
  전략(pattern_pullback 등)은 exit 가 허용목록에 있어도 legacy 로 돈다.
...
#             chandelier_exit — 트레일링 스탑 (atr/highest_high 주입 의존).
#             ⚠️ chandelier 를 쓰는 활성 전략(pattern_pullback)은 daily 타임
...
class VectorbtParityError(RuntimeError):
    """resolver 원장 ↔ vectorbt 원장 cross-check 불일치 (내부 불변식 위반).

    "표현 불가"(:class:`VectorbtNotSupportedError`)가 아니라 **러너 자체의
    parity 결함** 신호다 — 호출자(experiment seam)는 결과를 버리지 말고 legacy
    로 폴백하되, 조사 대상으로 경고 로그를 남긴다. 이 시점에는 어댑터가 이미
    해석 패스로 오염된 상태이므로 폴백은 반드시 어댑터를 재생성해야 한다
    (seam 의 fresh-adapter 계약)."""
...
class VectorbtRunner:
    ...
    def _ensure_supported(self, data: pd.DataFrame) -> None:
        ...
        # 일봉 어댑터 경로는 parity 미검증 (dual-run 증거는 분봉 어댑터 프로토콜
        # 전용) — chandelier_exit 허용목록 등재가 daily 전략(pattern_pullback)을
        # 무증거로 vbt 에 태우지 않도록 정적으로 거부한다. isinstance 가 가장
        # 덜 취약한 판별(이름 문자열/속성 추측 배제); daily_adapter 는 무거운
        # 모듈이라 lazy import 하되, seam 이 daily 전략을 라우팅할 땐 이미
        # import 된 상태라 실비용은 없다.
        from shared.backtest.daily_adapter import DailyBacktestAdapter

        if isinstance(self.strategy, DailyBacktestAdapter):
            raise VectorbtNotSupportedError(
                "daily adapter path (DailyBacktestAdapter) is parity-unverified"
            )
    ...
    def _trades_from_records(
        ...
        수량은 vbt 레코드에서, 가격·레거시 전용 규약 필드(pnl 의 진입비 제외,
        commission=청산수수료+매도세, exit_reason)는 resolver 이벤트에서 채운다.
        가격을 이벤트에서 채우는 이유: 이벤트는 체결 bar 종가 **원본**을 보존
        하지만 vbt 레코드의 entry/exit 가격은 내부 value/size 가중 재구성이라
        ULP 잔차가 생긴다 — 레코드 ↔ 이벤트의 가격 일치 자체는 _cross_check 가
        독립적으로 강제하므로(rel_tol 1e-12) 불변식은 유지된다.
        ``rec_sorted`` 정렬과 행 대응 일치도 _cross_check 가 선행 보장.
        """
