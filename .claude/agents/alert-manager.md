...
2. Cron 스크립트 스케줄 관리
3. LLM 브리핑 **전달**: 스케줄(야간/장전/장마감)·채널 라우팅·발송 (분석 **콘텐츠**는 `llm-analyst` 소유)
...
- LLM 분석 (콘텐츠는 `llm-analyst` 소유): `shared/llm/`
...
- **llm-analyst**: 브리핑 콘텐츠(llm-analyst) ↔ 전달·스케줄·채널(alert-manager) 인계
