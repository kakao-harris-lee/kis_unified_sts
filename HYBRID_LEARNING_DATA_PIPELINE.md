...
현재 워크트리에서 hybrid dataset / training / evaluation 파이프라인은 **실행 가능 상태**다. 다만 이 머신에서는 ClickHouse(`localhost:9000`) 연결이 실패하므로, Phase 1 `real regime catalog`는 현재 **`sample_fallback` provenance**로 생성된다.

따라서 현재 artifact 기준 상태는 다음과 같다.

- `artifacts/datasets/regime_catalog/summary.json`
  - `provenance.source_mode = sample_fallback`
  - `provenance.is_authentic_real = false`
- `artifacts/datasets/hybrid/manifest.json`
  - `rules.bootstrap_mode = false`
  - `rules.test_is_real_only = true`
  - `rules.real_catalog_authentic = false`
  - `rules.real_catalog_source_mode = sample_fallback`
  - `rules.final_selection_allowed = false`

핵심 해석은 아래와 같다.

- 현재 holdout split은 형식상 `real` only 이다.
- 그러나 그 `real` 카탈로그의 원천이 ClickHouse-backed KOSPI 실데이터가 아니라 fallback sample 이므로,
- **현재 머신에서 나온 hybrid train/eval 결과는 최종 모델 채택 근거로 사용할 수 없다.**

허용되는 사용 범위:

- 데이터 파이프라인 smoke 검증
- RL 학습 코드 경로 검증
- synthetic/transfer mixing 로직 점검
- bootstrap / pretraining 실험

허용되지 않는 사용 범위:

- 최종 모델 selection
- baseline 대비 공식 성능 우위 주장
- real KOSPI holdout 일반화 성능 보고

최종 채택 가능 상태로 전환하려면 다음 조건이 모두 필요하다.

1. ClickHouse에서 실제 `kospi.kospi200f_1m` / `101S6000` 데이터를 로드할 수 있어야 한다.
2. `build_real_regime_catalog.py` 재실행 후 provenance가 `clickhouse` 여야 한다.
3. `build_hybrid_rl_dataset.py` 재실행 후 manifest의 `rules.final_selection_allowed` 가 `true` 여야 한다.

즉, 지금 상태는 **"개발 검증 완료, 최종 검증 대기"** 로 보는 것이 맞다.
