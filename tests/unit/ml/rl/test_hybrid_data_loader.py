...
def test_hybrid_loader_respects_explicit_scaler_output_dir(tmp_path: Path):
    train_path = tmp_path / "train.parquet"
    validation_path = tmp_path / "validation.parquet"
    test_path = tmp_path / "test.parquet"
    ...
    loader = HybridRLDataLoader("ml/rl_mppo.yaml")
    loader.load_from_manifest(
        manifest_path,
        persist_scaler=True,
        scaler_output_dir=tmp_path / "custom_models",
    )

    assert (tmp_path / "custom_models" / "scaler.joblib").exists()
    assert not (tmp_path / "models" / "scaler.joblib").exists()


def test_hybrid_loader_accepts_suffixed_ohlc_columns(tmp_path: Path):
    train_path = tmp_path / "train.parquet"
    validation_path = tmp_path / "validation.parquet"
    test_path = tmp_path / "test.parquet"
    ...
    assert len(result["train_days"]) == 1
    assert result["train_prices"][0].shape[1] == 4
