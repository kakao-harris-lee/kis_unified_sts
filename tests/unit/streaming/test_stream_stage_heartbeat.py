...
@pytest.mark.parametrize(
    "interval",
    [
        # 179 + the shipped 2s block = a 181s gap: over the bound, on a
        # healthy day.
        179.0,
        # 178: the gap is exactly 180, which the harvester passes — but its
        # timestamps are truncated to whole seconds, so it can measure 181.
        LIVENESS_MAX_GAP_SECONDS - _SHIPPED_BLOCK,
        LIVENESS_MAX_GAP_SECONDS,
        # 300: the value that used to satisfy EVERY guard-rail the code and the
        # config comments stated, while making every healthy quiet day read
        # `stale_observation`. The ceiling derived from 1800 accepted it.
        300.0,
...
def test_interval_whose_observed_gap_would_exceed_the_bound_is_refused(interval):
    ...
    A service adding ``heartbeat_interval_seconds: 300`` to its config and
    ...
    The 179 case is the subtler one and the reason the ceiling is derived
...
def test_the_ceiling_itself_is_accepted():
    ...
    180 passes; its timestamps are second-truncated, so the second that
    truncation can add is already subtracted here. At the ceiling the worst
    case is 177 + 2 + 1 = 180 measured — the largest value that still passes.
    """
    ceiling = max_heartbeat_interval_seconds(_SHIPPED_BLOCK)

    assert ceiling == LIVENESS_MAX_GAP_SECONDS - _SHIPPED_BLOCK - 1.0
...
def test_ceiling_shrinks_as_the_poll_block_grows():
    ...
    Nothing about 180 changes; what changes is how much of it the poll spends.
    """
    assert max_heartbeat_interval_seconds(5.0) == 174.0  # 180 - 5 - 1

    with pytest.raises(ValueError, match="heartbeat_interval_seconds"):
        _heartbeat(_Clock(), interval_seconds=175.0, poll_block_seconds=5.0)

    accepted = _heartbeat(_Clock(), interval_seconds=174.0, poll_block_seconds=5.0)
    assert accepted.interval_seconds == 174.0
...
async def test_config_value_above_the_ceiling_is_clamped_not_fatal(monkeypatch, caplog):
    ...
    # 179 rather than 300: since the bound moved from the staleness gap (1800)
    # to the liveness gap (180), the field's own `lt` refuses anything at or
    # above 180, so a wilder value never reaches the clamp — it is caught at the
    # boundary and `load()` answers with the default. The clamp window is now
    # (ceiling, bound) = (177, 180), and this is inside it.
    monkeypatch.setenv("CONSUMER_STAGE_HEARTBEAT_INTERVAL_SECONDS", "179")
    ...
    assert clamped[0]["configured_seconds"] == "179.0"
    assert clamped[0]["clamped_seconds"] == "177.0"
    assert clamped[0]["consumer_group"] == "g"


@pytest.mark.parametrize(
    ("block_ms", "ceiling"),
    [(179_000, 0.0), (180_000, -1.0)],  # ceiling lands on zero, then negative
)
@pytest.mark.asyncio
...
async def test_an_explicitly_passed_interval_above_the_ceiling_still_raises():
    ...
    A service hardcoding 179 is a programming error, and it should fail loudly
    ...
    with pytest.raises(ValueError, match="heartbeat_interval_seconds"):
        _stage(
            ...
            _Clock(),
            xread_block_ms=2000,
            heartbeat_interval_seconds=179.0,
        )
...
def test_out_of_bound_config_falls_back_to_the_field_default_with_a_warning(
    ...
    """
    caplog.set_level(logging.WARNING, logger="shared.streaming.stage")
    monkeypatch.setenv("CONSUMER_STAGE_HEARTBEAT_INTERVAL_SECONDS", "300")
    ...
    assert config.heartbeat_interval_seconds < LIVENESS_MAX_GAP_SECONDS
    assert _audit_records(caplog, "stream_stage_config_load_failed")


def test_the_copied_bound_is_the_harvesters_liveness_bound():
    """This module's mirror must be the number the harvester actually scores.

    The mirror exists because a live consumer must not fail to start over an
    ops harvest config. The cost of a copy is that it can drift, and it did:
    until 2026-09-23 it mirrored ``observation_max_gap_seconds`` (1800), which
    the harvester applies to gaps between proofs of CONSUMPTION. Once the
    harvester grew a separate ``liveness`` group scoring heartbeats at 180s,
    the copy was defending a bound nothing checked — and
    ``heartbeat_interval_seconds: 300`` passed every guard-rail the code and
    the config comments stated while making every healthy quiet day read
    ``stale_observation``.

    So the assertion is equality against the harvester's DERIVED liveness
    bound, not an inequality against its staleness bound. An inequality is what
    let the two drift three orders of magnitude apart while still passing.
    """
    harvester = harvest.load_observation_config()

    assert harvester.liveness_max_gap_seconds == LIVENESS_MAX_GAP_SECONDS
    # And the staleness bound is a different, larger number that no longer
    # governs this file — pinned so that "they happen to be equal" can never
    # become the reason this test passes.
    assert harvester.observation_max_gap_seconds > harvester.liveness_max_gap_seconds


def test_the_shipped_interval_is_what_the_harvester_predicts():
    """Equality, in both directions, or the density rule measures the wrong thing.

    The harvester predicts how many heartbeats a stretch should hold from its
    OWN ``liveness_expected_interval_seconds``. Set this file's interval lower
    and a healthy day reads stale; set it higher and a dead stretch reads
    alive. Neither failure announces itself anywhere but in the verdict.
    """
    shipped = StreamStageConfig.from_yaml().heartbeat_interval_seconds
    expected = harvest.load_observation_config().liveness_expected_interval_seconds

    assert shipped == expected, (
        f"config/streaming.yaml heartbeats every {shipped}s but "
        f"config/f9_observation.yaml predicts {expected}s"
    )


def test_the_shipped_gap_clears_the_bound_it_is_scored_against():
    """Three real sources, no literals: the claim that actually matters.

    What the harvester measures is the *gap* between two emitted lines —
    interval plus the poll that closes it, plus the second its truncated
    timestamps can add — not the configured interval.
    ...
    119s to push the sum past the bound — but the next reader should know it is
    one service's value rather than a fleet-wide one.
    """
    max_gap = harvest.load_observation_config().liveness_max_gap_seconds
    shipped = StreamStageConfig.from_yaml().heartbeat_interval_seconds
