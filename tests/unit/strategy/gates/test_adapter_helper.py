...
def test_gate_allow_path_not_blocked_and_logs(monkeypatch):
    ...
    logged_rows = []
    monkeypatch.setattr(
        adapter_helper, "_log_decision", lambda **kw: logged_rows.append(kw)
    )
    blocked = adapter_helper.apply_regime_gate(
        gate_cfg=_cfg(),
        decision_signal=_sig("long"),
...
def test_gate_block_path_blocked_and_logs(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "LiveVolInputs",
        lambda **_kw: MagicMock(
            latest_vol_at=MagicMock(return_value=(vol_asof, 75.0)),
            events_within=MagicMock(return_value=[]),
            macro_for=MagicMock(return_value=None),
        ),
    )
    ...
    blocked = adapter_helper.apply_regime_gate(
        gate_cfg=_cfg(threshold=60.0),
        decision_signal=_sig("long"),
        context=_ctx(ts=ctx_ts),
...
def test_log_failure_does_not_propagate(monkeypatch):
    """Decision archive failure must NOT change the gate verdict or raise."""
    from shared.strategy.gates import adapter_helper
...
def test_log_decision_noops_after_external_archive_removal(monkeypatch):
    """The helper keeps verdict logging best-effort but no longer opens DB clients."""
    from shared.strategy.gates import adapter_helper
    ...
    monkeypatch.setattr(
        adapter_helper, "_log_decision", lambda **kw: captured.setdefault("row", kw)
    )
    ...
    assert captured["row"]["strategy"] == "setup_a_gap_reversion"


def test_acquire_infra_clients_returns_redis_when_available(monkeypatch):
    """Happy path: Redis is constructed; event reader is absent until archived."""
    from shared.strategy.gates import adapter_helper
    from shared.streaming.client import RedisClient

    monkeypatch.setattr(
        RedisClient, "get_client", classmethod(lambda _cls: MagicMock(name="redis"))
    )
    redis, event_reader = adapter_helper.acquire_infra_clients()
    assert redis is not None
    assert event_reader is None
