...
async def test_exit_trailing_stop_seeds_from_position_highest_price() -> None:
    # Simulate a process restart: a fresh exit instance (empty extreme map)
    # inherits a position whose highest_price (restored from Redis) is already
    # above entry. The trailing stop must arm immediately on a retrace, not
    # reset to entry.
    exit_strat = _trailing_exit(3.0)
    pos = _Pos("005930", entry_price=10000.0)
    pos.highest_price = 11000.0  # peak before the restart; 3% floor = 10670
    assert not exit_strat._extreme  # fresh instance, no in-memory peak
