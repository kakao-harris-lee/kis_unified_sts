...
_STRONG = {  # SK하이닉스-like
    ...
    "daily_sma_20": 90.0,
    "daily_rsi_14": 74.0,
    "daily_prev_rsi_14": 70.0,
    "daily_macd_hist": 120.0,
}
...
def test_rsi_not_rising_not_strong():
    assert (
        is_strong(
            {**_STRONG, "daily_rsi_14": 60.0, "daily_prev_rsi_14": 65.0},
            StrengthCriteria(),
        )
        is False
    )
...
def test_nan_field_not_strong():
    assert (
        is_strong({**_STRONG, "daily_rsi_14": float("nan")}, StrengthCriteria())
        is False
    )
