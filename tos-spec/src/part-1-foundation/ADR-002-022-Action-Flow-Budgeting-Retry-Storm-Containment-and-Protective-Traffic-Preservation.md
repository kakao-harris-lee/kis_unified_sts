...
ADR-002-024 orders the Action Flow Generation, exact decision/permit/claim state, reserve floors, and all other required generations in one per-send proof. A stale permit, local limiter, queue priority, or previous proof cannot cross a Restrictive Fence Record or a Local Restrictive Latch in `DENY_LATCHED` or `UNKNOWN` state.
