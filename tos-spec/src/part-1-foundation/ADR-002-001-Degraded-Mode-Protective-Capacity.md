...
* it is reduce-only by construction on every governed exposure dimension for the current authoritative (reconciled) position structure, so that it cannot increase any hard-limit exceedance in any credible intermediate or final state (the §6.2-equivalent non-worsening property, established at approval time for the pre-approved emergency-action set rather than recomputed live) without depending on the degraded classifier;
...
### v0.6 — Seam-Sealing (CORPUS-REVIEW-0001 Wave 7)
