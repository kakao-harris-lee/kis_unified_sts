...
> **재결속 기록 (현행 사이클 — 2026-08-29, v2.22 내용 · 에라타 47차 이후)**: 같은 날 **여덟 번째**.
> 재심 #7 은 **findings 2 → 1** 이었고 두 가지를 처음으로 확인해 줬다: ① 46차의 처분과
> **교정한 대조군이 실측상 작동**한다 ② **「명시 잔여 다섯의 단순 재확인은 finding 으로 세지
> 않았다」** — 잔여 등재가 판정에서 실제로 «(b)» 로 취급된 첫 자리다.  남은 1건(주석 속
> 비가시 식별자가 셀을 면제)을 47차(`c2d8fe79`)가 **한 줄(순서)** 로 처분했다.
> 계약 blob **`80b34903a51b…`** · 이전 결속 `41c3da69…` / `c4518f7b…` ·
> `requesting_plan_version` **v2.22 유지**(열 사이클 연속).
>
> **궤적**: findings 4 → 2 → 3 → 2 → 1 → 2 → **1**.  판정의 층: 주장 → 모집단 → 면제 →
...
> **재결속 기록 (현행 사이클 — 2026-08-30, v2.22 내용 · 에라타 49차 이후)**: 날이 바뀌었다
> — 이 연속 트랙의 **열 번째**이자 08-30 의 첫 번째다.  재심 #9 는 48차가 신설한 셀 수
> 래칫의 **«출처»가 결속되지 않았다**고 짚었다: 문서에 셀을 더하면서 기준값도 같이 올리면
> **위반 0건**이다(오케스트레이터 독립 재현 — PASS).  더 나쁜 실측은 기입값 17 자체가
> **워킹트리를 재고 커밋 핀은 그대로 둔** 값이었다는 것이다.  48차 문언 「C2U 래칫과 같은
> 규율」은 참이 아니었다 — C2U 는 그 규율을 산문이 아니라 **기계**(`check_c2up`)로 진다.
> **문언이 능력보다 넓었던 네 번째 자리**다.
>
> 49차(`5fc3ac00` 계약 문서 · `9f31605e` 검사기)의 처분: **기입 정수는 권위가 아니라
> «검증 대상 주장»** 이 된다.  기준은 `measured_against` 가 가리키는 **불변 blob** 이고,
> **기입값과 워킹트리 행을 둘 다** 그 blob 에 대조하며, 측정은 `derive_fixture_row_cells`
> **하나**로만 한다.  재측정 불가·기준선 부재·픽스처 행이 정확히 1회가 아님은 전부 red.
> 계약 blob **`8b54e1bcf522…`** · 이전 결속 `6f5cfce1…` / `f35abcc5…` ·
> `requesting_plan_version` **v2.22**(열두 사이클 연속).
>
> **동결의 형태가 이번 판에서 바뀌었다**: 문서를 먼저 커밋해야 그 blob 에 provenance 를
> 결속할 수 있으므로 에라타(`5fc3ac00`)와 검사기(`9f31605e`)를 **두 커밋으로 갈랐다**.
> 동결 실측 = **핀이 가리키는 blob 과 HEAD 의 blob 이 동일**(`8b54e1bc…`) · 검사기 rc 0 ·
> `--self-test` **137종 전건 판별 · 죽은 검사 0** · 미앵커 좌표 67 불변 · 닫힌 표 32 불변 ·
> 픽스처 셀 17 불변(래칫이 헐거워지지 않는 방향).
>
> **궤적**: findings 4 → 2 → 3 → 2 → 1 → 2 → 1 → 1 → **1**.  층은 다시 한 칸 내려갔다:
> 면제의 단위 → 판별 순서 → 운반체 → 정체의 종류 → **정체의 «출처»**.  대조군 135 → **137종**
> (문서와 기준값을 «함께» 올리는 paired mutant = 재심 #9 가 이름 지어 요구한 것).
>
> **부수 적발(정직)**: 48차가 `fixture_row_cells` 를 필수 키로 만들면서 **기준선 픽스처
> 상속을 하지 않았다** — 「한 가지만 바꾼다」 규율 위반이다.  상속 없는 대조 실행에서 모든
> 기준선 픽스처가 새 축에서 red 가 되어 계수가 포화하고 **대조군 둘이 죽었다**(실측).
> 새 축을 세우는 판은 **기존 픽스처가 그 축에서 조용한지**까지가 한 단위다.
>
> **등재 잔여(신규·정직)**: 핀을 «셀이 이미 늘어난 커밋»으로 옮기면 통과한다 — 정당한
> 전이와 세탁을 기계는 구별하지 못하며, 구별하는 것은 그것이 diff 에 보이는 사람의 기록
> 행위라는 사실뿐이다(C2U 가 사는 체제와 같다).  **48차에서 불변인 잔여**: 기존 셀 «안»에
> 숨기는 것은 여전히 탐지되지 않는다.
>
> **S-26 (2) 카운터는 열 판 연속 0.**
>
> **[에라타 — 재심 #10 이 위 문언을 반증했다 · 50차 처분]** 위 기록의 「기준은
> `measured_against` 가 가리키는 **불변 blob**」은 **참이 아니었다**: 그 필드의 `kind` 는
> 불변 값(`commit`)과 **가변 값(`worktree`)을 둘 다** 받았고, 가변 값이면 커밋 blob 이 아니라
> 워킹트리 파일을 읽었다.  셀 추가·기준값 갱신·`kind` 전환을 함께 하면 **위반 0건**이다
> (검사기 실행 실측: `commit` 이면 2건 red · `worktree` 면 0건 + rc 를 올리지 않는 NOTICE 하나).
> **문언이 능력보다 넓었던 다섯 번째 자리**다.  50차가 게이트 경로의 기본값을 뒤집어
> 처분했다(불변 출처만 받는 것이 기본 · 가변 출처는 측정 표시 경로만 명시 요청).
> **이 줄은 기록을 고쳐 쓰지 않는다** — 당시 기입은 그대로 두고 반증 사실만 덧붙인다.

> **재결속 기록 (현행 사이클 — 2026-08-30, v2.22 내용 · 에라타 50차 이후)**: 이 연속 트랙의
> **열한 번째**이자 08-30 의 두 번째다.  재심 #10 은 49차가 세운 축의 **«출처»가 불변이 아니었다**고
> 짚었다: `measured_against` 의 `kind` 는 `commit` 과 `worktree` 를 둘 다 받았고, 가변 값이면
> 커밋 blob 이 아니라 워킹트리를 읽었다.  셀 추가·기준값 상향·`kind` 전환을 함께 하면 통과였다.
> 49차 문언 「기준은 불변 blob」은 참이 아니었다 — **문언이 능력보다 넓었던 다섯 번째 자리**.
>
> **50차의 요지 = 잠금을 표면이 아니라 저작 레벨에 둔 것**: `read_baseline_source` 의 기본값을
> **«불변 출처만»** 으로 뒤집고, 가변 출처는 측정 표시 경로(`measure_baseline`) **하나**만
> 명시 요청하게 했다.  **호출자별 가드는 명시 기각**했다 — 다음 축이 빠뜨리면 구멍이 조용히 다시
> 열리고, 그것이 이 아크가 반복해 온 «부분 스윕» 결함이기 때문이다.  귀결로 worktree 기준선은
> C2UP·CAP2-FIXTURE **두 축 모두에서 red**(각 1건)이며, 이는 산문이 아니라 **실저장소 루트 실측**이다.
> 계약 blob **`ecbd478e16f3…`** · 이전 결속 `66b34272…` / `9f31605e…` ·
> `requesting_plan_version` **v2.22**(열세 사이클 연속).
>
> **이 판은 «전수 스윕»을 처음부터 세 자리 전부에 적용했다**: 검사기(`7c200c1c`) · 계약 문서
> (`c8beea93`) · **이 문서의 49차 재결속 기록**(`a62ece7f` — 기록을 고쳐 쓰지 않고 반증만 덧붙였다).
> 「규칙 신설 = 전수 적용까지가 한 단위」가 이 아크에서 여섯 번 깨졌던 자리다.
>
> **동결 실측**: 핀 blob == HEAD blob(`ecbd478e…`) · 검사기 rc 0 · `--self-test` **141종 전건
> 판별 · 죽은 검사 0** · 미앵커 67 / 닫힌 표 32 / 픽스처 셀 17 전부 불변 · 대조군 137 → **141종**.
> `--measure-baseline worktree` 는 이제 붙여넣기용 출처 블록을 내지 않는다 — **그 부재가 신호**다.
>
> **궤적**: findings 4 → 2 → 3 → 2 → 1 → 2 → 1 → 1 → 1 → **1**.  층: 면제의 단위 → 판별 순서 →
> 운반체 → 정체의 종류 → 정체의 «출처» → **출처의 «불변성»**.
>
> **측정 기록(정직) — 이 판이 스스로 여섯 번째가 될 뻔한 자리**: 계약 문서에 「두 축 모두 red」를
> 적어 커밋한 «뒤» 잰 첫 실측은 C2UP 만 red 였다.  스크래치 하네스 루트에 RULE manifest 가 없어
> **픽스처 축이 아예 돌지 않았기** 때문이고, 실저장소 루트로 다시 재서 판별했다.  **하네스의 한계를
> 기계의 한계로 읽으면 문언이 능력보다 넓어진다.**  또 스윕 점검에서 naive grep 이 «반증이 붙은
...
> **재결속 기록 (현행 사이클 — 2026-09-04, v2.22 내용 · 에라타 52차 이후)**: 이 연속
> 트랙의 **열세 번째**다.  52차는 **§7.4 D-1 처분 어휘의 두 공백**을 닫았다 —
> 다중 키 사이트에서 「정확히 하나의 처분」의 **단위를 «사이트»에서 «선언된 키»로**
> 재정의하고(D-3), 「의존 키가 없다」는 사례에 **처분 이름 하나**를 신설했다
> (D-4 `NO_DEPENDENCY` — 검사기의 구조 스캔이 참조 0 을 세웠을 때만 파생).
> 근거는 레인 A 심판 `review-mtlo6mst-93vt2j` finding 1(채택 3/3)이고, 편집 허가는
> **운영자 지시 2026-09-04**(「§7.4 에라타 저작 + 레인 B 재심」)다.  이 재결속은
> **결속 경로 두 파일 중 계약 자신의 내용 변경**에 대한 것이다 — O-6 의 정상 거동.
>
> **이 재결속이 결속하는 내용**: 계약 blob `18d04368…` · **10,286행**(52차에서
> +130/−9) · 개발계획 blob `ec3464c0…` · **612행**(**무접촉** — 이 판의 lockstep
> 스윕에서 §7.4 처분 규칙·D0-5 완료 문언의 재기술이 그 문서에 **0건**임을 전수
> 확인했다).  계약 blob 이 담는 것: §7.4 판정 절차 블록의 단위 재정의 · 처분 어휘
> 블록의 `NO_DEPENDENCY` 항 · v1.6 교정 문장과 v1.8 완료 기준의 대체 마커 ·
> 에라타 52차 본체(D-3/D-4/D-5 + 기각 근거 둘 + 정직 경계) · S-26 의 52차 자기
> 적용 항 · §11 D0-5 종료조건 행 · `현행(52차 이후)` 커런시 태그 스윕 4곳 ·
> 좌표 드리프트 정정 **여섯 좌표**(2943→3053 · 5103→5224 · 5360→5481 · 7245→7366 ·
> 7312→7433 · 7393→7514 — 인용 자리는 `:264` 와 `:7434` 둘).
>
> **계약 «밖» lockstep 한 층**: 계약 행수가 10,234 를 넘어서면서
> `tools/tos_contract_check.py` 의 self-test 픽스처 `C2R-capture-5digit-coordinate`
> 가 **죽은 검사**가 됐다(5자리 좌표 `:10234` 가 이제 범위 «안»이라 발화하지 않는다).
> `:12345` 로 회전했다 — 절단값 `:1234` 는 여전히 범위 안이라 «절단 fail-open 판별»
> 이라는 그 대조군의 하중은 보존된다.  51차의 3차 층(테스트 digest 리터럴)과 같은
> 계열이다: **계약 문서의 크기·좌표에 값이 결속된 자리는 계약 편집의 lockstep 이다.**
>
> `bound_set_digest e0729ff3ccbbab41b007464742290e4e875c07846b5a87d228727abc2ae4480f`
> → **`961135fd91c80b27d6f2822d80411a5212c94ca90442b96d76d53f1be382c41e`** ·
> `decided_at_head` 는 **재결속 편집 «직전» 실측 HEAD(= C1 커밋)** 로 오케스트레이터가
> 기입했다(`76a4e423` — 저작 시점에는 그 커밋이 아직 없어 자리표시자로 두었다가 C1 커밋 직후 채웠다) ·
> `requesting_plan_version` 은 **v2.22 로 유지**(열다섯 사이클 연속 — 에라타는 버전을
> 올리지 않는다).  `disposition`/`deferred_scope` 는 **무접촉**이다.
>
> **U-12 원장(`OQ-11-RAISE-LEDGER.md`)에는 행을 쓰지 않는다** — 51차와 같은 형상이다:
> 트리거(bound_set_digest 불일치)가 C1..C2 구간에서 순간 성립하고 원장은 append-only 라
> 잘못 쓴 행을 되돌릴 수 없으므로, 기입 여부는 운영자 판단으로 남긴다.
>
> **동결 실측**: 이 아티팩트를 편집한 «뒤» digest 를 재계산해 **불변**임을 확인했다 —
> `tos-spec/` 은 결속 경로가 아니라 피측정 범위 «밖»이다(측정자를 피측정 범위 밖에 둔다).
>
> **S-26 ② 카운터는 52차 계약 편집 자체로 이미 0 이다**(⑥ 의 자기 적용 — 계약 에라타
> 52차 항이 명시했다).  이 재결속은 그 리셋을 되돌리지 않는다.
>
> D0/P-0 착수 금지 불변 · `restricted_live`·`production` 권한 불변.

> **재결속 기록 (현행 사이클 — 2026-09-04, v2.22 내용 · 에라타 53차 이후)**: 이 연속
> 트랙의 **열네 번째**이며, **같은 날 두 번째**다.  52차의 D-4 가 레인 B 재심에서
> high 1 을 받았다(`review-mtmbw5f3-xpo98b` finding 1 · 채택 1/1) — `NONE` 스캔의 후보를
> «VER-002 우주 키»로만 잡아, 그 우주 «밖»의 의존 키를 선언 대신 `NONE` 으로 적으면
> `UNBOUND` + §13 등재를 `NO_DEPENDENCY` + 면제로 바꿀 수 있었다.  53차는 후보 우주를
> **VER-002 우주 키 ∪ §7.1 전 사이트 선언 키(CONTRAST 포함)** 로 닫고, 의존의 정의를
> D-1(docstring 리터럴 인용)로 환원하며, 토큰화를 원문 전 행·구분 없는 단어 경계
> 정규식으로 못박았다.  이 재결속은 **결속 경로 두 파일 중 계약 자신의 내용 변경**에
> 대한 것이다 — O-6 의 정상 거동.
>
> **이 재결속이 결속하는 내용**: 계약 blob `f88b251c…` · **10,353행**(53차에서 +80/−13) ·
> 개발계획 blob `ec3464c0…` · **612행**(**무접촉** — 52차 스윕에서 §7.4 처분 규칙·D0-5
> 완료 문언의 재기술이 그 문서에 0건임을 이미 전수 확인했고, 53차는 그 판정을 바꾸지
> 않는다).  계약 blob 이 담는 것: D-4 (가)(나)(다) · 처분 어휘 블록의 NO_DEPENDENCY 행 ·
> 52차 «정직 경계» 문단의 이관 표시 · «대조군과 발효 시점» 의 네 번째 red 대조군 ·
> 53차 블록 · S-26 의 53차 재적용 항 · `현행(53차 이후)` 커런시 태그 4곳 · 좌표 드리프트
> 정정 여섯 좌표(3053→3110 · 5224→5291 · 5481→5548 · 7366→7433 · 7433→7500 · 7514→7581
> — 인용 자리는 `:264` 와 `:7501` 둘).
>
> **계약 «밖» lockstep 은 이번엔 한 층뿐이다**: 테스트의 digest 핀.  계약 행수는
> 10,349 로 늘었으나 self-test 5자리 좌표 픽스처의 임계(`:12345`)를 넘지 않아 52차가
> 회전시킨 값이 그대로 유효하다 — 죽은 검사 0 을 실측으로 확인했다.
>
> `bound_set_digest 961135fd91c80b27d6f2822d80411a5212c94ca90442b96d76d53f1be382c41e`
> → **`4c65d626bb034263f66312426d3f1bde551baff8558718bb2ba0205525ad982c`** ·
> `decided_at_head` 는 **재결속 편집 «직전» 실측 HEAD(= 53차 C1 커밋)** 로 오케스트레이터가
> 기입한다 — 저작 시점에는 그 커밋이 아직 없다 ·
> `requesting_plan_version` 은 **v2.22 로 유지**(열여섯 사이클 연속 — 에라타는 버전을
> 올리지 않는다).  `disposition`/`deferred_scope` 는 **무접촉**이다.
>
> **U-12 원장(`OQ-11-RAISE-LEDGER.md`)에는 행을 쓰지 않는다** — 51차·52차와 같은 형상이다:
> 트리거(bound_set_digest 불일치)가 C1..C2 구간에서 순간 성립하고 원장은 append-only 라
> 잘못 쓴 행을 되돌릴 수 없으므로, 기입 여부는 운영자 판단으로 남긴다.
>
> **동결 실측**: 이 아티팩트를 편집한 «뒤» digest 를 재계산해 **불변**임을 확인했다 —
> `tos-spec/` 은 결속 경로가 아니라 피측정 범위 «밖»이다.
>
> **S-26 ② 카운터는 0 에 머문다** — 52차가 0 으로 만들었고 53차의 ⑥ 재적용이 그것을
> 되돌리지 않는다(계약 에라타 53차 항이 명시했다).
>
> D0/P-0 착수 금지 불변 · `restricted_live`·`production` 권한 불변.

> **재결속 기록 (현행 사이클 — 2026-09-04, v2.22 내용 · 에라타 54차 이후)**: 이 연속
> 트랙의 **열다섯 번째**이며, **같은 날 세 번째**다.  53차의 후보 우주가 레인 B 재심 #2
> 에서 **«부분 해소»** 판정을 받았다(`review-mtmcoagb-uym46j` finding 1 · 채택 1/1) —
> 후보 우주가 사이트들의 D-5 선언에서 파생되므로, donor 선언에서 키를 지우면 후보
> 집합이 먼저 줄어 `NONE` 사이트가 같은 리터럴을 품고도 통과한다.  54차는 **열거를 더
> 넓히지 않는다**(우주 밖 의존 이름의 전수 집합은 저작자 독립적으로 열거 불가) — 대신
> **우회의 이득을 없앤다**: `NO_DEPENDENCY` 가 `UNBOUND` 와 **같은 급의 §13 개별 등재
> 의무**(U-6′)를 지고, 완료 근거가 「NONE 선언 + 스캔 0건 + §13 행 실재」의 논리곱이
> 된다.  이 재결속은 **결속 경로 두 파일 중 계약 자신의 내용 변경**에 대한 것이다 —
> O-6 의 정상 거동.
>
> **이 재결속이 결속하는 내용**: 계약 blob `7da8199c…` · **10,433행**(54차에서 +97/−17) ·
> 개발계획 blob `ec3464c0…` · **612행**(**무접촉** — §7.4 처분 규칙·D0-5 완료 문언의
> 재기술이 그 문서에 0건이라는 52차 스윕 판정을 54차도 바꾸지 않는다).  계약 blob 이
> 담는 것: D-4 말미의 등재 의무 전환 · **U-6′ 신설**(§7.4 U-6 자리) · 처분 어휘 블록의
> NO_DEPENDENCY 행 · 52차 «§13 귀결» 문단의 폐기 표시(UNCHK-026 은 상설 공시 행으로
> 남는다) · «UNBOUND 재사용하지 않은 이유» (ㄴ)의 폐기 표시 · 다섯 번째 red 대조군
> (donor 선언 삭제 시나리오) · 54차 블록 · S-26 의 54차 재적용 항 · §11 두 행
> (D0-5 종료조건 · U-6) · `현행(54차 이후)` 커런시 태그 4곳 · 좌표 드리프트 정정
> 여섯 좌표(3110→3178 · 5291→5371 · 5548→5628 · 7433→7513 · 7500→7580 · 7581→7661 —
> 인용 자리는 `:264` 와 `:7581` 둘).
>
> **계약 «밖» lockstep 은 이번에도 한 층뿐이다**: 테스트의 digest 핀.  계약 행수는
> 10,433 으로 늘었으나 self-test 5자리 좌표 픽스처의 임계(`:12345`)를 넘지 않아 52차가
> 회전시킨 값이 그대로 유효하다 — 죽은 검사 0 을 실측으로 확인했다.
>
> `bound_set_digest 4c65d626bb034263f66312426d3f1bde551baff8558718bb2ba0205525ad982c`
> → **`8988849e7673a02001d11b4cadff8b65e1b7ac78c9ef2cbbaa0dd454809b730a`** ·
> `decided_at_head` 는 **재결속 편집 «직전» 실측 HEAD(= 54차 C1 커밋)** 로 오케스트레이터가
> 기입한다 — 저작 시점에는 그 커밋이 아직 없다 ·
> `requesting_plan_version` 은 **v2.22 로 유지**(열일곱 사이클 연속 — 에라타는 버전을
> 올리지 않는다).  `disposition`/`deferred_scope` 는 **무접촉**이다.
>
> **U-12 원장(`OQ-11-RAISE-LEDGER.md`)에는 행을 쓰지 않는다** — 51차~53차와 같은 형상이다:
> 트리거가 C1..C2 구간에서 순간 성립하고 원장은 append-only 라 잘못 쓴 행을 되돌릴 수
> 없으므로, 기입 여부는 운영자 판단으로 남긴다.
>
> **동결 실측**: 이 아티팩트를 편집한 «뒤» digest 를 재계산해 **불변**임을 확인했다 —
> `tos-spec/` 은 결속 경로가 아니라 피측정 범위 «밖»이다.
>
> **S-26 ② 카운터는 0 에 머문다** — 52차 이래 세 판 연속 계약 편집이며 어느 판도 그
> 카운터를 세우지 못했다(계약 에라타 54차 항이 명시했다).
>
> D0/P-0 착수 금지 불변 · `restricted_live`·`production` 권한 불변.

> **재결속 기록 (현행 사이클 — 2026-09-04, v2.22 내용 · 에라타 55차 이후)**: 이 연속
> 트랙의 **열여섯 번째**이며, **같은 날 네 번째**다.  54차가 레인 B 재심 #3 에서
> **«회피»** 판정을 받았다(`review-mtmd8qat-c1u166` finding 1 · 채택 1/1) — 공시 의무만
> 지운 것은 「의존 없음」이라는 **의미 판정**을 여전히 저작자에게 맡긴 것이고,
> `UNBOUND` 행이 공개하는 실제 키·소관 아티팩트에 견주면 NONE 행은 덜 말하고 더 얻는다.
> 운영자가 **선택지 A** 를 택해 55차는 `NO_DEPENDENCY` 완료를 **독립 리뷰 provenance**
> 에 결속한다: 완료 근거가 **넷의 논리곱**(NONE 선언 + 스캔 0건 + U-6′ 1:1 문법의 §13
> 행 + 저작자 아닌 심판의 기록)이 되고, 그 기록은 canonical `site_id` 와 **사이트 범위
> 소스 리비전**에 결속된다.  U-6′ 에는 1:1 문법(`axis` 가 `D0-5 NONE: <site_id>` 로
> 시작 · 사이트당 정확히 한 행 · 행당 최대 한 사이트 · (1)~(4) 동일 행)을 못박았다.
> 이 재결속은 **결속 경로 두 파일 중 계약 자신의 내용 변경**에 대한 것이다 — O-6 의
> 정상 거동.
>
> **이 재결속이 결속하는 내용**: 계약 blob `05a36619…` · **10,573행**(55차에서 +152/−12) ·
> 개발계획 blob `ec3464c0…` · **612행**(**무접촉**).  계약 blob 이 담는 것: D-4 말미의
> 네-조건 완료 근거 + (라) canonical site_id 일곱 + (마) 기록 형식·위치·검사 · U-6′ 의
> 1:1 문법과 (4) · 대조군 ⑤-a/⑤-b/⑥/⑦/⑧ 과 양성 하나 · 55차 블록 · S-26 의 55차
> 재적용 항 · §11 두 행 · `현행(55차 이후)` 커런시 태그 4곳 · 좌표 드리프트 정정 여섯
> 좌표(3178→3305 · 5371→5511 · 5628→5768 · 7513→7653 · 7580→7720 · 7661→7801 —
> 인용 자리는 `:264` 와 `:7721` 둘).
>
> **한 가지 «게이트와 다른» 선택을 명시한다**: 기록의 `reviewed_scope_digest` 는
> `plan_scope_digest` 가 «아니라» 그것에서 **`git rev-parse HEAD` 한 줄을 뺀**
> content-only 형태다.  게이트 digest 는 한 심사 사이클에 붙어 살고 죽지만 이 기록은
> 리비전을 건너 살아야 하므로, HEAD 를 넣으면 **무관한 커밋 하나에도 기록이 무효**가
> 되어 네 조건이 영구히 서지 못한다.  계약 (마)가 그 이유를 축자로 적는다.
>
> **계약 «밖» lockstep 은 이번에도 한 층뿐이다**: 테스트의 digest 핀.  계약 행수는
> 10,566 으로 늘었으나 self-test 5자리 좌표 픽스처의 임계(`:12345`)를 넘지 않아 52차가
> 회전시킨 값이 그대로 유효하다 — 죽은 검사 0 을 실측으로 확인했다.
>
> `bound_set_digest 8988849e7673a02001d11b4cadff8b65e1b7ac78c9ef2cbbaa0dd454809b730a`
> → **`e260c339f63bb0bbcbb56dfc0d6b58b6058976f2e52a8f6f7bdd3aecc6d2691b`** ·
> `decided_at_head` 는 **재결속 편집 «직전» 실측 HEAD(= 55차 C1 커밋)** 로 오케스트레이터가
> 기입한다 — 저작 시점에는 그 커밋이 아직 없다 ·
> `requesting_plan_version` 은 **v2.22 로 유지**(열여덟 사이클 연속).
> `disposition`/`deferred_scope` 는 **무접촉**이다.
>
> **U-12 원장(`OQ-11-RAISE-LEDGER.md`)에는 행을 쓰지 않는다** — 51차~54차와 같은 형상이다.
>
> **동결 실측**: 이 아티팩트를 편집한 «뒤» digest 를 재계산해 **불변**임을 확인했다 —
> `tos-spec/` 은 결속 경로가 아니라 피측정 범위 «밖»이다.
>
...
> **재결속 기록 (현행 사이클 — 2026-09-05, v2.22 내용 · 에라타 57차 이후)**: 이 연속
> 트랙의 **열여덟 번째**다.  2026-09-04 운영자 지시로 S-26 재심이 재개돼 계약 무편집
> 상태에서 다섯 판이 돌았고(전부 needs-attention · material 1 · 전부 검사기
> `tools/tos_completion_status.py` 의 U-6′ (ㄹ) 검사 한 함수) — 네 판은 «포함» 문법을
> 지키려다 모순 구획 병기 우회를 냈고, 다섯째 판(`review-mtnw4hor-zdytup`)은 닫힌 세계로
> 우회를 구조적으로 닫았으나 문언보다 강해졌다(정본 넷 뒤의 부수 설명이 red · S-26 ⑤
> 불성립).  운영자가 「5회차 결과 무관 종료 · 잔여 UNCHK 등재」(→ `UNCHK-027`) 뒤
> 「배타 문법 계약 명문화」를 지시했고, 57차는 U-6′ 에 **(ㅁ) reason 의 배타 문법**을
> 신설했다: reason 은 구획(` · `/`;`/LF)의 다중집합이고 정본 구획 넷(`선언: VER-002-KEYS:
> NONE` · `스캔 결과: 후보 우주 <N>개, 스캔 범위 <scope>, 파일 <F>개` · 경계 문장 축자 ·
> `독립 리뷰 기록: <record_path>`)과 **정확히 같아야** 한다 — 각 1회 · byte-exact ·
> 정규화 없음 · 그 밖의 구획 0 · 순서·구분자 자유 · 라벨 «인식» 구현 금지.  대조군
> ⑧-d(정본 넷 + 부수 구획 → red) · ⑧-e(정본 넷 + 라벨 변형 모순 구획 병기 → red) ·
> ⑧-f(순서 변경·구분자 혼용 → green).  이 재결속은 **결속 경로 두 파일 중 계약 자신의
> 내용 변경**에 대한 것이다 — O-6 의 정상 거동.
>
> **이 재결속이 결속하는 내용**: 계약 blob `3411f241…` · **10,705행**(57차에서 +89/−6) ·
> 개발계획 blob `ec3464c0…` · **612행**(**무접촉**).  계약 blob 이 담는 것: U-6′ (ㅁ) ·
> 대조군 ⑧-d/⑧-e/⑧-f · 57차 블록(§7.4) · S-26 의 57차 재적용 항 · `현행(57차 이후)`
> 커런시 태그 3곳 **[58차 재결속 기록에서 정정 — 실측 4곳(`:110` 포함) · 재심 #8 finding 1]** · 좌표 드리프트 정정 여섯 좌표(3346→3415 · 5560→5643 · 5817→5900 ·
> 7702→7785 · 7769→7852 · 7850→7933 — 인용 자리는 `:264` 와 `:7853` 둘).
>
> **검사기는 이 재결속 «전»에 이미 문언과 같은 강도였다**(`1fd98450` · 닫힌 세계) —
> 이번 사이클은 「구현이 더 충실하면 에라타가 정답」의 사례라 검사기 코드 변경 0 이고,
> C4 lockstep 은 `UNCHK-027` 행의 종결 문언과 생성물 재생성만이다.
>
> **계약 «밖» lockstep 은 이번에도 한 층뿐이다**: 테스트의 digest 핀(C1 에 동봉).
> 계약 행수 10,705 는 self-test 5자리 좌표 픽스처의 임계(`:12345`)를 넘지 않아 52차가
> 회전시킨 값이 그대로 유효하다 — self-test 145종 PASS 를 C1 직전 실측했다.
>
> `bound_set_digest 4e6c975f794696066a25abe4ee827594afa18f8fac8bfb5e7bf31d43508b3c2f`
> → **`16e97f44e9b32882d03c7d9327a2d088ef5f4b385e6b3823aa2e92fb59cf4f46`** ·
> `decided_at_head` = **재결속 편집 «직전» 실측 HEAD(= 57차 C1 커밋 `5dfcb236`)** ·
> `requesting_plan_version` 은 **v2.22 로 유지**(스무 사이클 연속).
> `disposition`/`deferred_scope` 는 **무접촉**이다.
>
> **U-12 원장(`OQ-11-RAISE-LEDGER.md`)에는 행을 쓰지 않는다** — 51차~56차와 같은 형상이다.
>
> **동결 실측**: 이 아티팩트를 편집한 «뒤» digest 를 재계산해 **불변**임을 확인했다.
>
...
> 문단 · S-26 의 58차 재적용 항 · `현행(58차 이후)` 커런시 태그 **4곳**(`:110` · `:5660` ·
> `:5661` · `:5871` — 재심 #8 `review-mtnydobs-g6hrlt` finding 1 정정: 초판은 3곳으로
> 과소 계수했다 · 57차 기록의 같은 계수도 아래 마커로 정정) · 좌표 드리프트 정정
