...
Under ADR-002-029, the actual compiler, verifier, mapping, schema, serializer, SDK, signer component, dependency/toolchain closure, and runtime-loaded bytes SHALL belong to the exact current Admitted Release Set and Runtime Artifact Attestation. Semantic conformance does not self-admit the binary that computed it, and software admission does not replace this ADR's exact command proof.
