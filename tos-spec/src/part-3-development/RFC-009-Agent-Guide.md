...
**Title:** Agent Guide
**Version:** 0.2
**Status:** Ratified (2026-07-18 — GOV-001 G5 record RR-0011, ARCHITECTURE-GATE-STATUS §9.7; ratification confers no live authorization, no ADR acceptance, and no capacity)
...
These questions remain open notwithstanding RFC-009's `Ratified` status (2026-07-18 —
GOV-001 G5 record RR-0011); ratification closes no open question. Each is addressed by a proposed
...
* **What changed.** RFC-009 has been `Ratified` since 2026-07-18 (GOV-001 G5 record RR-0011,
...
* **System Owner materiality determination, 2026-08-05 — NON-MATERIAL under GOV-001 G6.**
  This is the RFC-009 application of a batch determination of that date covering every
  post-ratification currency correction then outstanding; it was applied only after the
  RFC-009 diff was read on its own terms. **Verified grounds (from the `83987c7d` diff of
  this file).** The commit touches RFC-009 in exactly two places: the §14 preamble and the
  appended Review History entry. The single replaced sentence swaps the false "open while
  RFC-009 is a Review Draft" for the actual `Ratified` status (RR-0011) plus "ratification
  closes no open question". The proposed-ADR-DEV rider ("a proposed ADR-DEV does not resolve
  its question — resolution follows acceptance") and "None SHALL be resolved by informal
  authoring-actor convention" survive verbatim. RFC-009 is the agent guide and adds no
  enforcement of its own — it rests "entirely on the enforcement, review, and admission
  points already defined upstream", the line immediately above the edited section, which the
  diff leaves untouched — so the correction cannot have moved an enforcement point. No
  SAFE-xxx requirement, numeric bound, authority, gate, mode transition, traceability row, or
  evidence identifier appears in the diff. **Consequence:** GOV-001 G6 is not engaged — no
  RFC-000 §18 amendment and no re-ratification of RFC-009 are owed, retroactively or
  prospectively; `**Version:** 0.2` and `**Status:** Ratified` (RR-0011) stand unchanged, and
  the cited-version pins are unchanged. **Metadata.** `83987c7d` left `**Last Updated:**` at
  2026-07-17 while editing the §14 body; the field is set to 2026-08-05, the date of this
  determination. **Scope and limits:** this determination covers the `83987c7d` RFC-009 §14
