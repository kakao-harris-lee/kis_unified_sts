"""Strategy configuration endpoints (Phase 5: read-only listing).

After Phase 5 of the dashboard redesign, the write-side strategy management
UI was removed. Strategy editing now happens via YAML files committed to the
repository — there is no longer a runtime CRUD surface. This module keeps
only the list endpoint that powers strategy filters in the read-only
cockpit/positions/signals/trades pages.
"""
