...
- **현재 운용 전략**:
  ...
  - **`rl_mppo` — DECOMMISSION TARGET** (`enabled: false`, shadow logging 종료). 사유 정정(v4.11): "매 cycle 0 signals → HOLD bias"는 **오독** — 실제 entry action 55%/conf~0.56, "0 signals"는 shadow_mode 설계상 억제. 유효 사유는 **counterfactual EOD-proxy PnL 음수**(5/11–15 9 trades -1.35M) + Setup A/C 채택. 2026-06-03 결정으로 retraining/aux 보존 방향도 폐기하며, 코드 경로(`RLMPPOEntry/RLMPPOExit/rl_model_helpers`)는 제거 대상이다. 상세: `docs/plans/2026-06-03-ml-rl-removal-llm-indicator-futures.md`.
...
- **핵심 합의 사항**
  ...
  - 선물 청산은 Setup A/C 전략 자체의 청산 로직 또는 ATR/momentum/strategy-native exit를 사용한다. ~~`rl_mppo_exit`~~ 제거 대상. 규칙 기반 `three_stage`는 주식 전용이다.
...
#### RL 선물 운용 규칙 — **RETIRED / DECOMMISSION TARGET**
...
- `RL_MPPO_MODEL_PATH`, `RLMPPOEntry`, `RLMPPOExit`, `shared/strategy/rl_model_helpers.py`는 제거 대상이다.
- `sts rl *`, `sts tft *`, `shared/ml/rl/`, `shared/ml/tft/`, `config/ml/rl_*`, `config/ml/tft.yaml`은 decommission/archive 대상으로 관리한다.
...
계층적 RL은 active roadmap이 아니다. `shared/ml/rl/hierarchical/`와 관련 CLI/config/tests는 ML/RL 제거 계획의 decommission 대상이다. 아래 설명은 historical context로만 둔다.
...
**CLI 명령어**: `sts rl train-hierarchical`, `sts rl evaluate-hierarchical`는 decommission 대상이다. 새 작업에서 사용하지 않는다.
...
│   │   └── futures/                 # setup_a/c, williams_r_15m, bb_reversion_15m, legacy rl_mppo removal target
│   ├── exit/                        # 청산: three_stage (주식), ATR/momentum/strategy-native (선물), legacy rl_mppo_exit removal target
│   ├── kis/                         # KIS API 인증
│   └── ml/                          # legacy ML/RL configs, decommission target
│
├── shared/                          # 📁 공유 모듈 (핵심)
...
│   │   ├── rl_model_helpers.py      # legacy RL helper, decommission target
│   │   ├── entry/                   # 진입 전략 7개
│   │   ├── exit/                    # 청산 전략 (three_stage, momentum_decay, ATR/indicator exits, legacy rl_mppo_exit removal target)
...
│   ├── ml/rl/                       # legacy RL 구현, decommission target
...
| `rl_mppo` | `RLMPPOEntry` | Legacy RL Maskable PPO 진입 (선물) — decommission target |
...
| `rl_mppo_exit` | `RLMPPOExit` | Legacy RL 청산 정책 (선물) — decommission target |
...
- `rl-pipeline` — **RETIRED / DECOMMISSION TARGET** (신규 작업에 사용하지 않음)
...
# ML/RL commands
# sts rl * / sts tft * are decommission targets. Do not use for new work.
