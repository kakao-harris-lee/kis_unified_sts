"""§7.1 import-closure verification for ``tos.protective`` (design #11 §7.1).

Isomorphic to the canonical / rcl / authority / liveauth / recon / brokercap closure tests. It
imports every ``tos.protective`` submodule in a **fresh, spawned interpreter** (``subprocess``
/ ``os`` are firewall-forbidden even in tests) and asserts:
  ...
  3. ``numpy`` / ``pandas`` / ``yaml`` are absent (bounds are injected; YAML parsing is the
     harness's concern — design #11 §0.3/§7.1).
  4. **All ten consumers / siblings are absent** — ``tos.rcl`` / ``tos.authority`` /
     ``tos.orthostate`` / ``tos.liveauth`` / ``tos.recon`` / ``tos.evidence`` / ``tos.capsule``
     / ``tos.time`` / ``tos.dsl`` / ``tos.brokercap`` (design #11 §3.4/§3.5 — protective keeps
     a **sibling edge of 0**, producing bools / injected coordinates instead of importing any).
  5. **``tos.canonical`` AND ``tos.ordering`` ARE present** — the two core substrate
     ...
     forbid them.
  6. No ``tos.protective`` source references ``os.environ`` / ``os.getenv`` or a dynamic escape
...
_PROTECTIVE_SRC = (
    Path(__file__).resolve().parent.parent.parent / "src" / "tos" / "protective"
)

_DYNAMIC_CALL_NAMES = frozenset({"exec", "eval", "__import__"})
_AMBIENT_ENV_ATTRS = frozenset({"environ", "getenv"})
#: Real-clock modules that must never be imported by a clock-free protective source (§0.2).
_CLOCK_MODULES = frozenset({"time", "datetime"})


def _is_forbidden(module_name: str) -> bool:
    """Whether ``module_name`` is a forbidden member of the tos.protective closure (§7.1)."""
...
def _closure_child(queue: mp.Queue) -> None:
    """Child target: import every tos.protective submodule; report forbidden closure members."""
    import sys

    import tos.protective  # noqa: F401
    import tos.protective._base  # noqa: F401
    import tos.protective.predicates  # noqa: F401
    import tos.protective.records  # noqa: F401
    import tos.protective.vocabulary  # noqa: F401
...
def test_protective_import_closure_has_no_forbidden_packages() -> None:
    """(§7.1 items 1-4) A fresh import of tos.protective pulls no forbidden package (sibling edge 0)."""
    result = _run_child(_closure_child)
    assert (
        result["leaked"] == []
    ), f"forbidden packages reached tos.protective closure: {result['leaked']}"


def test_protective_closure_includes_core_substrate() -> None:
...
def test_leak_canary_is_detected() -> None:
    """The spawn+scan pipeline catches planted config / rcl / authority / liveauth / brokercap / numpy leaks."""
    ...
    assert "tos.brokercap" in leaked, "planted tos.brokercap leak was NOT detected"
...
def test_is_forbidden_classifier_canaries() -> None:
    ...
    assert _is_forbidden("tos.brokercap") is True
    # Allowed: self, the core substrate, third-party.
    assert _is_forbidden("tos.protective") is False
    assert _is_forbidden("tos.protective.predicates") is False
...
def test_protective_source_has_no_dynamic_escape_ambient_env_or_clock() -> None:
    """(§7.1 item 6) No tos.protective source uses exec/eval/importlib/os.environ or a real clock."""
    sources = sorted(_PROTECTIVE_SRC.rglob("*.py"))
    assert sources, f"no tos.protective source files found under {_PROTECTIVE_SRC}"
    offenders: list[str] = []
    for path in sources:
