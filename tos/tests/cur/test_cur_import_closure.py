"""§7.1 import-closure verification for ``tos.cur`` — **allowlist** form (design #23 §0.3/§7.1).

Written as an **allowlist** (design #23 §7.1): after importing every ``tos.cur`` submodule in a fresh
...
It also asserts (design #23 §0.3):
  ...
  4. no ``tos.cur`` source references ``os.environ`` / ``os.getenv``, a dynamic escape
     (``exec`` / ``eval`` / ``__import__`` / ``importlib``), or a real clock (``time`` /
     ``datetime``) — cur is **clock-free** (§5.4: currentness is an ordering identity, not wall-clock);
  5. the sibling-absence assertion **explicitly includes ``tos.egress`` / ``tos.venue`` / ``tos.iap``
     / ``tos.capsule``** (the four maximum re-authoring temptations — the egress
     ``QuorumCommitCertificate`` / ``RestrictiveLatchState`` / ``monotonic_denial_no_revival`` and the
     venue / iap / capsule per-decision egress-currentness leaves are re-authored NOT AT ALL and
     imported NOT AT ALL — produced facts injected as scalars / verdicts / digests, §0.4b/c/d).

A planted-leak canary (including ``tos.egress`` / ``tos.venue`` / ``tos.iap`` / ``tos.capsule``, a
...
#: Real-clock modules that must never be imported by a clock-free cur source (§5.4).
_CLOCK_MODULES = frozenset({"time", "datetime"})

#: The re-authored (never imported) sibling predicate / type names — asserted ABSENT from the cur
#: namespace (local authorship, not an import; design #23 §7.1).
_REAUTHORED_NOT_IMPORTED = (
    "QuorumCommitCertificate",  # egress
    "RestrictiveLatchState",  # egress
    "monotonic_denial_no_revival",  # egress
    "egress_currentness_active",  # venue leaf
    "active_egress_currentness",  # iap leaf
    "egress_currentness_ok",  # capsule leaf
)
...
def _closure_child(queue: mp.Queue) -> None:
    """Child target: import every tos.cur submodule; report the tos + forbidden closure."""
    import sys

    import tos.cur  # noqa: F401
    import tos.cur._base  # noqa: F401
    import tos.cur.predicates  # noqa: F401
    import tos.cur.records  # noqa: F401
    import tos.cur.state  # noqa: F401
    import tos.cur.vocabulary  # noqa: F401
...
def test_cur_tos_closure_is_within_the_allowlist() -> None:
    """(§7.1 allowlist) The tos closure ⊆ {canonical, ordering, cur} — no sibling leaks (edge 0)."""
    result = _run_child(_closure_child)
    extra = sorted(set(result["tos_tops"]) - _ALLOWED_TOS_PACKAGES)
    assert (
        extra == []
    ), f"tos.cur closure escaped the §7.1 allowlist {sorted(_ALLOWED_TOS_PACKAGES)}: {extra}"


def test_cur_closure_includes_only_the_two_core_packages() -> None:
    """(§7.1) tos.canonical + tos.ordering ARE present; NO sibling edge."""
    result = _run_child(_closure_child)
    tops = set(result["tos_tops"])
    for expected in ("tos.canonical", "tos.ordering", "tos.cur"):
        assert expected in tops, f"{expected} missing from the tos.cur closure"
    for sibling in _FORBIDDEN_SIBLINGS:
        assert sibling not in tops, f"{sibling} leaked into the tos.cur closure"


def test_cur_closure_has_no_forbidden_operational_package() -> None:
    """(§0.3 items 1-3) No shared.* operational package, no services/cli, no numpy/pandas/yaml."""
    result = _run_child(_closure_child)
    assert (
        result["forbidden"] == []
    ), f"forbidden packages reached the tos.cur closure: {result['forbidden']}"


def test_leak_canary_is_detected() -> None:
    """(both-ways) Planted config / numpy / egress / venue / iap / capsule / future-sibling leaks caught."""
...
def test_allowlist_classifier_canaries() -> None:
    """The classifier admits the three allowed packages and rejects every sibling (incl. egress / venue)."""
    for allowed in (
        "tos.cur",
        "tos.cur.predicates",
        "tos.canonical",
...
def test_reauthored_predicates_are_absent_from_cur_namespace() -> None:
    """(§7.1) The re-authored sibling predicates / types are NOT importable from tos.cur."""
    for name in _REAUTHORED_NOT_IMPORTED:
        assert not hasattr(tos.cur, name), (
            f"{name} must be re-authored NOT AT ALL and imported NOT AT ALL — its presence in "
            "tos.cur would mean a forbidden sibling import (§7.1)"
        )
...
def test_cur_source_has_no_dynamic_escape_ambient_env_or_clock() -> None:
    """(§0.3 item 4) No tos.cur source uses exec/eval/importlib/os.environ or a real clock."""
...
def test_cur_source_imports_no_forbidden_sibling_statically() -> None:
    """(§0.3 static mirror) No cur source contains an ``import tos.<sibling>`` statement."""
    offenders: list[str] = []
    ...
    assert offenders == [], f"forbidden sibling import in cur source: {offenders}"
