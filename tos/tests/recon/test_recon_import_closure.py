"""§7.1 import-closure verification for ``tos.recon`` (design #9 §7.1).

Isomorphic to the canonical / rcl / orthostate / liveauth closure tests. It imports every
``tos.recon`` submodule in a **fresh, spawned interpreter** (``subprocess`` / ``os`` are
firewall-forbidden even in tests) and asserts:
  ...
  4. **All eight siblings are absent** — ``tos.orthostate`` / ``tos.rcl`` / ``tos.time`` /
     ``tos.evidence`` / ``tos.capsule`` / ``tos.authority`` / ``tos.liveauth`` / ``tos.dsl``
     (design #9 §3.4/§3.5 — recon keeps a **sibling edge of 0**, producing bools / scalars
     / injected coordinates instead of importing any of them).
  5. **``tos.canonical`` AND ``tos.ordering`` ARE present** — the two core substrate
     dependencies (§3.1/§3.2) are explicitly ALLOWED; the closure *bounds* them, it does
     not forbid them.
  6. No ``tos.recon`` source references ``os.environ`` / ``os.getenv`` or a dynamic escape
...
_RECON_SRC = Path(__file__).resolve().parent.parent.parent / "src" / "tos" / "recon"

_DYNAMIC_CALL_NAMES = frozenset({"exec", "eval", "__import__"})
_AMBIENT_ENV_ATTRS = frozenset({"environ", "getenv"})
#: Real-clock modules that must never be imported by a clock-free recon source (§0.2).
_CLOCK_MODULES = frozenset({"time", "datetime"})


def _is_forbidden(module_name: str) -> bool:
    """Whether ``module_name`` is a forbidden member of the tos.recon closure (§7.1)."""
...
def _closure_child(queue: mp.Queue) -> None:
    """Child target: import every tos.recon submodule; report forbidden closure members."""
    import sys

    import tos.recon  # noqa: F401
    import tos.recon._base  # noqa: F401
    import tos.recon.predicates  # noqa: F401
    import tos.recon.records  # noqa: F401
    import tos.recon.state  # noqa: F401
    import tos.recon.vocabulary  # noqa: F401
...
def test_recon_import_closure_has_no_forbidden_packages() -> None:
    """(§7.1 items 1-4) A fresh import of tos.recon pulls no forbidden package (sibling edge 0)."""
    result = _run_child(_closure_child)
    assert (
        result["leaked"] == []
    ), f"forbidden packages reached tos.recon closure: {result['leaked']}"


def test_recon_closure_includes_core_substrate() -> None:
...
def test_leak_canary_is_detected() -> None:
    """The spawn+scan pipeline catches planted config / rcl / orthostate / evidence / numpy leaks."""
...
def test_is_forbidden_classifier_canaries() -> None:
    ...
    assert _is_forbidden("tos.orthostate.predicates") is True
    ...
    assert _is_forbidden("tos.recon") is False
    assert _is_forbidden("tos.recon.predicates") is False
...
def test_recon_source_has_no_dynamic_escape_ambient_env_or_clock() -> None:
    """(§7.1 item 6) No tos.recon source uses exec/eval/importlib/os.environ or a real clock."""
    sources = sorted(_RECON_SRC.rglob("*.py"))
    assert sources, f"no tos.recon source files found under {_RECON_SRC}"
    offenders: list[str] = []
    for path in sources:
