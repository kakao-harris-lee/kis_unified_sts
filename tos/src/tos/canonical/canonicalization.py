"""Canonicalization + digest contract (design #2 §3 / design #4 §3).

Promoted to the ``tos.canonical`` core so both ``tos.capsule`` and
``tos.evidence`` share one digest-binding substrate without importing each other
(design #4 §0.4/§3.1 — canonicalization PROMOTE). ``tos.capsule.canonicalization``
remains as a thin re-export shim, so existing import paths are unchanged.

ADR-002-018 OQ1 and ADR-002-016 §27 Q2 leave the production canonical
serialization *and* digest algorithm as unapproved open questions. This module
therefore never hard-codes a production choice. Instead:
...
* A :class:`CanonicalizationScheme` bundles a canonical-serialization function
  ...
  ``version`` string (design #2 §3.1 — precedent:
...
* Artifacts record their ``canonicalization_version`` and resolve the matching
  scheme from a registry, so the digest invariant (design #2 §4.1 / design #4
  §4.2) is self-verifying on every construction.
* :class:`EVL1ProvisionalCanonicalizer` (``ev-l1-provisional-0``) is the
  **explicitly non-production** fixture used for EV-L1 property tests
  (design #2 §3.4 / design #4 §3.4). Approving a production scheme means
  registering a new version; the must-pass property suite regresses against it
  unchanged.

The default digest algorithm is ``hashlib.sha256`` — a stdlib primitive chosen
for reproducible tests, **not** a production selection (design #2 §3.1, §9.2
item 1 / design #4 §9.2 item 2). It is injectable via ``digest_factory``.
...
the digest preimage and any derived ``id = f(digest)`` (design #2 §2.6 template
...
the same digest -> same id / same digest-comparison, i.e. a cross-scheme
collision. Resolving this (folding the version into the preimage/id, or
namespacing by version) belongs to the production canonicalization + id policy
...
class EVL1ProvisionalCanonicalizer:
    ...
    nor the algorithm is a production selection (design #2 §9.2 items 1 and 4 /
    design #4 §9.2 item 2).
...
def get_scheme(version: str | None) -> CanonicalizationScheme:
    """Resolve a registered scheme by ``version`` (design §3.1, §4.1/§4.2).
...
# artifacts can self-verify their digest (design §4.1/§4.2).
