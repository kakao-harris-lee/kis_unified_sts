...
from tos.engine.vocabulary import CommitmentStep, EvidenceKind
...
def test_on_record_is_called_for_every_kind(store: SqliteEvidenceStore) -> None:
    ...
    sealed = GatewayEvidenceRecord(
        kind="SEND_SEALED",
        step=CommitmentStep.SEND_BOUNDARY_VERIFICATION,
        attempt_id="a1",
    )
    started = GatewayEvidenceRecord(
        kind="SEND_STARTED", step=CommitmentStep.SEND_STARTED_DURABLE, attempt_id="a1"
    )
    refused = GatewayEvidenceRecord(
        kind="SEND_REFUSED",
        step=CommitmentStep.SEND_BOUNDARY_VERIFICATION,
        attempt_id="a1",
    )
