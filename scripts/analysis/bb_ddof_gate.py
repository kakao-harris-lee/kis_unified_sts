...
def render(base: dict[str, Any], pop: dict[str, Any], meta: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.extend(_verdict_lines(bs, ps))
    lines.append("")
...
def _verdict_lines(bs: dict[str, Any], ps: dict[str, Any]) -> list[str]:
    ...
    # smaller than these is backtest noise, not a behavior change. Entry COUNT is
    # expected to move (narrower bands admit more touches) and is not itself a fail.
    ...
    material = ret_material or sharpe_material
    verdict = (
    ...
    return [
        ...
        f"|Δreturn| ≥ {_RET_MATERIAL_PP}pp or |ΔSharpe| ≥ {_SHARPE_MATERIAL} would "
        "gate. The 2.53% band narrowing shifts entry timing without changing the "
