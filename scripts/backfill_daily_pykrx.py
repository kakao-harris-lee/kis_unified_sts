...
Parquet daily market data를 백필한다.
...
async def _run(days: int, symbols: list[str]) -> int:
    logger.info(
        "Backfilling %d symbols (~%d trading days) via KIS daily API",
        len(symbols),
        days,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KIS 일봉 백필 to Parquet")
    parser.add_argument(
        "--days", type=int, default=500, help="백필 거래일 수 (default: 500)"
    )
    parser.add_argument(
        "--symbols",
        type=str,
    ...
    logger.info("Done! Total collected rows: %d", rows)
    return 0
