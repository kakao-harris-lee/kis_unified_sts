...
export function OrderConfirmModal({
  ...
  const [limitPriceOverride, setLimitPriceOverride] = useState<number | null>(
    signal.target_price || null
  );
  const [showOrderbook, setShowOrderbook] = useState(true);

  const action: OrderAction = signal.action === "BUY" ? "BUY" : "SELL";
  const limitPrice = limitPriceOverride ?? currentPrice;
  ...
  const handlePriceSelect = useCallback((selectedPrice: number) => {
    setLimitPriceOverride(selectedPrice);
    setOrderType("limit");
  }, []);
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    type="number"
                    value={limitPrice}
                    onChange={(e) => setLimitPriceOverride(Number(e.target.value))}
