...
export function useTradingMode(): UseTradingModeReturn {
  ...
  const serverCooldown = Math.max(0, status.cooldown_remaining ?? 0);
  const cooldownRemaining = Math.max(cooldownTimer, serverCooldown);
  ...
  const canSwitch = status.authenticated && (status.can_switch_mode !== false) && cooldownRemaining === 0;
