...
| 13:09:18 | P-5b(모의 선물 · 미체결 조회) | `P-5b-20260923T040918Z.json` | live / MEASURED / MOCK_VTS | [] / 1 | 새 선물 계좌 `46c39c54d3bb` → page 0 **`rt_cd=2`** 행 0. ⚠ 프로브가 `rt_cd≠0` 를 오류로 기록하지 않고 `msg_cd`/`msg1` 도 남기지 않아(하네스 갭 — P-BAL 은 둘 다 기록) 거부 문구 **미확정**. 주식 쪽과 같은 원인(앱키↔새 계좌 미연결)과 정합적이나 측정은 아니다 |
