...
- **유일 후보**: `.env`의 legacy `KIS_APP_KEY`/`KIS_APP_SECRET` 페어(선물 키와
  해시 상이, 정체 미확인). 실전 키 여부는 브로커 응답만이 판정한다 — N-16
  재실행의 수락/거부가 곧 배선 판정이다.
...
| N-16 재실행 | wave-2 아티팩트 자격증명 결함(EGW02004) + 실전 선물 앱키 호스트 부재 확정 | 야간 세션 창(18:00–05:00 KST) 내 §5.4 분리 셸에서 legacy `KIS_APP_KEY` 페어 배선 실행(운영자 실행 — 자동화 정책상 시크릿 배선은 수동). 후보마저 모의/무권한이면 운영자 실전 앱키 발급 필요 |
