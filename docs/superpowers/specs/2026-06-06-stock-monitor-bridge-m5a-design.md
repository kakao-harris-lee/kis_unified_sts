...
The decoupled stock pipeline (M4-P/R/O/X) produces fills (`order.fill.stock.shadow`), final signals (`signal.final.stock.shadow`), and a code-keyed positions working store (`trading:stock:positions`). The monolithic orchestrator currently produces the **dashboard-native** state the React Cockpit reads (`trading:stock:status/trades/signals` + id-keyed positions snapshot) and sends Telegram alerts. M5a bridges the gap: it aggregates the daemon streams into the dashboard-native keys and owns alerting — the prerequisite "eyes" for a safe M5 cutover.
...
- **데몬 working store**: `trading:stock:positions` (M4-O/X, field=code, value `{code, entry_price, quantity, opened_at_ms, state, signal_id, high_water, low_water}`). orchestrator도 같은 키(field=id, `entry_time`) → `opened_at_ms` 가드로 구분.
...
| `trading:stock:positions` (read, opened_at_ms 가드) | — | 재시작 open-entry 복구 |
...
- **live (M5d)**: suffix 비움 → live 키. orchestrator stock off라 단독.
...
`STOCK_POSITIONS_KEY`(기본 `trading:stock:positions`) + `opened_at_ms` 가드(orchestrator id-keyed 레코드 skip). M5d에서 데몬 store를 `stock:daemon:positions`로 격리하면 그 키 읽음.
...
### 8.1 env
`STOCK_MONITOR_DAEMON`(off) · `STOCK_FILL_STREAM`/`STOCK_FINAL_STREAM`(mode별) · `STOCK_POSITIONS_KEY`(`trading:stock:positions`) · `TRADING_STATE_KEY_SUFFIX`(shadow=auto `shadow`) · `STOCK_TICK_STREAM`(`market:ticks`) · `STOCK_MONITOR_STATUS_INTERVAL`(`5`) · `STOCK_MONITOR_SIGNAL_META_MAX`(`1000`).
