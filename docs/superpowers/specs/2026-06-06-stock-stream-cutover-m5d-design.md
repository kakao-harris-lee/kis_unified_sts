...
- **스트림/키**(stock): `signal.candidate.stock[.shadow]`, `signal.final.stock[.shadow]`, `order.fill.stock[.shadow]`, `trading:stock:positions[:shadow]`(hash), `trading:stock:market_context[:shadow]`, `risk:state:stock`+`:meta`. consumer groups: `stock_risk_filter`, `stock_order_router`, `stock_exit`, `stock_monitor`.
...
stock은 EOD 청산 안 함(다일 보유) → 컷오버 시 orchestrator가 오픈 paper 포지션 보유 가능. M4-X는 `trading:stock:positions`를 읽지만 orchestrator 레코드는 `opened_at_ms` 없음 → M4-X가 "foreign"으로 skip → 고아. **운영자 결정**: 현 모의투자 데이터는 무의미하므로 **마이그레이션·보존 없이 전부 청산하고 decoupled를 fresh 시작**. 잔여 모의투자 계좌 포지션은 **후속 정리**(M5d 범위 밖).

→ 컷오버 시: (선택)`flatten_all.py`로 orchestrator 포지션 청산 → orchestrator 정지 → **`del trading:stock:positions`**(M4-X foreign skip 방지 + 대시보드 클린) → decoupled fresh 시작.
...
| 체크 | 내용 | critical |
|------|------|:---:|
| 스트림 + consumer group | `signal.candidate.stock{sfx}`/`signal.final.stock{sfx}`/`order.fill.stock{sfx}` 존재 + 기대 group(`stock_risk_filter`/`stock_order_router`/`stock_exit`/`stock_monitor`) XINFO GROUPS 연결 | ✅ |
...
| 포지션 해시 | `trading:stock:positions{sfx}` 개수(informational) | — |
...
② redis-cli -n 1 del trading:stock:positions   (decoupled live 포지션 폐기 — paper)
...
### 5.3 런북 `docs/runbooks/stock-stream-cutover.md`
...
① 청산+클리어: (선택)scripts/trading/flatten_all.py --asset stock
   ...
   → redis-cli -n 1 del trading:stock:positions
...
- [ ] `docs/runbooks/stock-stream-cutover.md` — Gate 0–2 + 컷오버 시퀀스 + 롤백(트리거/절차) + 포지션 청산 처리 명시.
- [ ] 포지션: 컷오버 시 `del trading:stock:positions`(fresh 시작), 잔여 계좌=후속 follow-up 명시.
