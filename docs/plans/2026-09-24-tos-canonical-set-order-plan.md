...
- **비교 불가 원소는 거부**: `sorted()` 가 `TypeError` 를 내면 훅은 대체 키로 조용히 넘어가지 않고 그대로 올린다. 오늘 커널에는
  그런 집합이 없다(§1.1).
- 구현 세부(파이썬 값 → 정렬 → JSON 네이티브화의 정확한 순서, 기본 직렬화기의 호출 방식)는 구현 레인이 pydantic 문서와 실측으로
  정한다 — 요구사항은 「JSON 모드 출력에서 집합이 `sorted()` 순이고, 그 밖의 출력은 바이트 동일」이다(§3 핀).
...
들이는 import 뿐**이어야 하고, import 는 **기존 import 줄에 이름을 더하는 방식**으로 줄 수를 늘리지 않는다(`black`·`ruff` 통과가
조건 — 불가능하면 구현 레인이 멈추고 대안을 보고한다). `covered_content()`(`:187`)는 **바꾸지 않는다** — 훅이 그 아래에서 일한다.
...
| **파이썬 모드 불변**: `model_dump()`(파이썬 모드)가 집합을 집합으로 돌려줌 | 훅이 모드를 가리지 않으면 → RED |
...
| **줄 고정 무회귀**: `test_orthostate_l2_fault.py` 무변경 통과 · `_base.py:187/205/209/211` 내용 불변 | `_base.py` 줄 이동 → ST-05·06·07·09·12 RED |
