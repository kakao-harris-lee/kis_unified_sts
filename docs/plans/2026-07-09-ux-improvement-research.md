...
8. **가격차트 + 시그널/체결 마커** (A3/B). **visx로 직접 구현 결정(§6)** — 진단용 제한
   윈도우. Signals/Trades에서 "왜 이 시점에 진입" 직관.
...
- **차트 라이브러리**: recharts는 캔들스틱/히트맵 부재 → Tier 2·3용 추가 필요. 최종 결정 §6.

## 6. 차트 라이브러리 결정 (확정: visx 단일 추가)

Tier 2·3의 히트맵·underwater/밴드·가격차트는 현 recharts로 불가. 후보를
**실제 스택(Next 16.1.6 / React 19.2.3 / TS5 / Tailwind v4)**에 대해 실측 비교.

**결정 축(운영자 재프레이밍)**: 이 시스템은 **config-driven 알고 트레이딩**이라
사람이 차트를 보고 매매하지 않는다. 캔들+마커의 용도는 **사후 진단/설명**(트레이드 전후
제한 윈도우 조회)이지 실시간 스캘핑이 아니다. 따라서 lightweight-charts의 핵심 강점
(실시간 스트리밍 캔들 60fps·TradingView 감성)은 **이 시스템에서 거의 소비되지 않는다** —
전용 금융차트 엔진을 진단 1용도로 이고 가는 것은 과설계. 실제 차트 수요는 **압도적으로
통계·분석형**(equity/underwater/rolling Sharpe/Monte Carlo fan/민감도·상관 히트맵/regime
밴드/walk-forward matrix)이며 캔들은 그중 하나. → **범용 통계 차트 1종이 캔들까지 충분히
커버하는 편이 일관성·유지보수에서 우월.**
...
(KLineChart·lightweight-charts는 캔들 전용/실시간 특화 — 아래 결정 축에 따라 제외.
Plotly 1.33MB 과중, Nivo 캔들 부재, uPlot React 래퍼 부재로 각각 탈락.)

### 6.2 결정: **visx 단일 추가, recharts 유지**

**근거:**
- **차트 수요가 통계·분석형이 지배적** — 히트맵(민감도·상관)·underwater·밴드(Monte Carlo
  fan·regime)·walk-forward matrix가 중심이고 캔들은 진단용 1종. visx는 이 전부를 SVG
  프리미티브로 커버: `@visx/heatmap`(그리드), `@visx/shape` Area `y0/y1`(밴드/underwater),
  rect+wick+scatter 조합으로 **캔들+체결마커 직접 구현**(진단용 제한 윈도우엔 충분, ~1일,
  재사용 컴포넌트화).
- **기존 스택과 동일 idiom** — 이미 쓰는 recharts(SVG)·React·Tailwind v4 토큰·다크모드
  CSS와 같은 렌더링 모델이라 **테마·다크모드가 이음매 없이 통합**. Canvas island(ECharts)나
  별도 테마 옵션 체계가 생기지 않음 → 밀도 높은 운영 대시보드의 일관성 우선.
- **React 19/Next 16 가장 깔끔** — visx v4 peer `^18‖^19` 명시, MIT, 모듈당 ~10-20KB(최경량).
- **lightweight-charts 제외 이유** — 강점이 실시간 스트리밍 캔들·트레이딩 터미널 감성인데,
  이 시스템은 차트-매매가 아니라 알고 자동실행 + 사후 진단이라 그 이점이 거의 소비 안 됨.
  전용 금융차트 엔진을 진단 1용도로 유지하는 비용(별도 canvas 엔진·61KB·이중 idiom) >
  visx로 캔들 직접 구현 비용(~1일).
- **ECharts 제외 이유** — 캔들을 네이티브로 주지만 `echarts-for-react` 래퍼에 React 19
  미해결 이슈 3건(#617/#628/#619) + canvas island + 테마 분리 + 360KB. 캔들 절약분이
  구조적 비용을 상쇄 못 함.
- **recharts는 무변경** — 3.8.1 peer가 이미 React 19 지원, 2종 공존 충돌 없음. 기존 4개
  단순차트 마이그레이션 ROI 없음. (훗날 원하면 visx가 recharts까지 흡수 가능 — 현재 액션 아님.)

### 6.3 통합 노트

- 설치: `npm i @visx/heatmap @visx/shape @visx/scale @visx/group @visx/axis @visx/tooltip`
  (쓰는 서브모듈만).
- **visx**: 순수 React/SVG, SSR-safe → `next/dynamic {ssr:false}` 불필요. 툴팁/hover
  상호작용 필요한 컴포넌트만 `"use client"`.
- **캔들+마커 컴포넌트**: rect(body)+line(wick)+`@visx/scale` 밴드/선형 스케일 + scatter로
  체결마커. 진단 윈도우(트레이드 전후 제한 봉수)라 대용량 실시간 최적화 불필요 → 재사용
  `CandleWithMarkers` 1개로 Signals/Trades 공유. PoC 1건으로 실현성 확정 권장.
- **코드 스플리팅**: visx import를 해당 차트 컴포넌트/라우트에만 국한(공유 layout import
  금지) → shared 번들 팽창 방지, recharts-only 페이지 무영향.
- **미래 통합 옵션**: 훗날 recharts까지 visx로 흡수 가능(동일 SVG/React idiom) — 현재 액션 아님.

## 7. 다음 단계 (제안, 미실행)

- 이 리서치를 근거로 **워크벤치 로드맵 v2 섹션** 또는 별도 실행계획 작성 여부 결정.
- Tier 1은 신규 로드맵과 독립 → 즉시 착수 가능. Tier 2는 P1/P3에 동반 배치.
- 차트 라이브러리 스파이크: **visx로 (a) underwater/히트맵 PoC + (b) `CandleWithMarkers`
  진단 캔들 PoC** 각 1건 — 후자로 "캔들 직접 구현 ~1일" 가정을 실측 확정.
