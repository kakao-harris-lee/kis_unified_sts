...
| 같은 grep 이 산문의 `class of bug` 도 잡아 허구 식별자 `of` 가 클래스 집합에 섞임 | **review-775 MEDIUM** | 이번 773 건엔 영향 0(실제 타입에 `of` 토큰 없음). 패턴 끝에 `\s*[:(]` 를 붙여 닫음 — 클래스 집합 206개, `of` 없음 |
...
| MEDIUM 1 | 같은 grep 이 산문 `class of bug` 를 잡아 `of` 가 클래스 집합에 섞임 | 패턴 끝에 `\s*[:(]` — 집합 206개, `of` 없음 확인 |
...
assert "of" not in TEST_CLASSES and len(TEST_CLASSES) == 206
