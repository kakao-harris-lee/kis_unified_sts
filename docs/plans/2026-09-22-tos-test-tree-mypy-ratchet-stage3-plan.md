...
- 작성: 2026-09-22 · 세션 모델 단독 저작(운영자 지시 2026-09-04)
...
그래서 이 계획은 분류기를 **먼저** 만들고 검증한 뒤 수치를 읽었다.

### 0.1 분류기 검증 — 결함 1건을 잡았다

| 검증 | 결과 |
|---|---|
| 메시지 파싱 | 초판 정규식이 **27건을 못 읽었다** — `Argument N has incompatible type ...` 처럼 **피호출자가 없는 형상**(positional 호출에 mypy 가 callee 를 안 붙임). 정규식을 고쳐 **773/773 파싱, 미파싱 0** |
| 더블 판정 | 상위 계획은 **이름 패턴**(`_Stub*`/`_Fake*`/`_Mutable*`)으로 26건이라 했다. 이 계획은 **「실제 타입에 등장하는 이름이 테스트 트리(`tos/tests`·`tos/runtime/tests`)에 `class` 로 정의돼 있는가」** 로 판정한다 — 이름이 아니라 정의 위치가 기준이다. 그러자 **87건**이다. `_Request`·`_FixedKeyProvider`·`_TrustedTimeService`·`_NullEvidence` 같은 더블은 이름 패턴이 놓쳤고, 상위 계획의 「더블 3%」는 **분류기 결함이지 데이터 성질이 아니었다** |
| 라벨 겹침 | 우선순위로 갈라 하나만 세지 않고 **겹침을 그대로 잰 뒤** 배타 분할을 따로 정했다(§0.3). 상위 §7.5 가 「겹치는 휴리스틱을 우선순위로 갈라 만든 분할이라 자연스러운 분류가 아니다」고 경고한 그 부류 |

파서와 규칙은 부록 A 에 그대로 있다. 누구든 같은 명령으로 같은 수치를 얻어야 하고, 얻지
못하면 이 계획이 아니라 분류기가 틀린 것이다.

### 0.2 라벨 실측 (겹침 허용, 773건)
...
| **D** 더블 | 실제 타입의 식별자 중 하나가 테스트 트리에 정의된 클래스 | 87 |
...
처방 쪽**에 둔다(A 는 빌더 관용구를 바꾸면 C 도 사라지므로 A 가 우선).

| 부류 | 배타 건수 | 비율 | 처방(§1) |
|---|---:|---:|---|
| **A** kwargs 언패킹 (C 겹침 193 포함) | **443** | 57% | 빌더 관용구 교체 — §1.1 |
| **B** 실제형 옵셔널 미좁힘 | 107 | 14% | 2단계 처방(좁히기·ISSUED 접근자) — §1.2 |
| **D** 더블 계약 미충족 (C 겹침 12 포함) | 87 | 11% | 더블을 계약에 맞춘다 — §1.3 |
| **E** `object` 산출자 (C 겹침 22 포함) | 66 | 9% | 산출자 애노테이션 교정 — §1.4 |
| **C′** 기대형만 옵셔널, 실제형이 다른 구체 타입 | 26 | 3% | 개별 판단 — §1.5 |
| **Z** 그 밖 | 44 | 6% | dict 불변성 21 + 개별 — §1.5 |
| 합 | **773** | 100% | |

상위 §7.5 의 분할(446 · 110 · 51 · 140 · 26)과 A·B 는 근사 일치하고, **D 가 26 → 87 로
3배**, 「단일 형상 없음」 140 이 **44 로 준다.** 잔여가 준 것은 형상이 생겨서가 아니라
**더블 판정 기준을 바꿨기 때문**이다 — 이번에도 분류기가 수치를 바꿨다.
...
`endpoint_binding_from_profile_ok` 28 + 3). 상위 12 피호출자가 **283건(64%)** 이다.
...
dict[str, object]` 도 같은 모양이고, `CapabilityTuple(**_base_kwargs(**{axis: None}))` 처럼
**불법 값을 일부러 넣어 seal 을 발화시키는** 용법이 함께 있다. 이 용법이 처방을 제약한다 —
...
빌더의 반환형은 그대로 모델이고, 호출부는 한 줄도 안 바뀐다.
...
그 TypedDict 를 반환하게 한다 — `**TypedDict` 는 mypy 가 **키·타입을 정밀 검사**한다. 즉
유예를 걷는 것을 넘어 **빌더와 시그니처의 드리프트를 mypy 가 잡게 된다**(지금은 아무도
안 잡는다).

TypedDict 로 표현 못 하는 케이스가 둘 있고, 둘 다 **스플랫을 버리고 명시 호출**로 푼다:

1. **불법 값 주입**(`{axis: None}` 처럼 시그니처 밖의 타입) — 그 케이스만 키워드를 직접
   써서 호출하고, 그 자리에는 그 값이 불법임을 **mypy 도 아는** 상태가 된다. 불법을
   일부러 넣는 줄은 `# type: ignore[arg-type]` 가 아니라 **호출 자체를 `pytest.raises` 안으로
   넣고 값은 `cast`** 로 표시한다 — 「이 줄은 타입을 어긴다」가 코드에 남아야 한다.
...
| `str | None → str` | 17 | `load_instance_document` 7 · `snapshot_consumer_binding_ok` 11 |
| `DimensionKey | None` 8 · `Path | None` 7 · 그 밖 11 | 26 | 개별 |
...
클래스인 것**이 갈리고, 후자가 더 많다:

| 기대 타입 | 건 | 종류 |
|---|---:|---|
| `TrustworthyTimeService` | **21** | 구체 클래스 `tos/runtime/src/tos_runtime/time/service.py:95` — 더블이 **5종**(`FakeTimeService` 9 · `_TrustedTimeService` 6 · `_NeverStartedTimeService` 4 · `_NotStartedTimeService` 1 · `_BrokenTimeService` 1) |
...
| `ReservationProjectionReader` | 6 | **Protocol** (`rcl/projection.py:48`) |
| `OrderConstructionStage` | 5 | 구체 (`tos/src/tos/egressgw/construction.py:948`) |
| `RecordingAggregateRiskService` 3 · `CredentialCustody`(Protocol) 2 · `SqliteEvidenceStore` 2 · `VenueConstraintService` 2 · `SqliteCommitLog` 1 · `SqliteEventInbox` 1 | 11 | 혼합 |

규칙:

1. **기대가 Protocol 이면**(≈ 23건) 더블을 계약에 맞춘다 — 빠진 멤버를 더하고 시그니처를
...
3. **기대가 구체 서비스 클래스면**(`TrustworthyTimeService` 21 · `SessionFactsOwner` 6 ·
   ...
   ≈ 51건) 선택지가 둘이고 **어느 쪽인지는 피호출자가 정한다**:
...
| 피호출자 | 건 | 산출자(레인이 확정) |
|---|---:|---|
| `netting_requires_positive_proof` | 20 | `tos/tests/posttrade/test_posttrade_no_favorable_default.py` — `ObligationLeg | None` 자리에 `object` 를 넘기는 픽스처/파라미터화 |
| `EgressResultPayload` | 14 | `tos/tests/engine/test_engine_finality_release.py` |
| `_outstanding` · `ProvisionalReservationLedger.*` | 18 | 같은 파일 — `ledger`/`key` 인자가 `object` 로 만들어지는 픽스처 |
| 그 밖 | 14 | 개별 |

처방: **산출자의 애노테이션을 실제 타입으로 교정**한다. `cast` 는 산출자를 못 고칠 때만
(외부 라이브러리 반환 등) 쓰고 그 사유를 그 줄에 적는다. `test_engine_finality_release.py`
**한 파일이 33건**이라 산출자는 몇 개 안 될 것이다 — 파일별로 고치기 전에 **`got` 타입으로
묶어** 근본을 찾는다(상위 §7.7 ①).

### 1.5 C′ 26 · Z 44 — 개별

- **dict 불변성 21건**(Z): `dict[CommitmentStep, Stage] → dict[CommitmentStep, object]` 14 ·
  `list[dict[str, str]] → list[dict[str, object]]` 4 · `dict[ConformanceAxis, str] → ... | None`
  3. `dict`/`list` 는 값 타입에 **불변**이라 `Stage` 를 `object` 자리에 못 넣는다. 피호출자
  `_driver`(`tos/runtime/tests/engine/test_replay_stage.py`) 는 **테스트 헬퍼**라 파라미터를
  `Mapping[CommitmentStep, Stage]` 로 고친다 — 읽기만 하면 `Mapping` 이 맞는 타입이다.
  **커널/런타임 소스 파라미터가 `dict[..., object]` 인 경우는 소스 변경**이므로 §1.3 ③과 같은
  절차(별도 커밋·사유·리뷰 지점).
...
- **C′ 26**: 기대 `X | None` 에 실제가 다른 구체 타입(`NonBrokerTransportNature →
  TransportNatureLike | None` 3 · `int` 9 …). 개별로 본다 — 진짜 타입 불일치일 가능성이
  있다(커널 결함 후보). 레인이 첫 표본에서 **진짜 결함이 나오면 멈추고 보고**한다.
- 나머지 Z ≈ 19: 개별.

## 2. 분할 — PR 5개

**한 파일은 한 PR 에만 산다.** 부류로 나누되 파일이 두 부류에 걸치면 큰 쪽 PR 로 몰아 넣는다
(공유 워크트리 인덱스 경합·병합 충돌을 피하는 규칙 — 메모리 `shared-worktree-git-index-race`).

| PR | 부류 | 건(근사) | 왜 이 순서인가 |
|---|---|---:|---|
| **3-a** | **E + B** | 173 | 2단계와 **같은 처방**이라 위험이 가장 낮다. 먼저 끝내 레인이 손을 푼다. `None → StageRequest` 23 의 픽스처가 여기서 생긴다 |
| **3-b** | **A — `test_brokercap_routing.py` 한 파일** | 96 | 한 파일에 A-pyd(65)와 A-fn(28)이 **둘 다** 있어 두 처방을 한 곳에서 검증한다. 여기서 확정한 TypedDict·`model_validate` 모양이 3-c/3-d 의 본이 된다 |
| **3-c** | **A-fn + A-issue** (brokercap 제외) | 131 | TypedDict 도입. `degraded_lease_invalidated` 56 이 3파일(`test_authority_lease.py` 32 + `test_seam_authority.py` ×2 24)에 걸쳐 있어 TypedDict **하나를 어디 둘지**(각 파일 vs 공유 `_authority_strategies.py`)를 첫 표본에서 정한다 |
...
건수는 근사다 — 부류 경계의 파일 이동으로 ±20 은 움직인다. **각 PR 은 착수 시점에 자기
몫을 다시 재고 본문에 적는다.** 합은 773 에서 **줄기만** 해야 한다(§3 ①).
...
4. **뮤테이션** — 처방마다 하나, **RED 를 실제로 본 뒤** 원복(`cp -p` 백업 · `cmp` 확인 ·
   ...
   - TypedDict: 필수 키 하나 제거 → `arg-type`(또는 `typeddict-item`) RED
...
| `model_copy(update=overrides)` 빌더 | 검증을 건너뛴다 — 불법 override 를 잡는 테스트가 **아무것도 안 잡게** 된다 |
| 한 PR | 773건 단일 diff 는 리뷰가 형식이 된다(상위 §3.3) |
| 더블 87건을 「이름이 `_Fake`/`_Stub` 인 것」으로 한정 | §0.1 — 그 기준이 61건을 놓쳤다 |
...
| ③ | §1.1 ① — 불법 입력 표시를 `cast` + `pytest.raises` 로 할 것인가, `# type: ignore[arg-type]` 를 그 줄에만 허용할 것인가 | **`cast`.** ignore 는 `unused-ignore` 단계에서 다시 세어야 하고, 부류가 섞인다 |
...
분류 규칙 (라벨은 겹침 허용, §0.3 의 배타 분할은 A > D > B > E > C′ > Z 우선순위):
...
_defs = subprocess.run(
    ["grep", "-rhoE", r"^class [A-Za-z_][A-Za-z0-9_]*", "tos/tests", "tos/runtime/tests"],
    capture_output=True, text=True,
).stdout.split("\n")
TEST_CLASSES = {x.split()[1] for x in _defs if x.startswith("class ")}
...
실측 결과(main `29f39fe6`): 파싱 773/773 · A 443 · B 107(B0 29) · C 253 · D 87 · E 66 ·
Z 44 · 배타 조합 = A 250 · A+C 193 · B 78 · B+B0 29 · D 75 · C+D 12 · E 44 · C+E 22 · C 26 ·
Z 44.
