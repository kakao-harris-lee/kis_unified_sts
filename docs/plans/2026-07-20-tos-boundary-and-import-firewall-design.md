...
- `shared/config`는 **재사용 대상에서 제외**한다(2026-07-20, §3.2/§6.1 개정):
  `__init__`이 `shared.config.secrets`(→ `os.environ`)를 무조건 전이 import하므로
  ConfigLoader만 쓰려 해도 ambient 자격증명 코드가 tos closure에 유입된다. 정책
  로딩은 설계 #2 §0.3대로 pyyaml 단일 수단.
...
| 커먼즈 | `shared.models`, `shared.indicators`, `shared.resilience`, `shared.utils`, `shared.exceptions` | 패키지 단위 허용은 "그 패키지의 closure가 §2.3을 포함하지 않는다"는 검증(§3.3-②) 하에서만 유효. **`shared.config`는 허용목록에서 제외**(2026-07-20, §6.1): `__init__`이 `shared.config.secrets`(→ `os.environ`)를 무조건 전이 import하므로 패키지 전체가 미허용이다 — secrets 서브모듈만 카브아웃해도 AST 게이트(직접 import만 검사)를 통과하는 전이 유입을 막지 못하기 때문. 정책 로딩은 설계 #2 §0.3대로 pyyaml |
...
프리미티브가 로드될 수 있다(예: `shared.resilience` → `http.client` — 리뷰 실측;
`shared.config` → `socket`/`subprocess`도 실측됐으나 그 때문에 §6.1대로 허용목록에서
제외됐다). 이는 자격증명·라우트·주문
...
- 개정 로그:
  ...
  - 2026-07-20: `shared.config`를 §3.2 허용목록에서 제거 — `__init__`이
    `shared.config.secrets`(`os.environ`)를 무조건 전이 import하여 AST 게이트(직접
    import만 검사)·import-linter(secrets 미열거) 둘 다 미검출하던 구멍을 닫음.
    병행: `.importlinter` forbidden에 `shared.config.secrets` 추가(layer② 전이
