...
> **[에라타 v1.3 — 절차 추가, 2026-08-06, 구현 후·오케스트레이터 소관]** REGREP 게이트에 다음 절차를
> 추가한다: **같은 커밋에서 src를 편집한 경우, 그 src를 가리키는 자기 인용(이 계약·신규 테스트의
> file:line 앵커)부터 재측정하라.** 근거: #38 구현 리뷰 MINOR-1 — 구현자의 "REGREP 드리프트 0" 주장이
> HEAD-vs-계약 스코프로만 측정되어, 자기 커밋의 docstring 삽입이 밀어낸 신규 테스트 앵커 3건(2·6·9행
> 변위)을 놓쳤다. 다른 앵커 1건은 삽입 위치가 아래였던 우연으로만 생존("luck, not method" — 구현자
> 자인). 커밋 전 전건 정정 완료. 이 절차는 #36 앵커 재조준 의무의 일반화다.
