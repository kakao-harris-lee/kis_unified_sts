...
- ClickHouse는 기본 runtime storage가 아니라 optional mirror, research,
  historical backfill, legacy analytics 경로로만 유지한다.

## 2026-06-04 ClickHouse 의존성 감사

결론: 기본 trading/dashboard runtime의 ClickHouse hard dependency는 남아 있지
않다. 다만 Python package dependency와 명시적 optional/research/backfill 경로는
남아 있으므로, "ClickHouse dependency fully removed"가 아니라 "runtime hard
dependency removed" 상태로 기록한다.
...
- `clickhouse`와 `mlflow` 서비스는 `docker compose --profile research
  config --services`에서만 나타난다.
- 기본 env는 `RUNTIME_STORAGE_CLICKHOUSE_MIRROR_ENABLED=false`,
  `MARKET_DATA_SOURCE=parquet`, `MARKET_DATA_CLICKHOUSE_ENABLED=false`다.
- Runtime storage 기본값은 Redis DB 1 + SQLite `RuntimeLedger`다. Hot path
  서비스의 ClickHouse client 생성은 mirror enablement 뒤에 있어야 하며,
  ClickHouse 장애가 paper/live order, fill, position ledger durability를 막으면
  안 된다.
- `pyproject.toml`의 default dependencies에는 `clickhouse-driver`와
  `aiochclient`가 아직 남아 있고, `uv.lock`에는 `clickhouse-connect`도 남아
  있다. dependency split 또는 optional extra 이동은 별도 cleanup 항목이다.
- 남아 있는 ClickHouse code path는 명시적 CLI/ops/research 용도다:
  `sts data export-clickhouse`, `sts backfill`, `sts stock-backfill`,
  historical collector, overnight accumulation scanner, dashboard legacy
  analytics/fallback, analysis/forecasting/migration scripts, live infra/perf
  tests.
...
상태: runtime removal checks completed; ClickHouse package cleanup follow-up open

- `git ls-files` 기준으로 active RL/TFT source, config, training script,
  `rl_mppo` strategy/exit/helper 파일이 제거되어 있다.
- `docker compose config --services` 기준 default stack에는 ClickHouse 서비스가
  없다. ClickHouse 서비스는 `research` profile에만 남아 있다.
...
- ClickHouse package dependencies(`clickhouse-driver`, `aiochclient`,
  `clickhouse-connect`)는 아직 남아 있으므로, default install에서 완전히 빼려면
  별도 PR에서 optional extra로 이동해야 한다.
...
- 유지: dashboard/API와 storage schema에는 `rl_trades` 같은 legacy ClickHouse
  table naming이 남아 있다. 이는 active `rl_mppo` strategy naming이 아니라
  historical compatibility로 취급한다.
- 후속 cleanup: default Python dependency에서 ClickHouse driver packages를
  제거하려면 ClickHouse CLI/backfill/research 경로를 optional extra로 분리한다.
...
| ClickHouse package dependency가 default install에 남음 | runtime hard dependency 제거와 package cleanup을 분리한다. 별도 PR에서 ClickHouse CLI/backfill/research 경로를 optional extra로 이동 |
