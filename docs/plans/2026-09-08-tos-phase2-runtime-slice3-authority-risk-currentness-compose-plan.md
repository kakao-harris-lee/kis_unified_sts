...
- 재심(`d1f24837` 대상) **needs-attention** — 1차 8건 전부 동작으로 종결(revocation·CURRENT 파생·owner verdict·`-1`·step 9·격리 창·append-after-check·nonce 키) · 종료 조건 5 중 4 실증, 1(권위-투영 불일치 경보)은 **테스트가 record_halt 를 대신 저작**(compose 에 verify_replay 호출 0) → HIGH · MEDIUM 5: `max_decision_age_ms` 로드만 하고 미강제(ADR-002-023 §12 항목 2 «unexpired») · `_rcl_tip_generation_provider` bare except→0(단조성 붕괴 · 뮤테이션 M4 green) · 거부 egress attestation 무테스트(M1 green) · step 6/7 admission 사실 7종을 테스트 리터럴로 주입 · Q4: 0 폴백은 admit 을 뒤집지 못함(digest 좌표 · 비교자 아님) · Q5 상수 스윕 0 · Q6 `decision_current` 는 «current» 연접 충실(«unexpired» 는 위 MEDIUM).
- 처분(8커밋 `304c23cb..65ef7cf6` · 커널 diff 0 · runtime tests **413 passed rc=0** 독립 실측 @`13c9fd53` · `65ef7cf6` 은 테스트 +1): P `7538c813` non-null `max_decision_age_ms` 로더 **거부**(커널 iap 는 clock-free · `tos.time` 술어는 issuer-signed age/continuity 앵커 요구 · 승인 파일에 발행시각 없음 · 로드 시각을 발행시각으로 쓰면 stale 결정이 age 0 — **커널 라운드 갭: continuity 앵커 발행시각을 내는 승인 발행자**) · S F1 `d49e46b5` 부팅 시 `verify_replay`+`record_halt` 자체 수행(테스트는 단언만) · F2 `74fdf6c0` bare except 제거·permit 경로만 typed catch→None(AtomicCommitStage 에 try 없음) · F3 `b35b0ef7` 거부 attestation e2e(M1 red 확인 · `worst_credible_capacity` 는 커널 gateway 에서 거부 사유 문자열에만 접힘 — 독립 게이트 아님) · F4 `a057e643` 6종 → `_risk_attestations.py` null 거부 설정 · `generation_current` 는 `tos.afg.generation_fenced`(caller decision_generation vs RCL tip) 실파생 · `OPERATOR_ATTESTED_INPUTS` 증적 11좌표(감사자가 attested/derived 구분) · **F4 후속 `9a31ddcd`**: wrap_* 무조건 override 가 caller False 를 attestation True 로 뒤집는 fail-open(오케스트레이터 적발) → `_restrictive_merge`(caller None/True→attestation · non-True 상향 불가) · addendum `13c9fd53` stale-log permit 경로 UNKNOWN·RCL 행 0 테스트 + `_time_permits_new_risk` catch → `TimeServiceNotStarted` 만(compose 내 bare except 0) · `65ef7cf6` step 7 `wrap_action_flow_inputs_provider` stale-log 경로도 Stage 수준 UNKNOWN 증명(typed catch 2곳 모두 red-선행).
- **재재심 approve** — 6건 전부 동작으로 종결 · 뮤테이션: M1(latch CLEAR 붕괴) green→**red 1** · M4(generation 상수) green→**red 11**(provider 가 egress 거부 경로의 하중을 받게 되어 6방향 적발) · 신규 M6(`max_decision_age_ms` 거부 제거) red 1 · M7(부팅 `verify_replay` 제거) red 1 · M8(`_restrictive_merge` → 무조건 override) red 2 · M9(`generation_current` 리터럴 True) red 1 · 판별 공백 0. HIGH 는 소스에서 종결(`_boot_integrity.py:36-61` · `_wiring.py:914` RCL/authority 빌드와 risk/currentness 빌드 사이 호출). 렌즈 2: «caller True 는 attestation 에 양보» 극성 정당 · truthy non-True(StrEnum) 통과 경로는 커널 `is not True`(`are/predicates.py:632`) 가 거부 — 병합과 커널 게이트 극성 일치 · attestation 이 producer 음성을 뒤집는 경로 0. 렌즈 3: `OPERATOR_ATTESTED_INPUTS`(`_boot_integrity.py:64-112`) 는 «어느 좌표가 어느 파일 내용에서 attested 됐나» 에 충분 · per-send 귀속은 runtime identity 로 부팅 레코드와 join(정적 집합+digest 결속이라 건전) · 17 currentness 차원(in-band `owner_identity`) 과 egress 5+risk 6(out-of-band 부팅 레코드) 의 강도 비대칭은 문서화됨. 렌즈 4: `worst_credible_capacity` 는 커널 gateway 에 1회 출현 · 비-ADMIT 분지 · 사유 문자열만(S 주장 확인). 렌즈 5: compose 내 bare except 0 · 상수 provider 0. `65ef7cf6` 포함 8커밋 범위 재확인 — M10(step 7 typed catch 제거) rc=1 · 2 failed(e2e 핀 + 유닛 핀 동시 발화) · runtime tests **414 passed** · 커널 `git diff --name-only 304c23cb 65ef7cf6 -- tos/src/` 공집합.
- **비차단 관찰 2건(후속)**: OBS-1 `numerically_safe`/`valuation_ok` 를 `| None` 으로 넓혀 «무의견» 관용구를 attested 6필드에 균일화(다음 런타임 슬라이스에서 처리) · OBS-2 worst-credible-capacity 의무가 사유 문자열에서 종단되고 아무것도 보존하지 않음 → **커널 라운드 목록**: ① `CommandType` 재사용 4건(멤버 신설) ② 결정 만료 술어(`max_decision_age_ms` + continuity 앵커 발행시각을 내는 승인 발행자 — Phase 5) ③ worst-credible-capacity 보존 의무.
