...
실제로 `compose_paper_runtime`을 호출하고 구동하는 코드 경로로 바뀐 시점). **표는
이 시점에 고정한다** — main 이 그 뒤로 움직였다고 매번 전체 표를 재측정하지 않는다;
아래 두 이름의 상태만 이 문서가 정확히 무엇을 다루고 다루지 않는지 알 수 있도록
따로 적어둔다:

- `kis_witness.yaml`(W3, PR #726)은 이 표의 측정 시점 **이후** main `3dae36a3`에
  착지했다(`tos_runtime.recon.witness_kis_config` — 로더 docstring이 스스로
  `tos/runtime/config/kis_witness.example.yaml`을 가리킨다). **이 표에 포함되어
  있지 않다** — 이 표의 30개/6개/24개/19개/5개 카운트 어디에도 들어 있지 않으며,
  이 표를 근거로 그 파일의 채택 여부나 필수/옵션 여부를 판단해서는 안 된다.
- `kis_quote.yaml`(W2, PR #727)은 이 글을 쓰는 시점까지도 **아직 병합되지 않았다**
  — 위와 같은 이유로 이 표에 포함되어 있지 않다.

두 이름 모두 이 표에 **행으로 추가하지 않는다** — 표의 나머지 30개 행과 다른
커밋 시점에서 측정한 행을 하나 섞으면, 표 전체의 "측정 시점" 표기 자체가
무의미해진다. 두 이름을 표에 넣으려면 표 전체를 그 새 시점에서 다시 측정해야
한다(산술로 끼워 넣지 말 것 — 이 레인이 처음에 한 것과 같은 방식으로 소스에서
다시 측정할 것).
