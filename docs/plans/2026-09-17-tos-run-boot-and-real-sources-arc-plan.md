...
**상태 정직 표기.** 운영자 지시 4항 중 **main 에 착지한 것은 3항**(① `run` 구동 ·
② (c2) 실 시세 · ③ 브로커 증인)이고, ④ 커널 라운드 #4 는 별도 계획으로 착수한다
(`docs/plans/2026-09-17-tos-kernel-round-4-plan.md`). 기준선 main `7b76e217` **2292** →
main `abd8ba44` **2445**(실측 · 커널 9522). 전 웨이브 **커널 diff 0**.

> fact-check 지적(2026-09-17): 이 절의 초고가 W2 를 「착지 완료」로, 2445 를 「최종」으로
> 적었다. 그때 W2 는 PR #727 OPEN 이었고 main 은 2374 였다 — **착지하기 전에 착지 기록을
> 쓴 것**이며, 이 아크가 내내 잡아온 과잉 주장과 같은 부류다. 정정해서 「2항 착지 · W2 머지
> 대기」로 되돌렸고, 그 뒤 #727 이 실제로 머지되어(`abd8ba44`) 지금 문장이 됐다. 숫자가
> 결국 맞았다는 것이 그때 쓴 것을 사후 정당화하지는 않으므로, 경위를 지우지 않고 남긴다.
...
| **W2** (c2) 실 시세 | #727 → `abd8ba44` (2445) | KIS 모의 시세 `ObservationIntake` · 토큰 수명주기 공유 추출 · `intake_kind` 명시 선택 · 팬텀 틱 방지 | 1차 HIGH 1 → 조치 → 재심 MEDIUM 1·LOW 1 → 조치 · `privacy-gate` 위반 0 |
