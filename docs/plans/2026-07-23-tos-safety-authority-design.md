# 설계 문서 #6 — Safety Authority Validity + Epoch Fencing + Partition Behavior 계약 (2026-07-23, v1.2)
...
>
> **비준 기록**: **2026-07-24 운영자 비준(v1.1) — 효력 발생.** *(v1.2 = §6.3 조건7 부등호
> 전치-오기 에라타만 — 의미 변경 아님(문서 자기모순·ADR 불일치 정정), 비준 효력 유지; §10.1 v1.2.)* 독립 비평 리뷰 **REJECT**(CRITICAL 0 /
...
7. no dominating safer state(`_PRECEDENCE_RANK[dominating_state] > rank[DEGRADED_PROTECTIVE]`이면 거부 —
   ...
   **[v1.2 에라타]** v1.1까지 본 조항의 부등호가 `<`로 **전치(transposition) 오기**돼 있었다 — §5.3의
   rank 정의(높을수록 safer/dominant: HALTED=4 > … > DEGRADED_PROTECTIVE=2)와 자기모순이며, 문면대로면
   HALTED/CONTAINED 지배 하에서 lease가 *유효*(치명적 fail-open)·LIVE_NORMAL에서 *무효*(역방향)가 된다.
   올바른 규칙: **strictly-safer 지배 상태(rank > 2 = CONTAINED/HALTED)만 거부**하고, 동급
   (DEGRADED_PROTECTIVE 자신) 및 less-safe(LIVE_*)는 이 조항으로 거부하지 않는다(ADR §13.2 "no dominating
   safer state forbids transmission"·§14.4 "dominating CONTAINED or HALTED" 정합). 구현이 이 오기를
   발견·안전-올바른 방향으로 구현했고 독립 코드 리뷰가 진리표·ADR 대조로 확정했다(§10.1 v1.2). 참고:
   §5.3 `restrictive_dominates`의 `rank≥2`(new permissive grant 지배)는 **다른 의미의 별개 연산자**로
   둘 다 올바르다(lease 자신의 posture는 자신을 금지하지 않으나, 새 permissive grant는 지배됨).
...
- 2026-07-24: **v1.2 — §6.3 조건7 부등호 전치-오기 에라타(의미 변경 아님, 비준 효력 유지).** 구현
  (`tos/src/tos/authority/predicates.py`)이 v1.1 조건7의 `_PRECEDENCE_RANK[dominating_state] <
  rank[DEGRADED_PROTECTIVE]`가 §5.3 rank 정의(높을수록 safer)와 **자기모순**임을 발견 — 문면대로면
  HALTED(4)/CONTAINED(3) 지배 하에서 lease **유효**(치명적 fail-open), LIVE_NORMAL(0)에서 **무효**(역방향).
  안전-올바른 `>`로 구현했고, 독립 코드 리뷰(ACCEPT-WITH-MINOR, CRITICAL 0/MAJOR 0/fail-open 0)가
  5-상태 진리표 + ADR §13.2("no dominating safer state")·§14.4("dominating CONTAINED or HALTED") 대조로
  **구현이 옳음을 확정, 문서 에라타 지시**. 본 v1.2가 조건7을 `>`로 정정하고 근거를 in-doc 기록(리뷰
  MINOR-2: out-of-band 근거의 계약 내 편입). §5.3 `restrictive_dominates`의 `rank≥2`는 별개 의미(new
  permissive grant 지배)로 둘 다 올바름을 명시. 그 외 조항·비준 효력(2026-07-24, v1.1) 불변.
