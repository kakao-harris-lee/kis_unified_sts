...
- **후속 정정(2026-09-16 저녁)**: 운영자가 §6 ①②③ 전부 채택 → 실인스턴스 3종 + DR-0003 `Accepted` 착지(PR #706 → `9ffc8371` · AFG 로더 `account_scope` TBD 거부 발견·수정) · **step 7 관측 부재는 스키마 변경 없이 해소**(action-flow 관측 완결 계획 · PR #707 → `da3b5ba1` — 서베이가 두 축 모두 기존 내구 evidence 로 관측 가능함을 실측 · 위 «범주적 불가» 는 `flow_observation.py` 초기 독스트링의 인박스 admission dedup 만 본 판단이었음 · e2e step 7 GRANT 도달). 잔여 = (a′) · (b′ 잔여: 단일 원천 포지션 · 계약 수 차원 · `committed_flow_vectors` `()`) · (c).
