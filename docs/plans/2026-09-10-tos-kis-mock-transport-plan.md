...
## 9. 실행 결과 — T2 착지 (2026-09-10 · 브랜치 `feat/tos-kis-mock-transport-t2` · main `7f3ffa52` 기점)

- **레인 A `b3c0f219` codec 결속**: `KisOrderWireCodec.encode_fields`(값 수준 · `encode(seal)` 은 위임) · `compose/_request_digest.py`(`RequestBytesDigestSource` · `CapsuleStandInDigest` · `KisWireCodecDigest`) · `ComposeContextResolver.request_bytes_digest_source` — attempt 마다 digest 1회 계산해 `EgressRequestRecord.request_bytes_digest` 와 `capsule_egress_request_digest` 양쪽에 같은 값 · e2e 핀: 실 게이트웨이 seal 의 `request_bytes_digest == codec.digest(encode(seal))` · `load_kis_mock_transport_config(codec_bound=...)` — `mode: live` 는 compose root 가 codec 을 결속했을 때만 로드.
- **레인 B `2bbca755` non-live admission**: `compose/_nonlive_admission.py::nonlive_broker_consuming_admitted` 5조건(posture · admissibility ∈ {ADMISSIBLE, REDUCED} · 전 tuple MOCK_ORDER/NON_AUTHORIZING_READ × BROKER_SIMULATION · `risk_relevant_live is False` · INSTANCE environment 일치) · `live_scope_authorized = ① ∧ (② ∨ admission)` — 게이트 ①② 불변 · `coordinator_preconditions.yaml::nonlive_broker_consuming.admitted`(null=부팅 거부) · 예시 스코프 전수 sweep: MOCK_STOCK_ORDER(`profile_evidence_ok: true`)+posture true 만 admit · REAL_* 는 어떤 posture 로도 거부.
- **레인 C `f29329db` compose 결선**: `compose/_transport_wiring.py`(`TransportKind` · `load_transport_config` — INSTANCE 문서의 `_kis.endpoints.rest_base` 로 host 봉인 · `refuse_transport_scope_mismatch`(kis-mock ⇔ broker-reaching MOCK 스코프 · synthetic ⇔ non-broker) · `refuse_custody_principal_mismatch` · `SealRegistry`(게이트웨이 `SEND_SEALED` evidence 의 seal 을 `on_record` 로 포획 · 단일 사용 pop) · `build_transport`(`build_client(config)` 만)) · `InstanceDocument.rest_base` · `FileCustody.PROVISIONED_SCOPES` += `kis_mock.app_key`/`app_secret` · `FileCustody.scope_principal`(secret I/O 0) · `--transport {synthetic,kis-mock}` · boot-integrity 에 `kis_mock_transport.yaml` digest 행 · 정직 deny e2e: Coordinator **통과** → 게이트웨이 deferred 게이트 deny · transport 호출 0.
- **독립 리뷰**: 1차 needs-attention(HIGH 1 · MEDIUM 5 · LOW 6) → 처분 `76d8c46e`(SealRegistry 를 kis-mock 에만 결선 + `SEND_REFUSED` 축출 · 스코프명 상수 단일화 · `risk_relevant_live` getattr 거부 · host 부재 12케이스 · `_finalize` → `compose/_finalize_wiring.py` 분리(`_wiring.py` 1238→**1088** · 함수 예외 폐기) · typed `TransportWiringError`) → 재심 **approve** · 뮤테이션 8종 red. 런타임 **1132** · 커널 9405 · 커널 diff 0.
- **잔여(비차단)**: ① crossing 테스트는 deferred 게이트를 단독 격리하지 못한다 — deferred 를 SATISFIED 로 강제해도 item 6(INSTANCE PROHIBITED)이 독립 차단 ⇒ **T3 반사실은 두 차단을 함께 들어야** transport 도달을 실증 ② 커널 `SendSeal` docstring «`request_bytes_digest` 는 account+instrument 단위» 문언이 stale(이제 attempt 단위) → 커널 라운드 #2 ③ `_finalize_wiring` 의 `_wiring` private dataclass import 는 단방향이나 역방향 금지가 기계적으로 없음 ④ `wire_engine_and_driver` 136→142.
- **정직 상태**: 실주문 0 · `mode: live` 는 로드 가능해졌으나 게이트웨이 deferred 6항목 UNKNOWN(Phase 5 W3) + item 6 INSTANCE PROHIBITED(P0-2) 가 여전히 송신을 막는다 · 값 제안표(런북 §4) 운영자 승인 전 예시 YAML null 유지.

## 10. 실행 결과 — T3 착지 (2026-09-11 · 브랜치 `feat/tos-kis-mock-transport-t3` · main `f52b69f5` 병합)

- **착지**(테스트만 · 커널 diff 0 · 프로덕션 diff 0): `tos/runtime/tests/compose/test_kis_mock_e2e_honesty.py`(8 테스트) + `_fixtures.py::write_kis_mock_transport_config` 오버라이드 확장. 런타임 **1182** · 커널 9405.
- **정직 deny(evidence 수준)**: Coordinator 통과(`LIVE_SCOPE_NOT_AUTHORIZED` 0) → 게이트웨이 SEND_BOUNDARY_VERIFICATION 에서 `SEND_REFUSED` 1 · applicability `BROKER_RESOURCE_CONSUMING` · 17항목 verdict 전부 기록 · deferred UNKNOWN = **{4, 5, 7, 8, 9, 10}** 정확 집합 · item 6 DENIED(PROHIBITED) · `SEND_SEALED` 0 · `TRANSPORT_*` 0 · `send_once` 0 · `SealRegistry` 비어 있음. 정직성 가드: 무패치 + `mode: live` + 응답 가능한 가짜 서버 ⇒ 요청 **0**(양성 대조: 같은 서버가 반사실 B 에서는 응답).
- **발견 — 차단은 셋**: deferred 게이트·item 6 외에 **item 12**(`VENUE_SESSION_ACCOUNT_AND_BROKER_CONSTRAINT_GENERATION` · `derive_item6_item12` 의 `broker_constraint_generation_current` · INSTANCE `degraded_since_authorization` 미기입 = 같은 P0-2 사실)가 독립 차단. 리뷰어가 독립 재현(deferred+item 6 을 들어도 item 12 «P0-2-blocked» 로 deny). 반사실은 `derive_item6_item12` 한 시임으로 6·12 를 함께 든다(P0-2 종결 = 하나의 사실). §0 «3중 차단» 문언은 «deferred(Phase 5) + item 6/12(P0-2)» 로 읽는다. 단독 lift 두 경우(c: deferred 만 · d: P0-2 만) 모두 여전히 deny · transport 0 — 핀 완료.
- **반사실 A(dry_run)**: `SEND_SEALED` → `NETWORK_CALL_ENTERED` 순서 · `TRANSPORT_DRY_RUN.request_bytes_digest` == seal `request_bytes_digest` == **수기 9필드 JSON 오라클**(codec 미호출 · 정렬 키·compact·ascii · `format(Decimal, "f")`) sha256 · 예약 → `QUARANTINED_UNKNOWN` · 소켓 0.
- **반사실 B(live · 가짜 KIS 127.0.0.1)**: 주문 POST **정확히 1** · 수신 본문 sha256 == seal digest == 오라클(«바이트 = 봉인 바이트» 가 인코딩 정확성으로 실증) · `ACK`(`broker_execution_id=ODNO`) `EGRESS_RESULT` 재주입 1회 · evidence 어디에도 app key/secret/token 바이트 0.
- **독립 리뷰**: 1차 needs-attention(MEDIUM 2 · LOW 3 — 페이싱 블록이 증명 없이 주장(두 번째 틱은 승인/액션플로 단계에서 정지) · «독립 재계산» 이 같은 codec 호출) → 처분 `eca0679f`(블록 삭제 · 수기 오라클 · item 12 정확 잔여 핀) → 재심 **approve** · 뮤테이션: digest 대조 제거 red · item 6+12 skip red · double-send red · `_decimal_to_kis_string→"1"` red(2건) · 직렬화 레시피 변경 red · 페이싱은 T1 테스트 소관(이 파일은 주장 안 함).
- **§5 T3 종료 조건**: 실 MOCK 송신 0 evidence 실증 ✔ · 반사실 바이트-봉인 일치 ✔ · ACK 재주입 1회 ✔. 남은 운영자 종료 조건 = 모의 서버 dry_run evidence 1건(T4 · 값 제안표 승인 후).
