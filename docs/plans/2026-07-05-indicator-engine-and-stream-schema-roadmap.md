...
- **WS-A0 (부분):** `pyproject.toml`에 `TA-Lib>=0.5.1`(core), `performance=[numba]`·
  `backtest=[vectorbt]`(extra) 선언. **CI 실증:** `python:3.11-slim`에서 TA-Lib wheel
  (`ta-lib-0.7.0`) 무빌드 설치 성공(PR #576 Docker test profile). **Polars는 이 PR에서
  제외** — 이 변경분이 polars를 import하지 않고, 설치 시 `test_data_engine.py`의 기존
  polars-1.x 비호환이 드러남 → polars 실제 사용 코드(WS-A2 배치)와 함께 도입(그 PR이
  data_engine 호환도 소유). (남은 것: Dockerfile TA-Lib 편입, numpy≥2.0 승격 스윕.)
...
- **WS-A2 (씨앗+본체 착수):** `IndicatorEngine`이 `compute_many` 스펙 dedup +
  `flat_panel` 제공. 신규 `cache.py` — `PanelStore`(추상)·`InMemoryPanelStore`·
  `IndicatorCacheEngine`(스펙 1회 dedup, 심볼별 flat 패널 계산·캐시, `refresh_many`가
  Polars/pool 병렬 진입점). Redis PanelStore·`talib.stream` 증분은 후속(인터페이스 불변).
- **2번째 백엔드(추상화 검증):** `NumpyBackend`(vwap/rvol, 순수 NumPy) 추가. `default_engine`이
  TALibBackend(표준)+NumpyBackend(커스텀) 합성 — 표준은 TA-Lib, 나머지는 NumPy로 라우팅.
- **DRY:** `last_finite()` base.py로 승격(양 백엔드 공유).
- **검증:** unit 45개 green(TA-Lib RSI parity·vwap/rvol 수기계산·dedup·라우팅 포함),
  ruff/black/mypy(engine) clean. 4커밋(`feat/indicator-engine-track-a`).
- **Candle 어댑터 + shadow parity 하네스(착수, 안전·추가):** `adapters.window_from_bars`
  (OHLCVBar Protocol — services 미import, 레이어 청결)·`shadow.ShadowDelta`·
  `test_shadow_parity`. **측정된 위임 게이트 판정(고정 시드):**
  - **ADX** = 신·구 Δ≈0.002(둘 다 Wilder) → **위임 안전, 값 변경 없음**.
  - **ATR** = 구 `_calc_atr_raw`(SMA-of-TR) vs TA-Lib(Wilder) 발산 → **백테스트 게이트 필요**
    (스톱/엣지 필터 영향).
  - **Stochastic** = 구 fast %K vs TA-Lib slow 발산 → **게이트**(또는 backend를 `STOCHF`로
    바꿔 fast 규약 보존).
- **미착수(다음 경계 = 값변경/기존파일 접촉, 게이트 필요):** 손구현 `_calc_*` 위임/삭제
  (ADX부터 = 무게이트 안전; ATR/Stoch = 백테스트 게이트), 빌더 배관 배선(WS-A3),
  Redis PanelStore, `talib.stream` 증분, vectorbt/Numba(WS-A4/A5).
