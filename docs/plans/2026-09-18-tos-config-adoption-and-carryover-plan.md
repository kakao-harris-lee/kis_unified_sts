...
| fact-check(#744) | 「283 = 270 직접 + 4 캐스케이드 + 11 무관」 | **합이 안 맞는다**(285). 팀리드가 `comm` 으로 다시 재 **53 + 230** 으로 정정 |
...
| 부류 | 횟수 | 사례 |
|---|---|---|
| **부분을 전체로 읽음** | 5 | `grep \| head` 절단 3회(그중 하나가 #740 의 HIGH — 「`STOCK_MARKET_DATA_SOURCE` 는 테스트에만 남았다」가 거짓이었고 프로덕션 호출부가 31건 중 **마지막 줄**이었다) · `gh` 의 「볼 것 없음」과 「다 통과」가 둘 다 exit 0 인 것에 2회 |
