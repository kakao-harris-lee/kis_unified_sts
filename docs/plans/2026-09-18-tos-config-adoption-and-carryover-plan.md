...
| 13종을 한 커밋에 몰아서 채운다 | 부팅 실패 시 어느 값이 원인인지 분리되지 않는다. 파일 단위로 나눠 각 단계가 부팅 진척을 증명하게 한다 |
...
| # | 내용 | 종료 조건 |
|---|---|---|
| A-1 | **제안표 저작**(문서). (가) 12개 정책값 + 13종 example-only 파일의 필수 리프. 값마다 **근거 한 줄**. 안전 계열은 `⚠` 표로 분리 | 저작한 값 **전부에 근거**가 붙어 있음 · (다) 부류 **0건** |
