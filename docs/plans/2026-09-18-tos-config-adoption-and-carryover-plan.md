...
| **B-1** | `rcl/gates.py` · `rcl/log.py`: 전이 payload 에 `committed_vector` 를 **명시 키**로(없음 = `null`), fold 가 읽고 `held_reservation_map` 이 열을 읽어 둘 다 `CapacityVector` 재검증으로 정규화. **B-1 이전 엔트리(키 부재) = 벡터 비교 제외**(`UNRECORDED_VECTOR`) — pre-K-4 와 post-K-4 가 payload 로 구별되지 않고, post-K-4 행의 열에는 엔트리가 기록하지 않은 **정당한** 벡터가 있을 수 있다. ⚠ 1차 구현은 키 부재를 `None` 으로 접어 **정당한 기존 로그에서 `CommitLogCorruption` → 부팅 거부**를 냈다 — 자기 리뷰가 잡았고, main 코드로 쓴 RCL 파일을 1차 코드가 거부 · 수정 코드가 통과함을 실측했다. 상태·범위는 계속 비교되고, 그 예약의 다음 전이부터 벡터도 다시 덮인다. 「없음」과 「빈 벡터」는 양쪽 모두 구별된다. KNOWN LIMITATION 주석 제거 | 기존 로그 재폴드 판정 불변(3017 통과 · **main 이 쓴 로그 교차 실측**) · 열 직접 변조 5종 검출 · 뮤테이션 4종 전부 red |
| **C-1** | `recon/service.py` `_join_orders_by_execution_id`: `attempt_id=None` 증인 주문을 **같은 거래일**·**같은 ODNO** 영수증의 attempt 에 1:1 일 때만 조인. 고아 유지: 두 attempt 에 걸친 id · 한 attempt 로 모이는 두 주문 · 이미 직접 매치가 있는 attempt · 그 attempt 의 영수증이 id 를 하나로 말하지 않음 · 수량 비교에 쓰이는 영수증의 id 가 다름 · 영수증 또는 증인 조회일 **날짜 부재/불일치** · 공백 외 정규화. ⚠ **폴백 리뷰(Codex 미가용, 비독립)가 1차 구현의 fail-open 2건을 재현했다** — (S1) 다른 영수증으로 조인해 불일치를 MATCHED 로 병합, (S2) KIS ODNO 는 일별 순번인데 영수증은 전 이력·증인은 당일만 읽어 **다른 날 같은 ODNO 로 조인 → 두 permit True**. 둘 다 막았다. **그 결과 현재 조인은 발화하지 않는다** — egress-result 증거가 거래일을 기록하지 않기 때문(KIS 증인은 조회일을 이제 보고함). 조인이 실효를 가지려면 증거에 거래일 기록이 필요하다(후속 결정 사항) | 합성(날짜 있는) 이중으로 조인·거부 규칙 전부 핀 · 실 KIS 증인 + 실 영수증 리더는 날짜 부재로 **여전히 조인 안 됨**(pin 이 그 사실을 고정) · 가드 6종 각각 제거 시 red |
| digest | `expected_code_digest` 5차 재도출 `b96213f5…` → `2a7e483d…` → (B-1 회귀 수정 후) `90260205…` → (C-1 폴백 리뷰 조치 후) `1da8b701…`(`print-digests` = `observe_source_tree_digest()`) · `release.yaml` + `_VALUE_PINS` | 새 digest 로 Stage A 통과 부팅 확인 |
