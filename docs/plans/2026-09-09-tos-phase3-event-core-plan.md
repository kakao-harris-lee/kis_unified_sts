...
**CR8 하우스키핑 착지(`9ca76084` · 2026-09-09 · 비차단)**: S1 P12 공존 테스트(게이트 거부 tick 은 unverifiable 로 오귀속되지 않음 · MR3 하에서 `2 == 1` red) · S2 `REPLAY_RECEIPTS_UNVERIFIABLE` append 를 divergence raise 앞으로 — 테스트가 수정 전 스스로 실패(실결함이었음) · S3 `_replay_build_core` 3중 복제 제거(import). 격리 재검증: 런타임 `711 passed` · mypy 75 clean · ruff/black/firewall/lint-imports/예산(36)/completion PASS · `tos/src` diff 0. 브랜치 tip `9ca76084`.
