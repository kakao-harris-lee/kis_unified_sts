...
ClickHouse는 백테스트와 장기 시계열 분석에는 유용하지만 개발/모의투자/실전투자 runtime의 필수 인프라로 두기에는 과하다. 현재 `docker-compose.yml`은 Redis만 컨테이너로 띄우고 ClickHouse는 `host.docker.internal`의 외부 설치를 기대한다. 그 결과 프로젝트가 ClickHouse 설치 서버에 묶이고, 개발 서버 이동과 paper/live 환경 분리가 어려워진다.
...
| 용도 | 예시 | 필수성 |
|------|------|--------|
| 과거 OHLCV 소스 | backtest, prewarm, removed ML/RL research 자료의 historical export | research에는 중요, runtime에는 fallback 가능 |
...
| research | `kis_research` | 백테스트/대용량 분석 | Parquet/DuckDB + optional ClickHouse |
...
- backtest/research commands can run against Parquet/DuckDB data without ClickHouse. ML/RL commands are decommissioned under the ML/RL removal plan.
