...
- `20260820-184748/U17-PREVENTION-CHECK-V222-ADDENDUM.md` — **v2.22 에라타 1차~4차 재동결 `11e138a5`
  ...
  **레인 3 GET-only 재측정**(계약이 «실측»이라 적은 값 전건 재현 — suite↔run 1:1·`filter=latest` 가
  ...
  PR #562 close→reopen→run `run_attempt 1` 까지 재현) · **§5 관측 7건**(T-2 «`E` 불변» 문장은 행이
  편집됨[의미 무접촉·행 무접촉 구별] · T-3 `--include` 0회 계수 범위 비대칭 · T-4 «41블록» 인용에
  엔드포인트 부재 · **T-6/T-7 = 이 부속 자신의 결함 2건**[술어 timeout ERE 를 파싱값에 걸어 `010`·`+5`
  통과 = fail-open → 훅 H20/H21 로 폐쇄 · 검사기 라벨 naive grep 위양성]) · 부기 = 하지 않은 것
