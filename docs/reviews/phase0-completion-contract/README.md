...
- `20260821-151013` — 레인 B **v2.22 재심** (`needs-attention`·`NOT_PASSED`, 재결속 `a0346749`
  [**계약 에라타 6차 `5e96512e` 만 개정 — 개발계획 blob `b2985a05…` 은 직전 결속과 동일**] 후 심사.
  **이 아크 최초의 «회피 0 · 부분해소 0» 재심 — 직전 4건 전건 해소, 아크 누적 해소 11 → 15.**
  #1 은 4판 연속 회피/부분 끝에 닫혔고(`검증 → 실행` 순서 + 잡 객체 닫힌 세계 + `SHELL_OK` +
  전 노드 중복 키 + 자기수복 반사실에서 검증 rc 1·실행 미도달 실측), #2 도 3연속 부분해소를 벗어났다
  (`u17-verify` 소유·도입 순서 고정 + `D=∅` 에서 `(b-blob)@target` 양성/음성 실제 분기).
  F#2·F#4 도 해소. 신규 5건 = **high 2 / medium 2 / low 1 · 전건 채택 · 기각 0**:
  **F#1** `workflow_run.path` 완전일치 핀이 **미확인 거동 의존**(GitHub 공식 예시 응답은
  `".github/workflows/build.yml@main"` 으로 `@<ref>` 포함 — 발화 시 정본 run 이 영구
  `PREVENTION_UNVERIFIED_REVISION` 이 되고 진짜 개변과 구별되는 관측면 없음; 계약이 v2.21 3차에서
  **스스로 성문화한** «미확인 거동을 핀하면 미확인 거동의 승인이다»[`:5278`] 의 정반대 적용.
  오케스트레이터 실측 정밀화: 이 repo 1,769 run 전수 `@` 포함 **0** = 도달 가능하나 현재 미발화,
  부수로 `.github/workflows/` **밖** 값 1건[`dynamic/copilot-pull-request-reviewer/…`] = 값 공간이
  계약 모델보다 넓다) · **F#2** check-runs 열거 완전성이 **공식 1,000-suite 상한을 미포섭**
  (verbatim: «If there are more than 1000 check suites on a single git reference, this endpoint
  will limit check runs to the 1000 most recent check suites.» — `--paginate` 는 **이미 잘린 우주**를
  끝까지 돌고 `total_count` 도 같은 잘린 값이라 «수집 수 == total_count» 가 **잘림을 관측하지 못한 채
  성립** = vacuous green. 5차 ⓧ 가 관측면을 헤더→본문으로 옮겼으나 **옮긴 관측면도 같은 잘린 우주 안**)
  · **F#3** 예방 통제 4종(`tos-gate.yml`·하니스·`u17-verify`·**외부 룰셋**)의 rollback 부재
  (§12.2 revert 표 실측 = `P-0`/`D0-A`/`D0-4`/`D0-5` 4행뿐이고 4종은 어느 행에도 없음 —
  나머지는 전부 «커밋 revert» 인데 **룰셋은 커밋이 아니다**[서버측 상태]) · **F#4** 에라타 처분
  집계 **형제 미전파**(지배 소스 `:224` 는 «[2차 ⓢ — MINOR-11] 1차 문언은 채택 26 이라 적어 **1 과다
  계상**했다 → 채택 25·기각 1·무변경 1» 로 **자기 교정을 이미 기록**했는데 `:4414`·`:4437` 은 정정 전
  «채택 26» 그대로 — **그리고 6차 ⓑ 가 바로 이 문제형에 «자기 문서를 세는 숫자는 그 문서에 두지
  않는다» 규칙을 성문화하고 `:5466` 한 자리에만 적용했다** = v2.22 의 결함이면서 동시에 **6차 에라타
  자신의 S-22 실패**) · **F#5** 뮤테이션 종수 25 vs 실제 26행(자체 요약 «뒤집힘 19 / 불변 7» = 26 이
  행 수와 일치 — 지배값은 행 집합이므로 **26 이 참**, 계약 `:141` 이 오기 «25/19» 를 재인용).
  **분포의 의미**: 신규 5건이 처음으로 계약 «내부» 논리를 벗어나 **repo 밖 경계**로 이동했다
  (F#1·F#2 = 외부 API 의 문서화된 형상·상한 · F#3 = 외부 서버 상태의 rollback · F#4·F#5 = 집계 전파).
  내부 술어 설계는 6판에 걸쳐 수렴했고 남은 것은 **계약이 자기 힘으로 검증할 수 없는 외부 사실에 대한
  가정**이다. D 기준(개발계획 stale)은 심판·오케스트레이터 **독립 이중 확인으로 건전** — 개발계획이
  계약을 인용하는 자리는 라벨 5종 + 하니스 sha 1종이고 전부 HEAD 에서 해소되며 버전 문자열은 **0건**
  (판 번호 드리프트 구조적 불가). C 기준 «층» 축의 다른 형제 없음. 심판 실행 = Codex 모드 B
  (`task --effort high`·write=false·10m 40s·thread `01a022e1`) — 모델 식별자는 `--model` 미지정 +
  companion 미기록으로 **정직하게 미확인**. 다음 개정(v2.23)의 입력. `evidence/focus.txt` 동일 규칙으로 보존)
...
- `20260822-000132/U17-PREVENTION-CHECK-V222-ADDENDUM-3.md` — **v2.22 에라타 7차 동결 `c9a530ab`
  S-24 addendum**(기록 커밋 시점 872행).  **판정: ⓐ «미종결» — 「닫혔다」로 적을 수 없다.**
  §1 절 범위 diff 공집합 = **8층 rc 0**(⓪ 양 끝 결속 핀 · ① 여집합 hunk 15/구간 16/차이 0 ·
  **①-a 분할 완전성**[구간 ∪ hunk = 원소 전체 · join 무손실] · **①-c hunk 내용 핀**[구조 15 +
  슬라이스 sha 30 + 증감행 19/121] · ①-b 귀속 · ②-b 목록 크기 핀 · ② 명명 절 · ③ 불변식) ·
  증명기 자기 뮤테이션 **24종 죽은 검사 0**.  §2 실행 = **76셀 불일치 0** · **(ㅁ-1) 양극성**
  (gen-2 UR/1 ↔ gen-3 A/0 = **F#1 과잉 차단 실재**) · **(ㅎ-4) 양극성**(A/0 ↔ UV/1 = **F#2
  vacuous green 실재**) · **기각된 처방 대조 구현**(REJECTED·SPLIT 이 (ㅁ-3)·(ㅁ-2)를 A/0 통과
  = 계약 주장 A 실증) · **계약 주장 B 반증**(「첫 `@` 절단이 더 나쁘다」는 등가라 unsatisfiable ·
  예시도 오류) · 뮤턴트 11 죽은 검사 0 · **계약 픽스처 충돌 4자리(C-1~C-4) 판단 기록** ·
  하네스 결함 9건 자기 기록 · 쓰기 감사 shim 증명 + 판별력 대조.  §3 독립 적대 = **findings 9**
  (high 2 · medium 5 · low 2) 중 **7건 오케스트레이터 직접 재검증** — **F-A** ⓐ 가 사다리 단일성
  증명을 무효화(접기 키 `(name, path)` 미터치 · 증명이 «없는 정의»를 인용 · 5차 ⓦ 재개방 ·
  §2-4 가 실행으로 독립 확증 `\|C\|=2`) · **F-B** 「형제 전수 전파」가 거짓 전칭(살아있는
  «정본 path» **19행** · `:6779` U-17-c 정의행 ⑤ = `PREVENTION_UNVERIFIABLE` 의 선언된 유일
  소스가 `path` 로 키잉 = 양방향 파손) · F-C T-84 ⑭(ㄷ) 한 절 안 자기모순 · F-D·F-E **문서화된
  상한 2건 추가 미포섭**(`actions/runs?check_suite_id=` 1,000-결과 · **동명 check-run 1,000 +
  자동 삭제** — 후자는 계약의 축에 직접 걸리고 ⓑ 가 인용한 같은 페이지에 있다) · F-F ⓐ 신설
  파생의 `state` 축 미처분 + 단일객체 닫힌 열거 미갱신 · F-G ⑥ currency 2자리 잔존 · F-H 문헌
  실측 2건 부정확(**6,569 run 에서 `@` 0건** — 「도달 가능」은 무라벨 추론) · F-I NIT.
  **건전 확인 12항 명시**(과대 집계 2건 자기 기각 포함).  §5 = 이 부속 자신의 결함 **12건**
  (T-5→①-b · T-6→②-b · T-10→①-c 가 층을 낳았다 · T-11 값 갱신은 교정이 아니다 ·
  T-12 규칙을 너무 넓게 쓰면 그 문서가 위반자 + 하드코딩 census 재발).  초안 자기계약 검사기
  = 항목 12종 · 모집단 0 인 항목 0 · **초안 뮤테이션 17종 죽은 검사 0**.
  **관통 형태**: 7차는 ⓓ 에서 「규칙 신설은 그 규칙의 전수 적용까지가 한 단위다」를 성문화하고
  **ⓐ·ⓑ·⑥ 세 자리에서 그 규칙을 위배**했다.  다음 = **에라타 8차** → addendum-4 → 재결속 →
  v2.23 재심.  **D0/P-0 착수 금지 불변.**  서버 쓰기 0 · GET-only.

- `20260822-100701/U17-PREVENTION-CHECK-V222-ADDENDUM-4.md` — **v2.22 에라타 8차 동결 `e3d2ddf6`
  S-24 addendum**(기록 커밋 시점 421행).  **판정: 「부분 미종결」 — ⓓ 한 자리 + 문언 2건.**
  §1 절 범위 diff 공집합 = **8층 rc 0**(hunk 21/구간 22/차이 0 · 분할 위반 0 · 내용 핀 21 ·
  귀속 46 선언 헛선언 0 · 닿음 18/신설 10/닿지 않음 28[전건 byte 동일·재분류 0]/부분문자열 10 ·
  불변식 — (4) 열거는 8차 **무접촉** 7==7 · 의도 변경 축은 **단일객체 비-대상 열거 6→7**) ·
  증명기 자기 뮤테이션 **24종 죽은 검사 0**(M13 심층 방어 ⓪/①-a/①-c 각각 · M22 기준선 교체
  ⓪=2) · **오케스트레이터 독립 재실행으로 rc 0·메타값 재현**.  §2 실행 = gen-4 파생(델타가
  접기 키·신규 limb 에 국한 — [STEP]/[LIMB] 태그 byte 동일) · 신규 대조군 (ㅎ-5)/(ㅎ-6) 4/4 ·
  **76셀 불일치 0 · 양극성 = M1X 단독**(gen-3 UR `\|C\|=2` ↔ gen-4 A `\|C\|=1` = **F-A 과잉
  차단 제거의 직접 증거**) · pagelimb 축 양극성((ㅎ-5)/(ㅎ-6) 양성: gen-3 PAGES_OK ↔ gen-4 UV)
  · 역방향 뮤턴트 4종 LIVE(`MU5-gte-to-gt` 가 `\|E\|==1000` 경계 fail-open 직접 실증) ·
  **e5 뮤턴트 7종 gen-4 재실행 전건 LIVE**(addendum-3 §3-4 항 7 이행 · 재-앵커 1건 MP-ii =
  F-A 이동 자리) · C-2 `\|C\|` 로그·C-4 행동 무변경·F-F 비판별 실측 · 하네스 결함 1건(HD-1
  양극성 경로 잠복 크래시 — 판별 셀에서만 발화) 자기 기록 · 쓰기 감사 shim 0건.  §3 독립 적대
  = **findings 3**(medium 1 · low 2 · 전건 오케스트레이터 직접 재검증) — **F-1** ⓓ 가
  `total_count` 관측가능성 잔여를 미등재(**같은 판의 ⓔ 는 등재 — 비대칭** · «규칙 신설 =
  전수 적용» 위배가 8차에서도 한 자리 재발) · **F-2** 전역 지배 선언 범위어 «아래» ↔ «본문
  전역» 자기모순(파손 없음 — 판정 피연산자 0 유지) · **F-3** 8차 커밋 메시지의 (3-1) no-op
  근거가 계약이 기각한 1:1 을 인용(정본 텍스트는 건전 · 오케스트레이터 전사 결함으로 자기
...
- `20260826-000532/U17-PREVENTION-CHECK-V222-ADDENDUM-5.md` — **v2.22 에라타 9차 동결 `c340d602`
  S-24 addendum**(기록 커밋 `a065ca6b` 시점 415행 → **에라타 v1.1 `578388f9` 후 578행**).
  **판정: 「미종결」 — findings 7(high 1 · medium 3 · low 3 · 기각 0).**
  **⚠ 초판 판정 「부분 미종결」·findings 4 는 에라타 v1.1 이 상향했다** — 적대 레인이
  무응답이 아니라 **늦게 응답**했고 그 보고가 초판 판정을 반증했다(오케스트레이터가
  «미도착 레인»을 null 결과로 읽고 판정을 발행한 절차 결함 · 부속 §5 항 5 자기 등재).
  **차단 사유 = 「전건 문언 전용」이 거짓**(에라타 §E-1): 부수① hunk 가 「«미발견»은
  «없음»이 아니라 «미확인»으로 적는다(전칭 부정 규율)」 — **계약 내부에서 이 규율을 진술한
  유일한 자리** — 와 「다음 사이클 문헌 스윕 입력물」을 삭제했고 **둘 다 복원되지 않았다**
  (각 grep 계수 0).  극성 fail-open.  §1 절 범위 diff 공집합 = **7층 rc 0**(⓪ 결속 6핀 · ① 여집합 8구간 ·
  ...
  실증**).  §3 독립 적대 = 초판 **findings 4** → **에라타 v1.1 에서 7**(high 1 · medium 3 · low 3 ·
  기각 0 · 전건 오케스트레이터 직접 재측정) — 상향·신규분: **F-2 low→HIGH**(census 6어휘에
  `limit`/`limits` 부재인데 계약이 verbatim 인용한 상한 셋이 전부 그 동사를 쓴다 = **어휘
  폐쇄 미증명** · 실측된 것은 «상한 부재»가 아니라 «이 여섯 토큰의 부재») · **F-3 low→medium**
  (stale 자기인용이 **둘** — ⓑ `:5827`→`:5841` + ⓒ `:5972-5979`→`:5986-5990` · **두 좌표 모두
  addendum-4 가 8차 blob 에서 잰 값과 일치 = 재측정 없이 전사**) · **F-5 신규 medium**
  (`:5473` 이 (4) 를 «6개»로 적는데 열거는 7 · 9차의 「(4) 열거 7==7」 불변식이 두 자리만 보고
  세 번째를 못 본 **fail-open 검사기** · **이 부속도 같은 구멍을 재현**했고 독립 재실행이
  「위반 0」을 재현했다 = 독립 재실행은 «재현»을 세우지 «검사의 완전성»을 세우지 않는다) ·
  **F-6 신규 low**(ⓐ 가 형제 둘이 진술하는 극성 문장 결손 — §3-3 항 1 을 «부분 미러»로 정정) ·
  **F-7 신규 low**(ⓑ 지배 확장이 `:2910` 재-범위화 · 영향 nil).  유지분: **F-1** 9차가 «유지한다»고 선언한 currency 스윕 술어를 9차가 실행하지 않음
  ...
  **이 커밋이 파일명 5건을 전수 보충하되 불변식 성립은 주장하지 않는다**).  §5 = 이 부속 자신의
  결함 **6건** 자기 기록(초판 4 + 에라타 2 — **⑤ 미도착 레인을 null 결과로 읽고 판정을
  발행 · ⑥ fail-open 검사를 「위반 0」으로 재현하고 근거로 씀**).  오케스트레이터 독립 재실행: 증명기 로그 **byte-동일 재현**.
  다음 = **F-2 는 10차 필수**(「문언 전용」 라벨은 F-2 격하 전 사용 불가) · F-3·F-5 는 한 토큰
...
- `20260826-081517/U17-PREVENTION-CHECK-V222-ADDENDUM-6.md` — **v2.22 에라타 10차 동결 `30987efe`
  ...
  §2 = **35픽스처 × 2세대 = 70셀 · 세대 불일치 13** · **★ 양극성 11자리**(**에라타 v1.2 정정** —
  초판은 13분 낡은 `sweep-CLEAN` 을 읽어 33/66/11·양극성 9 로 적었다.  정본은 `sweep-FINAL`
  이고 신규 픽스처 J9·J7b 가 판별 자리로 추가됐다.  **계수는 «유리한 방향»으로 틀렸으나 그것이
  놔둘 이유가 되지 않는다**) (gen-5 `ACTIVE` →
  ...
  판정 동일 22(비판별 처분의 상태값 불변) · GET-only · **역방향 뮤턴트 13종 LIVE 10 · DEAD 3**
  (**에라타 v1.1 정정** — 초판은 stale 로그를 근거로 「11종 LIVE 8 · DEAD 3 · 그 가드들은
  대조군이 없다」고 적었으나, DEAD 3 은 **구멍이 아니라 방어 중첩**이다: 같은 조건의 «부주의
  구현» 형상 뮤턴트가 **LIVE** 이고 `MU-beta-drop-b` 는 설계 자체가 방어 중첩 탐침이다.
  그 가드들의 독립적 하중은 **«미측정»이지 «부재»가 아니다**).
  ...
  **에라타 v1.1 에서 오케스트레이터 자기 결함 4번째 추가** — SD-1·SD-2 를 자기가 적발해 기록해
  놓고 **같은 판에서 둘을 합친 형태를 저질렀다**(SD-2 형 stale 로그를 읽고 SD-1 형 단정).
  교훈: **재실행할 수 없는 산출물은 읽어서 판정하지 말고 실행 가능한 레인에 재실행시킨다.**
