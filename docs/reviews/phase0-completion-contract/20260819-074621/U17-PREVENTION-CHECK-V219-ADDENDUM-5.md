# U17-PREVENTION-CHECK-V219-ADDENDUM-5 (최종) — v2.19 에라타 5차 `eddbd241` S-24 addendum (절 범위 diff 기계 증명 + E14 파생 경로 «결합 기준» 재실행)

> **비규범 부속** — 계약 v2.19 에라타 5차·최종 `eddbd241`(7,386행) 후 **S-24** 이행. 선행 증거 5건(`90a5ce7d` · `197f4fe4` · `c83e44db` · `d988bd0f` · `4f102c73`)은 U-15-e **(4d) 불변 규율을 준용해 편집하지 않고**,
> ① `git diff db6ce918..eddbd241 -- <계약>` 전문과 **닿는/닿지 않는 절 범위의 기계 증명**(§1)으로 **비영향 변이의 증거가 그대로 결속됨을 선언**하고, ② **영향 변이(E14)만 재실행**(§2~§4)한다.
> **결속**: 실행 시점 HEAD == `eddbd241` · 계약 워킹트리 blob `e8ae0b39` == `git show eddbd241:<계약>` blob(`git diff --quiet eddbd241 -- <계약>` rc=0 · 워킹트리 sha256
> `98d81dacee09b2b523e9472418d8156bdafc21d5a67214274338a10654c2e8fd`) · `eddbd241..HEAD` 계약 커밋 **0** ·
> 하니스 §12.3.4-R 블록 `sed -n '4625,4725p'` sha256 **`957bf49da8fc6ae39f97abe679411afeaa5a59f707f35bf3b3a8c6f9de141f0d`** — **`db6ce918` 의 `:4620-4720` 과 byte-동일**(§1 ④ · §6).
> **판정 소비자는 이 파일의 응답을 신뢰하지 않고 스스로 live 조회한다**(대조용) · **서버 쓰기·설정 변경 0** · 픽스처는 scratchpad 하위 **독립 git 저장소**(`fx84i/*`·`fx82i/*` — 본 저장소 무접촉·worktree 미사용).
- **생성 시각**: 2026-08-19T03:55:09Z (UTC) · 실행 `t84v219e5_utc=2026-08-19T03:52:45Z` · `t82v219e5_utc=2026-08-19T03:54:29Z` · **생성 주체**: 오케스트레이터 지시 하의 실행·조립 에이전트(저작자·심판 아님)
- **실행기 결속**:
  - sha256(`s24-proof-5.sh`) = **`ba19a332e72b7415ec2845bf8bafcee79d3804f90dcb567d47ed7294bc30ecb5`** (§1)
  - sha256(`u17-verify-v219e5.sh`) = **`cd3e9e1eae5b1de7fb582869dedae1bb5763eb41a6f07e7880e119e66857a944`** — 직전 `dd0617ab…` 대비 **diff 23행**(E14)
  - sha256(`u17-ctrl-cwdrel-e5.sh`) = **`c142ed79cef507b4ca04c5445bf7ffc8b29f911eb131ca59a18be29ca3ad56da`** (**E14 대조군** — 결합 «없이» cwd 기준 검사 + ㉠ 발화 제거. diff 24행. 판정용 아님)
  - sha256(`u16-full-exec-v219e5.py`) = **`26f0583afbac6b01b5f97d35d336448e86b1dc0d4d94e7c944297e181718e90d`** (직전 `2a9d254f…` 대비 **diff 25행**) · sha256(`u16-ctrl-cwdrel-e5.py`) = `4f96504a0a5a8a1777c340ba4b672b398416991d04a6631b937acfe356f563c6` (diff 21행)
  - sha256(`t84v219e5.sh`) = `554a174d29dfc75844627d3275e7a4a6d7f0632e98b5eb818a4eabb8f609289f` · sha256(`t82v219e5.sh`) = `b4645cfec96bdded3a9c2f07d04793baad1498d1fbc723bbcddd53ad99fd8403`

## 0. 결속 선언 (S-24 ②·①)

| 변이 (선행 증거 5건) | 닿는 에라타 5차 절 | 처분 |
| --- | --- | --- |
| **T-84 ①~⑫ · T-82 ⑮~⑳ 전건** · (a)·(b)·(α) · U-17-c 전순서/상태표/기계 조건 · **[E11] P_first 카디널리티** · U-16-d 전순서/선-검사/g-단락 · `c_APP` 수식 · **[E8] 참조 D·C_R** · U-16-a2/h/b · U-15-g-4 | — (§1 ③ 이 **23건 ∅** 로 증명) | **비영향 — 선행 증거 5건 그대로 결속** |
| addendum-4 **L-1**(`--git-path` 상대 반환 → 저장소 밖 cwd 에서 거짓 ABSENT) | **E14** 결합 기준 고정 | **재실행**(§3·§4) |
| addendum-4 **L-2·L-3·L-5** | 계약 본문에 한 줄씩 성문화 | **비영향(문언 정합)** — L-2 는 §4-1 [K-4] 실측이 이미 addendum-4 에 있음 |

- **결과 요약 (재실행분 · stdout·rc 원문 그대로)**:

| # | 변이 | 실행기 | 방출값 | rc | 기대 (eddbd241 E14) | 대조 |
| --- | --- | --- | --- | --- | --- | --- |
| **0** | 파생·결합 프로브 (U-17·U-16) | (드라이버) | `--git-path info/grafts` = **`.git/info/grafts`(상대)** · **cwd(저장소 밖) 기준 → ABSENT(거짓)** · **저장소 루트 결합값 → present** | — | L-1 재현 | **일치 — 거짓 ABSENT 실측** |
| **1** | **[E14]-1** 저장소 «밖» cwd + grafts (U-17) | **대조군**(결합 없음) / v2.19-5 | **`PREVENTION_ACTIVE`(0)** / **`PREVENTION_UNVERIFIABLE`(1)** | **0**/1 | 대조군 fail-open · 판정기 차단 | **일치 — fail-open 실증** |
| | **[E14]-U16-1** 같은 구성 (U-16) | **대조군** / v2.19-5 | **`NO_ROWS_CLEAR`(0)** / **`PROVENANCE_UNVERIFIABLE`(2)** | **0**/1 | 같음 | **일치 — fail-open 실증** |
| **2** | **[E14]-2** `--separate-git-dir`(파생이 «절대») (U-17) | v2.19-5 | 검사 경로 = 파생값 **그대로**(present) · «이중 결합» 가정 경로는 ABSENT → **`PREVENTION_UNVERIFIABLE`**(1) | 1 | «상대면 결합·절대면 그대로» 분기 정확 | **일치 (자기신고)** |
| | **[E14]-U16-2** 같은 구성 (U-16) | v2.19-5 | 같음 → **`PROVENANCE_UNVERIFIABLE`**(2) | 1 | 같음 | **일치 (자기신고)** |
| **3** | **정상 회귀** (U-17) | v2.19-5 | cwd «밖» **`PREVENTION_ACTIVE`**(0) · cwd «루트» **`PREVENTION_ACTIVE`**(0) — 부재→부재·상태값 불변 | 0/0 | cwd 무관 | **일치** |
| | **정상 회귀** (U-16) | v2.19-5 | cwd «밖» **`APPROVAL_MALFORMED`**(3) · cwd «루트» **`APPROVAL_MALFORMED`**(3) | 1/1 | cwd 무관 | **일치** |
| | **본 저장소 live** | v2.19-5 | **`PREVENTION_ABSENT`**(2) | 1 | 계약 문언 그대로 | **일치 (인증 실측)** |

**이 파일은 본 저장소의 `PREVENTION_ACTIVE` 를 주장하지 않는다** — 본 저장소 live 는 `PREVENTION_ABSENT` 이고 `ACTIVE` 는 전부 SIMULATED seam·픽스처다.

---

## 1. S-24 ① — `git diff db6ce918..eddbd241 -- <계약>` 전문 + 절 범위 diff 기계 증명

에라타 5차가 닿는 절(hunk **4개**): **H1** 심사 이력 v2.19 행(:118) · **H2** 변경 이력 v2.19 행(:201) · **H3** §12.3.3 (B) 처분표 머리(에라타 5차 서술 +5행) ·
**H4** `U-16-c` `[PARENTS-UNTRUSTED]` 정의 블록(**E14** 결합 기준 + L-2/L-3/L-5 한 줄).
닿지 «않는» 절은 §1-3 ③ 이 리터럴 grep 으로 위치를 파생해 sha256 으로 대조한다 — **하니스 블록 · §8 T-84/T-82/T-81 행 · (a)·(b)·(α) 술어 전문 · C6 host · U-17-c 전순서 10단·상태표·기계 조건 ·
[E11] P_first/P_last 정의+카디널리티 · U-17-d · [E8] 참조 `D`·`C_R` · `c_APP` 수식 · U-16-d 전순서 12단·① 선-검사·② g-단락 · U-16-a2 · U-16-h · U-16-b · U-15-g-4 CORR** — **23건 전부 ∅**.
...
index c2e5763b..e8ae0b39 100644
...
@@ -4368,6 +4368,11 @@ addendum 으로 이행**한다(S-24: 재동결에 대한 절 범위 `git diff` 
...
@@ -7063,9 +7068,22 @@ c_APP(a) = { x ⊑ HEAD :  a ∈ rows(x:LEDGER)  ∧  ∀ p ∈ parents(x): a 
...
### 1-2. `s24-proof-5.sh` — 원문 (sha256 `ba19a332e72b7415ec2845bf8bafcee79d3804f90dcb567d47ed7294bc30ecb5`)

```bash
#!/usr/bin/env bash
# s24-proof-5.sh — S-24 ① «절 범위 diff 기계 증명» (에라타 4차 재동결 db6ce918 → 에라타 5차·최종 재동결 eddbd241).
...
sec "[E14] U-16-c [PARENTS-UNTRUSTED] 정의 블록 (유일 소스)" 'c_APP(a) = { x ⊑ HEAD :  a ∈ rows(x:LEDGER)' '```'
sec "§12.3.3 (B) 처분표 머리 (에라타 5차 서술)" '**[v2.19 에라타 2차 — addendum 적발]**' '| v2.18 finding | 심판 지적'
...
printf '  %s :4620-4720  %s\n' "$OLD" "$(sed -n '4620,4720p' "$b_old" | shasum -a 256 | cut -d' ' -f1)"
printf '  %s :4625-4725  %s\n' "$NEW" "$(sed -n '4625,4725p' "$b_new" | shasum -a 256 | cut -d' ' -f1)"
printf '  두 값 동일? %s\n' "$([ "$(sed -n '4620,4720p' "$b_old" | shasum -a 256 | cut -d' ' -f1)" = "$(sed -n '4625,4725p' "$b_new" | shasum -a 256 | cut -d' ' -f1)" ] && echo yes || echo no)"
printf '  계약 리터럴(본문 인용) 957bf49da8fc6ae39f97abe679411afeaa5a59f707f35bf3b3a8c6f9de141f0d 과 일치? %s\n' \
  "$([ "$(sed -n '4625,4725p' "$b_new" | shasum -a 256 | cut -d' ' -f1)" = 957bf49da8fc6ae39f97abe679411afeaa5a59f707f35bf3b3a8c6f9de141f0d ] && echo yes || echo no)"
...
printf '  sed -n 4625,4725p <워킹트리> sha256 = %s\n' "$(sed -n '4625,4725p' "$WT" | shasum -a 256 | cut -d' ' -f1)"
rm -f "$b_old" "$b_new"
```

### 1-3. `s24-proof-5.sh` 출력 원문 (∅ = byte-동일 · ≠ = 에라타가 건드린 범위)

```text
=== S-24 절 범위 diff 기계 증명 (2026-08-19T03:51:53Z) — 동결 db6ce918(7368행) → 에라타 재동결 eddbd241(7386행) ===
$ git diff db6ce918..eddbd241 --stat -- <계약>
   ...-08-12-tos-phase0-completion-contract-design.md | 28 ++++++++++++++++++----
   1 file changed, 23 insertions(+), 5 deletions(-)
$ git diff db6ce918..eddbd241 -- <계약> | grep '^@@'   (hunk 목록 — 이것이 변경의 전부)
  @@ -115,7 +115,7 @@
  @@ -198,7 +198,7 @@
  @@ -4368,6 +4368,11 @@ addendum 으로 이행**한다(S-24: 재동결에 대한 절 범위 `git diff` 
  @@ -7063,9 +7068,22 @@ c_APP(a) = { x ⊑ HEAD :  a ∈ rows(x:LEDGER)  ∧  ∀ p ∈ parents(x): a 
...
-- ① hunk 사상 (기계 파싱: 각 hunk 안에서 실제로 «바뀐» 행의 old/new 범위) --
  ...
  H3  old[4370,0]        new[4371,5]
  H4  old[7066,3]        new[7071,16]
...
-- ② «닿지 않는» 구간 자동 생성 + sha256 대조 (∅ = byte-동일) --
  ...
  ∅  old[202,4370] == new[202,4370]  sha256=dfdebeb76fedc13c…
  ≠   H3: old[4370,4370] vs new[4371,4375]  — 에라타가 건드린 범위(≠ 기대)
  ∅  old[4371,7065] == new[4376,7070]  sha256=81c077188c14976f…
  ≠   H4: old[7066,7068] vs new[7071,7086]  — 에라타가 건드린 범위(≠ 기대)
  ∅  old[7069,7368] == new[7087,7386]  sha256=0922536a59ad7d7c…   (말미~EOF)

-- 자동 구간 전건 결과: 전 구간 ∅ (변경은 hunk 안에만 있다) --

-- ③ 명명 절 대조 (각 blob 에서 «리터럴 grep 으로 위치 파생» — 하드코딩 행번호 아님) --
  ∅   §12.3.4-R 하니스 블록 (101행) : old[4621,4720] == new[4626,4725]  sha256=0d3a33b007033c4b…
  ...
  ∅   (a) 술어 블록 전문 : old[5266,5303] == new[5271,5308]  sha256=5570120023328a32…
  ∅   (b) 리비전 특정 전문 : old[5366,5431] == new[5371,5436]  sha256=63c63d4e387c6f37…
  ∅   (α) 연속성 술어 전문 : old[5490,5516] == new[5495,5521]  sha256=98e4571de23fbc3f…
  ∅   C6 host 결속 블록 (E3) : old[5239,5254] == new[5244,5259]  sha256=6966d914f5e1cb36…
  ∅   U-17-c 전순서 10단 : old[5611,5621] == new[5616,5626]  sha256=efca55f331d90ca9…
  ∅   U-17-c 상태표 (E9 조건 — 3차는 무변경) : old[5594,5610] == new[5599,5615]  sha256=0060054207110b5a…
  ∅   U-17 (c) 두 상태의 «기계 조건» (E9) : old[5575,5582] == new[5580,5587]  sha256=969719aaa9d8db84…
  ∅   [E11] U-17 (c) P_first/P_last 정의 + 카디널리티 (4차 무변경) : old[5544,5575] == new[5549,5580]  sha256=6a0acf54c7909dcc…
  ∅   U-17-d 강제 지점·종료조건·대조군 : old[5623,5627] == new[5628,5632]  sha256=d31967d667226129…
  ∅   [E8] 참조 1/4 — U-15-g-1 D 정의 : old[4952,4957] == new[4957,4962]  sha256=463ba244686c9fe7…
  ∅   [E8] 참조 2/4 — g6 C_R 정의 꼬리 : old[6946,6951] == new[6951,6956]  sha256=7d78e31d5e99bca6…
  ∅   U-16-c c_APP 수식 3행 : old[7019,7021] == new[7024,7026]  sha256=34771ac4e3a056c6…
  ∅   U-16-d 전순서 12단 표 : old[7156,7168] == new[7174,7186]  sha256=de24dfc1244e20ba…
  ∅   U-16-d ① 선-검사 (E6/E8 — 3차는 무변경) : old[7173,7183] == new[7191,7201]  sha256=35d6bf7fd4d08008…
  ∅   U-16-d ② g-단락 (5~11) : old[7183,7185] == new[7201,7203]  sha256=6bed132451327946…
  ∅   U-16-a2 전칭 판정 블록 : old[6747,6754] == new[6752,6759]  sha256=fc6bae4bfcbd4cc1…
  ∅   U-16-h 시점 고정 블록 : old[6987,6992] == new[6992,6997]  sha256=394535f673cbb928…
  ∅   U-16-b 간선 대응·고아 (E7) : old[6815,6831] == new[6820,6836]  sha256=8f6b4646b910a32e…
  ∅   U-15-g-4 CORR 술어 블록 : old[5006,5010] == new[5011,5015]  sha256=77e8185e57e2ea05…
  --
  ≠   [E14] U-16-c [PARENTS-UNTRUSTED] 정의 블록 (유일 소스) : old[7019,7077]=1feb7b5523941241… vs new[7024,7095]=24a111bf5cf2a8ce…  (에라타가 건드림)
  ≠   §12.3.3 (B) 처분표 머리 (에라타 5차 서술) : old[4349,4371]=6aecf13fe56988ed… vs new[4349,4376]=7b5d5bf36d3675cb…  (에라타가 건드림)
  ≠   심사 이력 v2.19 행 : old[118,118]=9d1e361898b5f2c4… vs new[118,118]=f95623fff97cb6c4…  (에라타가 건드림)
  ≠   변경 이력 v2.19 행 : old[201,201]=7461052d425f2097… vs new[201,201]=1c5fbeacdd849fd7…  (에라타가 건드림)
...
-- ⑤ 워킹트리·HEAD 결속 --
  HEAD                    = eddbd241ac1ec2b4ce25c922a879814d0ea3b9bf
  blob(HEAD:계약)         = e8ae0b3969bc5cf44ce95d69949c132d41beb0e4
  blob(eddbd241:계약)     = e8ae0b3969bc5cf44ce95d69949c132d41beb0e4
  워킹트리 hash-object    = e8ae0b3969bc5cf44ce95d69949c132d41beb0e4
  git diff --quiet eddbd241 -- 계약 → rc=0
  sha256(워킹트리)        = 98d81dacee09b2b523e9472418d8156bdafc21d5a67214274338a10654c2e8fd
  eddbd241..HEAD 계약 커밋 수  = 0
  sed -n 4625,4725p <워킹트리> sha256 = 957bf49da8fc6ae39f97abe679411afeaa5a59f707f35bf3b3a8c6f9de141f0d
```

---

## 2. 실행기 델타 — 에라타 5차가 요구한 코드 변경만

### 2-1. `u17-verify-v219e5.sh` — 직전 `dd0617ab…` 대비 diff (E14: `gitpath()` 가 상대면 `--show-toplevel` 과 결합)
...
`u17-ctrl-cwdrel-e5.sh` — **E14 대조군**: 경로 산출을 `cd` «이전»으로 옮기고 **결합 없이 cwd 기준**으로 검사(E14 가 금지한 형태) + ㉠ 발화 제거. 판정용 아님:

```diff
2c2
< # u17-verify (v2.19 에라타 5차·최종 eddbd241) — U-17 «예방 통제 활성 증거» 실행기 (계약 eddbd241 §12.3.4 U-17)
---
> # u17-ctrl-cwdrel-e5 — [E14 대조군] 파생 경로를 «결합 없이 cwd 기준»으로 검사하는 변형(E14 가 금지한 형태) + ㉠ 발화 제거. 판정용 아님.
23a24,26
> REPO_ARG="${1:-.}"
> SHALLOW_PATH=$(gitpath shallow); GRAFTS_PATH=$(gitpath info/grafts)   # [대조군] cd «이전»에 산출·검사 (cwd = 저장소 밖)
> GRAFTS_PRESENT_CTRL=$( [ -f "$GRAFTS_PATH" ] && echo yes || echo no )
68,69c71,72
< TOPLEVEL=$(git rev-parse --show-toplevel 2>/dev/null || printf '.')
< gitpath() { local v; v=$(git rev-parse --git-path "$1" 2>/dev/null); case "$v" in /*) printf '%s' "$v";; "") printf '';; *) printf '%s/%s' "$TOPLEVEL" "$v";; esac; }
---
> # [대조군] 결합 «없이» — 상대 파생을 그대로 반환해 cwd 기준으로 검사하게 한다(E14 위반)
> gitpath() { git -C "$REPO_ARG" rev-parse --git-path "$1" 2>/dev/null; }
71d73
< SHALLOW_PATH=$(gitpath shallow); GRAFTS_PATH=$(gitpath info/grafts)
75c77
< GRAFTS_PRESENT=$( [ -f "$GRAFTS_PATH" ] && echo yes || echo no )
---
> GRAFTS_PRESENT="$GRAFTS_PRESENT_CTRL"
275c277
< [ "$PU_N" -eq 0 ] || fire PREVENTION_UNVERIFIABLE "[PARENTS-UNTRUSTED ㉠] 부모 재파생 ≠ 이력 뷰 — 이력 뷰가 재작성됨: $PU_MISMATCH"
---
> # [대조군] ㉠ 발화 «제거» — ㉡ 경로 결합 결함만 남긴다
```

<details><summary>`u17-verify-v219e5.sh` 전문</summary>
...
### 2-2. `u16-full-exec-v219e5.py` — 직전 `2a9d254f…` 대비 diff (E14: 상대면 `--show-toplevel` 결합·절대면 그대로)
...
`u16-ctrl-cwdrel-e5.py` — **E14 대조군**(결합 없이 반환) + ㉠ 발화 제거:

```diff
2c2
< """U-16 «전 규칙» 손 실행기 — v2.19 에라타 5차·최종 (계약 eddbd241 §13.6.5
---
> """[E14 대조군 — 판정용 아님] 파생 경로를 «결합 없이 cwd 기준»으로 검사(E14 금지 형태) + ㉠ 발화 제거.  (계약 eddbd241 §13.6.5
101,107c101
...
<     return _os.path.join(top, v)
---
>     return g("rev-parse", "--git-path", rel)   # [대조군] 결합 «없이» — 상대면 cwd 기준으로 검사된다(E14 위반)
382,385c376
<     if PU_MISMATCH:
...
<details><summary>`u16-full-exec-v219e5.py` 전문</summary>
...
### 3-1. `t84v219e5.sh` (U-17 축 · sha256 `554a174d29dfc75844627d3275e7a4a6d7f0632e98b5eb818a4eabb8f609289f`)

```bash
#!/usr/bin/env bash
# t84v219e5.sh — v2.19 에라타 5차·최종(eddbd241) «영향 변이» 재실행 드라이버 (U-17 축):
#   [E14] 파생 경로의 «결합 기준»(상대→저장소 루트 결합 · 절대→그대로 · cwd 상대 검사 금지) · 정상 회귀.
...
EX="$SP/u17-verify-v219e5.sh"; CTRL="$SP/u17-ctrl-cwdrel-e5.sh"
FX="$SP/fx84i"; SEAM="$SP/seam219e5"
...
printf 't84v219e5_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
printf 'sha256(u17-verify-v219e5.sh)=%s   (재실행 실행기 — 에라타 5차 델타 E14)\n' "$(shasum -a 256 "$EX" | cut -d" " -f1)"
printf 'sha256(u17-ctrl-cwdrel-e5.sh)=%s  (E14 대조군 — 결합 «없이» cwd 기준 검사 + ㉠ 발화 제거)\n' "$(shasum -a 256 "$CTRL" | cut -d" " -f1)"
printf '드라이버 cwd(= «저장소 밖») = %s\n' "$PWD"

########################################################################
sec "[E14]-1 저장소 «밖» cwd + grafts — 결합 없이 cwd 기준 검사하면 «거짓 ABSENT» → ㉡ 통과(fail-open) / 루트 결합하면 발견 → UNVERIFIABLE"
R1="$FX/grafts"; SEED=$(initrepo "$R1"); P1=$(art "$R1"); W1=$(wf "$R1"); D1=$(d0a "$R1")
mkdir -p "$R1/.git/info"; printf '%s %s\n' "$D1" "$P1" > "$R1/.git/info/grafts"
rev_seam "$SM" "$D1" "$W1"
REL=$(git -C "$R1" rev-parse --git-path info/grafts); TOP=$(git -C "$R1" rev-parse --show-toplevel)
echo "  --git-path info/grafts (파생, 상대)   = $REL"
echo "  cwd(=$PWD) 기준 [ -f \"$REL\" ]        → $( [ -f "$REL" ] && echo present || echo 'ABSENT  ← 거짓 ABSENT (E14 가 닫는 자리)')"
echo "  [E14] 저장소 루트 결합 = $TOP/$REL"
echo "  결합값 기준 [ -f ]                     → $( [ -f "$TOP/$REL" ] && echo 'present ← 실제 grafts 발견' || echo ABSENT )"
echo "-- (a) 대조군: 결합 «없이» cwd 기준 검사(E14 위반) --"; run "$R1" "file:$SM" "$CTRL"
echo "-- (b) v2.19-5 판정 실행기: 루트 결합 --"; run "$R1" "file:$SM"

########################################################################
sec "[E14]-2 --separate-git-dir (파생이 «절대») — «상대면 결합·절대면 그대로» 분기가 «이중 결합» 없이 정확한가 (자기신고)"
...
ABS=$(git -C "$SEP" rev-parse --git-path info/grafts); TOP2=$(git -C "$SEP" rev-parse --show-toplevel)
echo "  --git-path info/grafts (파생, 절대) = $ABS"
echo "  절대 판별: case \"\$v\" in /*) 그대로 ;; → 결합 «안 함» ⇒ 검사 경로 = $ABS"
echo "  («이중 결합»이 났다면 $TOP2$ABS 가 되어 ABSENT 였을 것: $( [ -f "$TOP2$ABS" ] && echo present || echo ABSENT ))"
echo "  실제 검사 경로 존재? $( [ -f "$ABS" ] && echo present || echo ABSENT )"
rev_seam "$SM" "$DS" "$WS"; run "$SEP" "file:$SM"

########################################################################
sec "[정상 회귀] grafts·replace 부재 — cwd 루트/밖 «무관»하게 부재→부재 · 상태값 불변(ACTIVE)"
R3="$FX/normal"; SEED=$(initrepo "$R3"); P3=$(art "$R3"); W3=$(wf "$R3"); D3=$(d0a "$R3")
REL3=$(git -C "$R3" rev-parse --git-path info/grafts); TOP3=$(git -C "$R3" rev-parse --show-toplevel)
echo "  cwd 기준 [ -f $REL3 ] → $( [ -f "$REL3" ] && echo present || echo ABSENT ) · 결합값 [ -f $TOP3/$REL3 ] → $( [ -f "$TOP3/$REL3" ] && echo present || echo ABSENT )  (둘 다 ABSENT = 정상)"
rev_seam "$SM" "$D3" "$W3"
echo "-- (a) 드라이버 cwd(저장소 밖)에서 --"; run "$R3" "file:$SM"
echo "-- (b) 저장소 루트를 cwd 로 두고(서브셸) — 상태값 «불변» 확인 --"
( cd "$R3" && echo "\$ (cwd=$R3) bash u17-verify-v219e5.sh ." && U17_RESPONDER="file:$SM" U17_CAPTURE_DIR="$(mktemp -d)" bash "$EX" . 2>&1 | grep -avE '^U17-(A00|A0 |A1|A2|A3|A4|B1|B2|B3|B4|B5) |^  \| |^U17-H '; echo "u17_rc=${PIPESTATUS[0]}" )

sec "본 저장소 현행 상태 — live (에라타 5차·최종 실행기 · HEAD eddbd241)"
echo "\$ bash u17-verify-v219e5.sh $REPO"; U17_CAPTURE_DIR="$(mktemp -d)" bash "$EX" "$REPO" 2>&1 | grep -avE '^  \| ' ; echo "u17_rc=${PIPESTATUS[0]}"
```

### 3-2. `t82v219e5.sh` (U-16 축 · sha256 `b4645cfec96bdded3a9c2f07d04793baad1498d1fbc723bbcddd53ad99fd8403`)

```bash
#!/usr/bin/env bash
# t82v219e5.sh — v2.19 에라타 5차·최종(eddbd241) «영향 변이» 재실행 드라이버 (U-16 축):
#   [E14] 파생 경로의 «결합 기준»(상대→저장소 루트 결합 · 절대→그대로 · cwd 상대 검사 금지) · 정상 회귀.
# 서버 조회 0(순수 in-repo) · 픽스처는 scratchpad 독립 git repo(본 저장소 무접촉·worktree 미사용).
set -u
SP=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence
EX="$SP/u16-full-exec-v219e5.py"; CTRL="$SP/u16-ctrl-cwdrel-e5.py"

FX="$SP/fx82i"; REF=reviews/review.md; RAT=rationale/r1.md
export GIT_AUTHOR_NAME=fixture GIT_AUTHOR_EMAIL=f@x GIT_COMMITTER_NAME=fixture GIT_COMMITTER_EMAIL=f@x
sec(){ printf '\n########## %s ##########\n' "$*"; }
...
rm -rf "$FX"; mkdir -p "$FX"
printf 't82v219e5_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
printf 'sha256(u16-full-exec-v219e5.py)=%s   (재실행 실행기 — 에라타 5차 델타 E14)\n' "$(shasum -a 256 "$EX" | cut -d" " -f1)"
printf 'sha256(u16-ctrl-cwdrel-e5.py)=%s  (E14 대조군 — 결합 «없이» cwd 기준 검사 + ㉠ 발화 제거)\n' "$(shasum -a 256 "$CTRL" | cut -d" " -f1)"
printf '드라이버 cwd(= «저장소 밖») = %s\n' "$PWD"
echo "D_NO = $DNO"
...
sec "[E14]-U16-1 저장소 «밖» cwd + grafts — 결합 없이 cwd 기준이면 «거짓 ABSENT» → fail-open / 루트 결합하면 발견 → PROVENANCE_UNVERIFIABLE(2)"
R="$FX/grafts"; read -r H0 X CN Y <<< "$(build20a "$R")"; M=$(git -C "$R" rev-parse HEAD)
mkdir -p "$R/.git/info"; printf '%s %s\n' "$M" "$CN" > "$R/.git/info/grafts"
REL=$(git -C "$R" rev-parse --git-path info/grafts); TOP=$(git -C "$R" rev-parse --show-toplevel)
echo "  --git-path info/grafts (상대) = $REL · cwd 기준 존재? $( [ -f "$REL" ] && echo present || echo 'ABSENT ← 거짓 ABSENT' ) · [E14] 결합값 존재? $( [ -f "$TOP/$REL" ] && echo present || echo ABSENT )"
probe "$R" "$M"
echo "-- (a) 대조군: 결합 «없이» cwd 기준 검사(E14 위반) --"; run "$R" "$CTRL"
echo "-- (b) v2.19-5 판정 실행기: 루트 결합 --"; run "$R"

sec "[E14]-U16-2 --separate-git-dir (파생이 «절대») — «절대면 그대로» 분기가 이중 결합 없이 정확 (자기신고)"
...
echo "  --git-path info/grafts (절대) = $ABS · 그대로 존재? $( [ -f "$ABS" ] && echo present || echo ABSENT )"
echo "  («이중 결합»이 났다면) $TOP2$ABS → $( [ -f "$TOP2$ABS" ] && echo present || echo ABSENT )"
probe "$R2" "$M2"; run "$R2"

sec "[정상 회귀]-U16-3 grafts·replace 부재 — cwd 무관하게 부재→부재 · 상태값 불변(⑳ⓐ APPROVAL_MALFORMED(3))"
R3="$FX/normal"; read -r H3 X3 CN3 Y3 <<< "$(build20a "$R3")"; M3=$(git -C "$R3" rev-parse HEAD)
REL3=$(git -C "$R3" rev-parse --git-path info/grafts); TOP3=$(git -C "$R3" rev-parse --show-toplevel)
echo "  cwd 기준 $REL3 → $( [ -f "$REL3" ] && echo present || echo ABSENT ) · 결합값 → $( [ -f "$TOP3/$REL3" ] && echo present || echo ABSENT )  (둘 다 ABSENT = 정상)"
probe "$R3" "$M3"; run "$R3"
echo "-- 저장소 루트를 cwd 로 두고(서브셸) — 상태값 «불변» 확인 --"
( cd "$R3" && echo "\$ (cwd=$R3) python3 u16-full-exec-v219e5.py ." && python3 "$EX" . > "$FX/cwdroot.out" 2>&1; RC=$?; tail -6 "$FX/cwdroot.out"; echo "u16_rc=$RC" )
```
...
**전제**: 두 드라이버는 **저장소 «밖» cwd**(`/Users/harris/Development/private/kis_unified_sts` — 픽스처는 scratchpad 하위)에서 실행됐다. 그 사실이 각 출력 머리의 `드라이버 cwd(= «저장소 밖»)` 라인에 기록된다.

### 4-1. U-17 축 — `bash t84v219e5.sh`

```text
t84v219e5_utc=2026-08-19T03:52:45Z
sha256(u17-verify-v219e5.sh)=cd3e9e1eae5b1de7fb582869dedae1bb5763eb41a6f07e7880e119e66857a944   (재실행 실행기 — 에라타 5차 델타 E14)
sha256(u17-ctrl-cwdrel-e5.sh)=c142ed79cef507b4ca04c5445bf7ffc8b29f911eb131ca59a18be29ca3ad56da  (E14 대조군 — 결합 «없이» cwd 기준 검사 + ㉠ 발화 제거)
드라이버 cwd(= «저장소 밖») = /Users/harris/Development/private/kis_unified_sts

########## [E14]-1 저장소 «밖» cwd + grafts — 결합 없이 cwd 기준 검사하면 «거짓 ABSENT» → ㉡ 통과(fail-open) / 루트 결합하면 발견 → UNVERIFIABLE ##########
  --git-path info/grafts (파생, 상대)   = .git/info/grafts
  cwd(=/Users/harris/Development/private/kis_unified_sts) 기준 [ -f ".git/info/grafts" ]        → ABSENT  ← 거짓 ABSENT (E14 가 닫는 자리)
  [E14] 저장소 루트 결합 = /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/grafts/.git/info/grafts
  결합값 기준 [ -f ]                     → present ← 실제 grafts 발견
-- (a) 대조군: 결합 «없이» cwd 기준 검사(E14 위반) --
  * 85d90c7 d: introduce config/tos_completion.yaml
  * 476ea77 P: artifact
  * 54ba54e seed
$ bash u17-ctrl-cwdrel-e5.sh <fixture>
/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/u17-ctrl-cwdrel-e5.sh: line 25: gitpath: command not found
/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/u17-ctrl-cwdrel-e5.sh: line 25: gitpath: command not found
U17-0 target=kakao-harris-lee/kis_unified_sts@main
U17-0 pin=github.com/kakao-harris-lee/kis_unified_sts remotes: origin=github.com/kakao-harris-lee/kis_unified_sts match=origin | actions_app_id=15368 (apps/github-actions http=200) | responder=file:/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/seam219e5/rs capture_dir=/var/folders/mh/c3r1z1fj6l9_7vfsn7bq9dqr0000gq/T/tmp.cwin0UndGh
U17-PU [PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[ ] · (--git-path 파생)=no · ㉢ is_shallow=false · (--git-path 파생) 목록=[ ] · git-dir=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/grafts/.git · 무력화 GIT_NO_REPLACE_OBJECTS=1 · ㉠ 주 판별=git --no-replace-objects cat-file commit <x> parent 줄
U17-A0 repos/kakao-harris-lee/kis_unified_sts  utc=2026-08-19T03:52:46Z  http=200  x-github-request-id=  (.default_branch=main)
...
힌트: "git config advice.graftFileDeprecated false"
힌트: 명령을 사용하십시오
P_first(집합·|1|)=[476ea7744855e5af32fda997071d67df6eceed2f ] P_last(집합·|1|·blob=762145cfb2a9a719deb125bef8ecea955d7e656e)=[476ea7744855e5af32fda997071d67df6eceed2f ] |D|=1 D=[85d90c74175857080f3e6647fb2afb2cae082455 ]  [E9 ∀-부모]
U17-PU㉢ [E12] ㉢ 먼저 — 얕은 경계로 «특정»돼 국소 귀속된 불일치 0건=[]
U17-PU㉠ 재파생 대조: 검사 후보 2건 · «남는» 전역 불일치 1건=[85d90c74175857080f3e6647fb2afb2cae082455[재파생=(7f261e0b567be284309a365363b8bf04cf3f71f0) vs 뷰=(476ea7744855e5af32fda997071d67df6eceed2f)] ]
U17-SHALLOW is_shallow=false shallow 목록()=[ ] · 후보 우주 내 경계 커밋: D=[ ](0건) P=[ ](0건)  (E6: 전역 단축 아님 — 경로별 국소 판정)
U17-B5x 보조(선택·판정 미소비): 로컬 git show 7f261e0b567be284309a365363b8bf04cf3f71f0:.github/workflows/tos-gate.yml → 0aefd2ab57db63d19548f877328f66bef3a45100
U17-B d=85d90c74175857080f3e6647fb2afb2cae082455 head=7f261e0b567be284309a365363b8bf04cf3f71f0 merged_at=2026-08-10T00:00:00Z: name/conclusion/app.id=15368/head_sha/suite/workflow(path·head)/server-blob(2 리터럴) 전부 일치
...
reason=(a) 술어 충족(checks[].app_id==Actions 15368) ∧ 핀 원격 존재·선언 대조 일치 ∧ [C6] 핀 host 결속(--hostname github.com · GH_HOST 재핀) ∧ countersign 유효 ∧ [E9] |P_last|=1 ∧ ∀d∈D: x_last ⊰ d ∧ 소비 blob==blob(x_last) ∧ (b) 전 리비전 검증(|D|=1) ∧ (α) 연속성 성립(t_land=2026-08-10T00:00:00Z) — responder=file:/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/seam219e5/rs
u17_rc=0
-- (b) v2.19-5 판정 실행기: 루트 결합 --
  * 85d90c7 d: introduce config/tos_completion.yaml
  * 476ea77 P: artifact
  * 54ba54e seed
$ bash u17-verify-v219e5.sh <fixture>
U17-0 target=kakao-harris-lee/kis_unified_sts@main
U17-0 pin=github.com/kakao-harris-lee/kis_unified_sts remotes: origin=github.com/kakao-harris-lee/kis_unified_sts match=origin | actions_app_id=15368 (apps/github-actions http=200) | responder=file:/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/seam219e5/rs capture_dir=/var/folders/mh/c3r1z1fj6l9_7vfsn7bq9dqr0000gq/T/tmp.XTuvHo2rZc
U17-PU [PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[ ] · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/grafts/.git/info/grafts(--git-path 파생)=yes · ㉢ is_shallow=false · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/grafts/.git/shallow(--git-path 파생) 목록=[ ] · git-dir=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/grafts/.git · 무력화 GIT_NO_REPLACE_OBJECTS=1 · ㉠ 주 판별=git --no-replace-objects cat-file commit <x> parent 줄
U17-fire PREVENTION_UNVERIFIABLE: [PARENTS-UNTRUSTED] /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/grafts/.git/info/grafts 실재 — 부모 집합 재작성 = 신뢰 불가 (GIT_NO_REPLACE_OBJECTS 로 무력화되지 않는다)
U17-A0 repos/kakao-harris-lee/kis_unified_sts  utc=2026-08-19T03:52:48Z  http=200  x-github-request-id=  (.default_branch=main)
U17-T declared-vs-pin: 일치/선언 없음 (declared owner_repo=kakao-harris-lee/kis_unified_sts target_branch=main host=∅(선택 키 부재 → 핀 host 유일 소스))
U17-α0 적용 룰셋(연속성 입력우주) = [42]  (rules/branches/main 의 ruleset_id · rulesets 목록 전체=[42])
u17_live_state=PREVENTION_ACTIVE
u17_live_reason=(a) 술어 충족: classic=False ruleset=True
...
U17-PU㉢ [E12] ㉢ 먼저 — 얕은 경계로 «특정»돼 국소 귀속된 불일치 0건=[]
U17-PU㉠ 재파생 대조: 검사 후보 2건 · «남는» 전역 불일치 1건=[85d90c74175857080f3e6647fb2afb2cae082455[재파생=(7f261e0b567be284309a365363b8bf04cf3f71f0) vs 뷰=(476ea7744855e5af32fda997071d67df6eceed2f)] ]
U17-fire PREVENTION_UNVERIFIABLE: [PARENTS-UNTRUSTED ㉠] 부모 재파생 ≠ 이력 뷰 — 이력 뷰가 재작성됨: 85d90c74175857080f3e6647fb2afb2cae082455[재파생=(7f261e0b567be284309a365363b8bf04cf3f71f0) vs 뷰=(476ea7744855e5af32fda997071d67df6eceed2f)] 
U17-SHALLOW is_shallow=false shallow 목록(/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/grafts/.git/shallow)=[ ] · 후보 우주 내 경계 커밋: D=[ ](0건) P=[ ](0건)  (E6: 전역 단축 아님 — 경로별 국소 판정)
U17-B5x 보조(선택·판정 미소비): 로컬 git show 7f261e0b567be284309a365363b8bf04cf3f71f0:.github/workflows/tos-gate.yml → 0aefd2ab57db63d19548f877328f66bef3a45100
U17-B d=85d90c74175857080f3e6647fb2afb2cae082455 head=7f261e0b567be284309a365363b8bf04cf3f71f0 merged_at=2026-08-10T00:00:00Z: name/conclusion/app.id=15368/head_sha/suite/workflow(path·head)/server-blob(2 리터럴) 전부 일치
U17-α t_land = min{merged_at(착지 PR) : d∈D} = 2026-08-10T00:00:00Z  (서버 부여 값만 · 커밋 author/committer date 불신)
U17-α ruleset 42: ruleset 42 created_at=2026-08-01T00:00:00+00:00 ≤ t_land ∧ updated_at=2026-08-05T00:00:00+00:00 ≤ t_land
prevention_control_state=PREVENTION_UNVERIFIABLE
reason=[PARENTS-UNTRUSTED] /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/grafts/.git/info/grafts 실재 — 부모 집합 재작성 = 신뢰 불가 (GIT_NO_REPLACE_OBJECTS 로 무력화되지 않는다) [수집 2건 중 전순서 최소]
u17_rc=1

########## [E14]-2 --separate-git-dir (파생이 «절대») — «상대면 결합·절대면 그대로» 분기가 «이중 결합» 없이 정확한가 (자기신고) ##########
  --git-path info/grafts (파생, 절대) = /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd/info/grafts
  절대 판별: case "$v" in /*) 그대로 ;; → 결합 «안 함» ⇒ 검사 경로 = /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd/info/grafts
  («이중 결합»이 났다면 /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sep/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd/info/grafts 가 되어 ABSENT 였을 것: ABSENT)
  실제 검사 경로 존재? present
  * b402919 d: introduce config/tos_completion.yaml
  * b9f9216 P: artifact
  * 240db2b seed
$ bash u17-verify-v219e5.sh <fixture>
U17-0 target=kakao-harris-lee/kis_unified_sts@main
U17-0 pin=github.com/kakao-harris-lee/kis_unified_sts remotes: origin=github.com/kakao-harris-lee/kis_unified_sts match=origin | actions_app_id=15368 (apps/github-actions http=200) | responder=file:/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/seam219e5/rs capture_dir=/var/folders/mh/c3r1z1fj6l9_7vfsn7bq9dqr0000gq/T/tmp.zwMik1KyT0
U17-PU [PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[ ] · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd/info/grafts(--git-path 파생)=yes · ㉢ is_shallow=false · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd/shallow(--git-path 파생) 목록=[ ] · git-dir=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd · 무력화 GIT_NO_REPLACE_OBJECTS=1 · ㉠ 주 판별=git --no-replace-objects cat-file commit <x> parent 줄
U17-fire PREVENTION_UNVERIFIABLE: [PARENTS-UNTRUSTED] /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd/info/grafts 실재 — 부모 집합 재작성 = 신뢰 불가 (GIT_NO_REPLACE_OBJECTS 로 무력화되지 않는다)
U17-A0 repos/kakao-harris-lee/kis_unified_sts  utc=2026-08-19T03:52:51Z  http=200  x-github-request-id=  (.default_branch=main)
U17-T declared-vs-pin: 일치/선언 없음 (declared owner_repo=kakao-harris-lee/kis_unified_sts target_branch=main host=∅(선택 키 부재 → 핀 host 유일 소스))
U17-α0 적용 룰셋(연속성 입력우주) = [42]  (rules/branches/main 의 ruleset_id · rulesets 목록 전체=[42])
u17_live_state=PREVENTION_ACTIVE
u17_live_reason=(a) 술어 충족: classic=False ruleset=True
...
힌트: "git config advice.graftFileDeprecated false"
힌트: 명령을 사용하십시오
P_first(집합·|1|)=[b9f9216ab7c4a193821c780bdd087472396c5517 ] P_last(집합·|1|·blob=762145cfb2a9a719deb125bef8ecea955d7e656e)=[b9f9216ab7c4a193821c780bdd087472396c5517 ] |D|=1 D=[b40291973b4a650f43b2c681b44395bfe1281cdc ]  [E9 ∀-부모]
U17-PU㉢ [E12] ㉢ 먼저 — 얕은 경계로 «특정»돼 국소 귀속된 불일치 0건=[]
U17-PU㉠ 재파생 대조: 검사 후보 2건 · «남는» 전역 불일치 1건=[b40291973b4a650f43b2c681b44395bfe1281cdc[재파생=(dc65e1acee5da3a6fa9cbad056c3ce22a5f5d697) vs 뷰=(b9f9216ab7c4a193821c780bdd087472396c5517)] ]
U17-fire PREVENTION_UNVERIFIABLE: [PARENTS-UNTRUSTED ㉠] 부모 재파생 ≠ 이력 뷰 — 이력 뷰가 재작성됨: b40291973b4a650f43b2c681b44395bfe1281cdc[재파생=(dc65e1acee5da3a6fa9cbad056c3ce22a5f5d697) vs 뷰=(b9f9216ab7c4a193821c780bdd087472396c5517)] 
U17-SHALLOW is_shallow=false shallow 목록(/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd/shallow)=[ ] · 후보 우주 내 경계 커밋: D=[ ](0건) P=[ ](0건)  (E6: 전역 단축 아님 — 경로별 국소 판정)
U17-B5x 보조(선택·판정 미소비): 로컬 git show dc65e1acee5da3a6fa9cbad056c3ce22a5f5d697:.github/workflows/tos-gate.yml → 0aefd2ab57db63d19548f877328f66bef3a45100
U17-B d=b40291973b4a650f43b2c681b44395bfe1281cdc head=dc65e1acee5da3a6fa9cbad056c3ce22a5f5d697 merged_at=2026-08-10T00:00:00Z: name/conclusion/app.id=15368/head_sha/suite/workflow(path·head)/server-blob(2 리터럴) 전부 일치
U17-α t_land = min{merged_at(착지 PR) : d∈D} = 2026-08-10T00:00:00Z  (서버 부여 값만 · 커밋 author/committer date 불신)
U17-α ruleset 42: ruleset 42 created_at=2026-08-01T00:00:00+00:00 ≤ t_land ∧ updated_at=2026-08-05T00:00:00+00:00 ≤ t_land
prevention_control_state=PREVENTION_UNVERIFIABLE
reason=[PARENTS-UNTRUSTED] /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/sepgd/info/grafts 실재 — 부모 집합 재작성 = 신뢰 불가 (GIT_NO_REPLACE_OBJECTS 로 무력화되지 않는다) [수집 2건 중 전순서 최소]
u17_rc=1

########## [정상 회귀] grafts·replace 부재 — cwd 루트/밖 «무관»하게 부재→부재 · 상태값 불변(ACTIVE) ##########
  cwd 기준 [ -f .git/info/grafts ] → ABSENT · 결합값 [ -f /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal/.git/info/grafts ] → ABSENT  (둘 다 ABSENT = 정상)
-- (a) 드라이버 cwd(저장소 밖)에서 --
  * 0d85f4f d: introduce config/tos_completion.yaml
  * 1105a6e W: workflow
  * b9c4097 P: artifact
  * 59a7e62 seed
$ bash u17-verify-v219e5.sh <fixture>
U17-0 target=kakao-harris-lee/kis_unified_sts@main
U17-0 pin=github.com/kakao-harris-lee/kis_unified_sts remotes: origin=github.com/kakao-harris-lee/kis_unified_sts match=origin | actions_app_id=15368 (apps/github-actions http=200) | responder=file:/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/seam219e5/rs capture_dir=/var/folders/mh/c3r1z1fj6l9_7vfsn7bq9dqr0000gq/T/tmp.pGd40kOYpZ
U17-PU [PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[ ] · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal/.git/info/grafts(--git-path 파생)=no · ㉢ is_shallow=false · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal/.git/shallow(--git-path 파생) 목록=[ ] · git-dir=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal/.git · 무력화 GIT_NO_REPLACE_OBJECTS=1 · ㉠ 주 판별=git --no-replace-objects cat-file commit <x> parent 줄
U17-A0 repos/kakao-harris-lee/kis_unified_sts  utc=2026-08-19T03:52:54Z  http=200  x-github-request-id=  (.default_branch=main)
U17-T declared-vs-pin: 일치/선언 없음 (declared owner_repo=kakao-harris-lee/kis_unified_sts target_branch=main host=∅(선택 키 부재 → 핀 host 유일 소스))
U17-α0 적용 룰셋(연속성 입력우주) = [42]  (rules/branches/main 의 ruleset_id · rulesets 목록 전체=[42])
u17_live_state=PREVENTION_ACTIVE
u17_live_reason=(a) 술어 충족: classic=False ruleset=True
P_first(집합·|1|)=[b9c40979b53148d0e2edd694e9d4765bb751cc60 ] P_last(집합·|1|·blob=762145cfb2a9a719deb125bef8ecea955d7e656e)=[b9c40979b53148d0e2edd694e9d4765bb751cc60 ] |D|=1 D=[0d85f4fa4aa185272ed1c4b2ee56a93fee5297a8 ]  [E9 ∀-부모]
U17-PU㉢ [E12] ㉢ 먼저 — 얕은 경계로 «특정»돼 국소 귀속된 불일치 0건=[]
U17-PU㉠ 재파생 대조: 검사 후보 2건 · «남는» 전역 불일치 0건=[]
U17-SHALLOW is_shallow=false shallow 목록(/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal/.git/shallow)=[ ] · 후보 우주 내 경계 커밋: D=[ ](0건) P=[ ](0건)  (E6: 전역 단축 아님 — 경로별 국소 판정)
U17-B5x 보조(선택·판정 미소비): 로컬 git show 1105a6e6d9c3c498e4530e8aa7363581a26dd5db:.github/workflows/tos-gate.yml → 0aefd2ab57db63d19548f877328f66bef3a45100
U17-B d=0d85f4fa4aa185272ed1c4b2ee56a93fee5297a8 head=1105a6e6d9c3c498e4530e8aa7363581a26dd5db merged_at=2026-08-10T00:00:00Z: name/conclusion/app.id=15368/head_sha/suite/workflow(path·head)/server-blob(2 리터럴) 전부 일치
U17-α t_land = min{merged_at(착지 PR) : d∈D} = 2026-08-10T00:00:00Z  (서버 부여 값만 · 커밋 author/committer date 불신)
U17-α ruleset 42: ruleset 42 created_at=2026-08-01T00:00:00+00:00 ≤ t_land ∧ updated_at=2026-08-05T00:00:00+00:00 ≤ t_land
prevention_control_state=PREVENTION_ACTIVE
reason=(a) 술어 충족(checks[].app_id==Actions 15368) ∧ 핀 원격 존재·선언 대조 일치 ∧ [C6] 핀 host 결속(--hostname github.com · GH_HOST 재핀) ∧ countersign 유효 ∧ [E9] |P_last|=1 ∧ ∀d∈D: x_last ⊰ d ∧ 소비 blob==blob(x_last) ∧ (b) 전 리비전 검증(|D|=1) ∧ (α) 연속성 성립(t_land=2026-08-10T00:00:00Z) — responder=file:/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/seam219e5/rs
u17_rc=0
-- (b) 저장소 루트를 cwd 로 두고(서브셸) — 상태값 «불변» 확인 --
$ (cwd=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal) bash u17-verify-v219e5.sh .
U17-0 target=kakao-harris-lee/kis_unified_sts@main
U17-0 pin=github.com/kakao-harris-lee/kis_unified_sts remotes: origin=github.com/kakao-harris-lee/kis_unified_sts match=origin | actions_app_id=15368 (apps/github-actions http=200) | responder=file:/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/seam219e5/rs capture_dir=/var/folders/mh/c3r1z1fj6l9_7vfsn7bq9dqr0000gq/T/tmp.grOBbyJOQC
U17-PU [PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[ ] · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal/.git/info/grafts(--git-path 파생)=no · ㉢ is_shallow=false · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal/.git/shallow(--git-path 파생) 목록=[ ] · git-dir=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx84i/normal/.git · 무력화 GIT_NO_REPLACE_OBJECTS=1 · ㉠ 주 판별=git --no-replace-objects cat-file commit <x> parent 줄
U17-A0 repos/kakao-harris-lee/kis_unified_sts  utc=2026-08-19T03:52:56Z  http=200  x-github-request-id=  (.default_branch=main)
U17-T declared-vs-pin: 일치/선언 없음 (declared owner_repo=kakao-harris-lee/kis_unified_sts target_branch=main host=∅(선택 키 부재 → 핀 host 유일 소스))
U17-α0 적용 룰셋(연속성 입력우주) = [42]  (rules/branches/main 의 ruleset_id · rulesets 목록 전체=[42])
u17_live_state=PREVENTION_ACTIVE
u17_live_reason=(a) 술어 충족: classic=False ruleset=True
...
########## 본 저장소 현행 상태 — live (에라타 5차·최종 실행기 · HEAD eddbd241) ##########
$ bash u17-verify-v219e5.sh /Users/harris/Development/private/kis_unified_sts
U17-0 target=kakao-harris-lee/kis_unified_sts@main
U17-0 pin=github.com/kakao-harris-lee/kis_unified_sts remotes: origin=github.com/kakao-harris-lee/kis_unified_sts match=origin | actions_app_id=15368 (apps/github-actions http=200) | responder=gh capture_dir=/var/folders/mh/c3r1z1fj6l9_7vfsn7bq9dqr0000gq/T/tmp.WmelEjCHgZ
...
U17-A00 apps/github-actions  utc=2026-08-19T03:53:04Z  http=200  x-github-request-id=BF1C:335F3A:412FAB:48A6A8:6A85289F
U17-A0 repos/kakao-harris-lee/kis_unified_sts  utc=2026-08-19T03:53:04Z  http=200  x-github-request-id=A430:1D7764:412D18:48A500:6A8528A0  (.default_branch=main)
U17-fire PREVENTION_ABSENT: 아티팩트 HEAD 부재: tos-spec/src/part-1-foundation/decisions/D0A-PREVENTION-CONTROL.md
U17-A1 repos/kakao-harris-lee/kis_unified_sts/branches/main/protection  utc=2026-08-19T03:53:05Z  http=200  x-github-request-id=DFD5:21B9D:420B01:498319:6A8528A1
U17-A2 repos/kakao-harris-lee/kis_unified_sts/rules/branches/main  utc=2026-08-19T03:53:05Z  http=200  x-github-request-id=76F5:C76AD:41224C:4898F4:6A8528A1
U17-A3 repos/kakao-harris-lee/kis_unified_sts/rulesets  utc=2026-08-19T03:53:06Z  http=200  x-github-request-id=B95B:335F3A:4131F6:48A941:6A8528A2
U17-A4 repos/kakao-harris-lee/kis_unified_sts/rulesets/17017682  utc=2026-08-19T03:53:07Z  http=200  x-github-request-id=F90A:21B9D:420C97:4984E6:6A8528A2
...
### 4-2. U-16 축 — `bash t82v219e5.sh`

```text
t82v219e5_utc=2026-08-19T03:54:29Z
sha256(u16-full-exec-v219e5.py)=26f0583afbac6b01b5f97d35d336448e86b1dc0d4d94e7c944297e181718e90d   (재실행 실행기 — 에라타 5차 델타 E14)
sha256(u16-ctrl-cwdrel-e5.py)=4f96504a0a5a8a1777c340ba4b672b398416991d04a6631b937acfe356f563c6  (E14 대조군 — 결합 «없이» cwd 기준 검사 + ㉠ 발화 제거)
드라이버 cwd(= «저장소 밖») = /Users/harris/Development/private/kis_unified_sts
...
########## [E14]-U16-1 저장소 «밖» cwd + grafts — 결합 없이 cwd 기준이면 «거짓 ABSENT» → fail-open / 루트 결합하면 발견 → PROVENANCE_UNVERIFIABLE(2) ##########
  --git-path info/grafts (상대) = .git/info/grafts · cwd 기준 존재? ABSENT ← 거짓 ABSENT · [E14] 결합값 존재? present
  ㉠ 재파생=[f3edfed774f52ed9c1f24ecb264ccfc565740db6 4f88581083f1c705093f4c42c5b4c3c73f9d1853 ] · ㉠ 이력 뷰 %P=[f3edfed774f52ed9c1f24ecb264ccfc565740db6] · ㉡ replace -l=[]
...
-- (a) 대조군: 결합 «없이» cwd 기준 검사(E14 위반) --
  * 91b6690 M: merge sibling identical approval introduction
  * f3edfed CN: NO transition (child of X)
  * 8693021 X: approval row A [side x]
  * b1096d4 H0: base (r1=YES · reviewer digest)
$ python3 u16-ctrl-cwdrel-e5.py <fixture>
[PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[] · .git/info/grafts(--git-path 파생)=ABSENT · ㉢ is_shallow=False · shallow 파생 경로=.git/shallow · 무력화 = git --no-replace-objects (전 호출) · ㉠ 주 판별 = cat-file commit <x> parent 줄 재파싱
HEAD=91b6690 is_shallow=False .git/shallow=[] EVAL_ORDER=precheck-first  ([E6] 전역 단축 아님 — 경계는 «해당 행/간선의 후보 우주»에 있을 때만 크기 0을 만든다)
NO_rows=['r1']
EDGES(r1)=[('4f88581', '91b6690', 'YES->NO'), ('8693021', 'f3edfed', 'YES->NO')]  (edge_seq 표시용 파생=[1, 2] · 판정 미소비)
ledger_rows=[('r1', 'YES->NO', '|c_APP|=1', ['8693021'])]

[사실 수집] 규칙별 측정값 (순서 적용 «전»)
  row r1/YES->NO raw#0: |c_APP|=1 g4_bad=False g2_bad=False 대응간선=2 row_id간선=2
[PARENTS-UNTRUSTED ㉢] [E12] ㉢ 먼저 — 얕은 경계 국소 귀속 0건: []
[PARENTS-UNTRUSTED ㉠] «남는» 전역 불일치 1건: [('91b6690', ['4f88581', 'f3edfed'], ['f3edfed'])]

[상태 귀속] 계약 U-16-d 순서 적용
  · edge#1[r1 4f88581->91b6690 YES->NO]: COVERED by c_APP=8693021 C_R={b1096d4}
  · edge#2[r1 8693021->f3edfed YES->NO]: COVERED by c_APP=8693021 C_R={b1096d4}
...
-- (b) v2.19-5 판정 실행기: 루트 결합 --
  ...
  * 8693021 X: approval row A [side x]
  * b1096d4 H0: base (r1=YES · reviewer digest)
$ python3 u16-full-exec-v219e5.py <fixture>
[PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[] · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/grafts/.git/info/grafts(--git-path 파생)=present · ㉢ is_shallow=False · shallow 파생 경로=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/grafts/.git/shallow · 무력화 = git --no-replace-objects (전 호출) · ㉠ 주 판별 = cat-file commit <x> parent 줄 재파싱
HEAD=91b6690 is_shallow=False .git/shallow=[] EVAL_ORDER=precheck-first  ([E6] 전역 단축 아님 — 경계는 «해당 행/간선의 후보 우주»에 있을 때만 크기 0을 만든다)
NO_rows=['r1']
EDGES(r1)=[('4f88581', '91b6690', 'YES->NO'), ('8693021', 'f3edfed', 'YES->NO')]  (edge_seq 표시용 파생=[1, 2] · 판정 미소비)
ledger_rows=[('r1', 'YES->NO', '|c_APP|=1', ['8693021'])]
...
[상태 귀속] 계약 U-16-d 순서 적용
  · global: PROVENANCE_UNVERIFIABLE(2) — [PARENTS-UNTRUSTED ㉠] 부모 재파생 ≠ 이력 뷰 — 이력 뷰가 재작성됨: [('91b6690', ['4f88581', 'f3edfed'], ['f3edfed'])]
...
rules_missing=∅
closable_no_provenance_state=PROVENANCE_UNVERIFIABLE
reason=전순서 최소 = PROVENANCE_UNVERIFIABLE(2) @ global — [PARENTS-UNTRUSTED ㉠] 부모 재파생 ≠ 이력 뷰 — 이력 뷰가 재작성됨: [('91b6690', ['4f88581', 'f3edfed'], ['f3edfed'])] · 발화 전체=['PROVENANCE_UNVERIFIABLE']
u16_rc=1

########## [E14]-U16-2 --separate-git-dir (파생이 «절대») — «절대면 그대로» 분기가 이중 결합 없이 정확 (자기신고) ##########
  --git-path info/grafts (절대) = /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/sepgd/info/grafts · 그대로 존재? present
  («이중 결합»이 났다면) /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/sep/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/sepgd/info/grafts → ABSENT
  ㉠ 재파생=[6722d4c585585530c7ebb83f75172554812acc3d d5297bc2ae278b3fd119efc737e4d0ff4fd4a4d5 ] · ㉠ 이력 뷰 %P=[6722d4c585585530c7ebb83f75172554812acc3d] · ㉡ replace -l=[]
  [E13] --git-path info/grafts=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/sepgd/info/grafts → present | «리터럴» .git/info/grafts=ABSENT · --git-path shallow=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/sepgd/shallow → ABSENT · ㉢ is_shallow=false
  * b053532 M: merge sibling identical approval introduction
  * 6722d4c CN: NO transition (child of X)
  * 946ea18 X: approval row A [side x]
  * 8611d34 H0: base (r1=YES · reviewer digest)
$ python3 u16-full-exec-v219e5.py <fixture>
[PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[] · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/sepgd/info/grafts(--git-path 파생)=present · ㉢ is_shallow=False · shallow 파생 경로=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/sepgd/shallow · 무력화 = git --no-replace-objects (전 호출) · ㉠ 주 판별 = cat-file commit <x> parent 줄 재파싱
HEAD=b053532 is_shallow=False .git/shallow=[] EVAL_ORDER=precheck-first  ([E6] 전역 단축 아님 — 경계는 «해당 행/간선의 후보 우주»에 있을 때만 크기 0을 만든다)
NO_rows=['r1']
EDGES(r1)=[('946ea18', '6722d4c', 'YES->NO'), ('d5297bc', 'b053532', 'YES->NO')]  (edge_seq 표시용 파생=[1, 2] · 판정 미소비)
ledger_rows=[('r1', 'YES->NO', '|c_APP|=1', ['946ea18'])]

[사실 수집] 규칙별 측정값 (순서 적용 «전»)
  row r1/YES->NO raw#0: |c_APP|=1 g4_bad=False g2_bad=False 대응간선=2 row_id간선=2
[PARENTS-UNTRUSTED ㉢] [E12] ㉢ 먼저 — 얕은 경계 국소 귀속 0건: []
[PARENTS-UNTRUSTED ㉠] «남는» 전역 불일치 1건: [('b053532', ['6722d4c', 'd5297bc'], ['6722d4c'])]

[상태 귀속] 계약 U-16-d 순서 적용
  · global: PROVENANCE_UNVERIFIABLE(2) — [PARENTS-UNTRUSTED ㉠] 부모 재파생 ≠ 이력 뷰 — 이력 뷰가 재작성됨: [('b053532', ['6722d4c', 'd5297bc'], ['6722d4c'])]
  · global: PROVENANCE_UNVERIFIABLE(2) — [PARENTS-UNTRUSTED] .git/info/grafts 실재 — 부모 집합 재작성 = 신뢰 불가 (GIT_NO_REPLACE_OBJECTS 로 무력화되지 않는다)
  · edge#1[r1 946ea18->6722d4c YES->NO]: COVERED by c_APP=946ea18 C_R={8611d34}
  · edge#2[r1 d5297bc->b053532 YES->NO]: COVERED by c_APP=946ea18 C_R={8611d34}
rules_executed=U-16-a(EDGES);U-16-b(edge_seq 표시용 파생·판정 미소비);U-16-c(c_APP 구조 집합·카디널리티·진 조상);g1;g2;g3;g4;g5;h;g6(C_R blob·∃witness);U-16-a2(∀edge∃row);MALFORMED(orphan/double-cover);U-16-d(선-검사 1~4 → g-단락 5~11 · 전순서 최소)
rules_missing=∅
closable_no_provenance_state=PROVENANCE_UNVERIFIABLE
reason=전순서 최소 = PROVENANCE_UNVERIFIABLE(2) @ global — [PARENTS-UNTRUSTED ㉠] 부모 재파생 ≠ 이력 뷰 — 이력 뷰가 재작성됨: [('b053532', ['6722d4c', 'd5297bc'], ['6722d4c'])] · 발화 전체=['PROVENANCE_UNVERIFIABLE']
u16_rc=1

########## [정상 회귀]-U16-3 grafts·replace 부재 — cwd 무관하게 부재→부재 · 상태값 불변(⑳ⓐ APPROVAL_MALFORMED(3)) ##########
  cwd 기준 .git/info/grafts → ABSENT · 결합값 → ABSENT  (둘 다 ABSENT = 정상)
  ㉠ 재파생=[ed4b3a51b1272573f920853596d8d38c222e0f4c afcf66a50e725814a53a457de8efed7dfccc7eda ] · ㉠ 이력 뷰 %P=[ed4b3a51b1272573f920853596d8d38c222e0f4c afcf66a50e725814a53a457de8efed7dfccc7eda] · ㉡ replace -l=[]
  ...
  *   3c3795a M: merge sibling identical approval introduction
  |\  
  | * afcf66a Y: approval row A (byte-identical) [side y]
  * | ed4b3a5 CN: NO transition (child of X)
  * | a4b8f74 X: approval row A [side x]
  |/  
  * b566da2 H0: base (r1=YES · reviewer digest)
$ python3 u16-full-exec-v219e5.py <fixture>
[PARENTS-UNTRUSTED] ㉡ 전역 관측: git replace -l=[] · /private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/normal/.git/info/grafts(--git-path 파생)=ABSENT · ㉢ is_shallow=False · shallow 파생 경로=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/normal/.git/shallow · 무력화 = git --no-replace-objects (전 호출) · ㉠ 주 판별 = cat-file commit <x> parent 줄 재파싱
HEAD=3c3795a is_shallow=False .git/shallow=[] EVAL_ORDER=precheck-first  ([E6] 전역 단축 아님 — 경계는 «해당 행/간선의 후보 우주»에 있을 때만 크기 0을 만든다)
NO_rows=['r1']
EDGES(r1)=[('a4b8f74', 'ed4b3a5', 'YES->NO'), ('afcf66a', '3c3795a', 'YES->NO')]  (edge_seq 표시용 파생=[1, 2] · 판정 미소비)
ledger_rows=[('r1', 'YES->NO', '|c_APP|=2', ['afcf66a', 'a4b8f74'])]
...
[상태 귀속] 계약 U-16-d 순서 적용
  · row[r1/YES->NO]: APPROVAL_MALFORMED(3) — |c_APP|=2>1 (동일 승인 행 병렬 독립 도입) ['afcf66a', 'a4b8f74']
  · edge#1[r1 a4b8f74->ed4b3a5 YES->NO]: APPROVAL_MALFORMED(3) — |c_APP|>1 (후보 1 · 대응 1)
  · edge#2[r1 afcf66a->3c3795a YES->NO]: APPROVAL_MALFORMED(3) — |c_APP|>1 (후보 1 · 대응 1)
rules_executed=U-16-a(EDGES);U-16-b(edge_seq 표시용 파생·판정 미소비);U-16-c(c_APP 구조 집합·카디널리티·진 조상);g1;g2;g3;g4;g5;h;g6(C_R blob·∃witness);U-16-a2(∀edge∃row);MALFORMED(orphan/double-cover);U-16-d(선-검사 1~4 → g-단락 5~11 · 전순서 최소)
rules_missing=∅
closable_no_provenance_state=APPROVAL_MALFORMED
reason=전순서 최소 = APPROVAL_MALFORMED(3) @ row[r1/YES->NO] — |c_APP|=2>1 (동일 승인 행 병렬 독립 도입) ['afcf66a', 'a4b8f74'] · 발화 전체=['APPROVAL_MALFORMED']
u16_rc=1
-- 저장소 루트를 cwd 로 두고(서브셸) — 상태값 «불변» 확인 --
$ (cwd=/private/tmp/claude-503/-Users-harris-Development-private-kis-unified-sts/159918e8-47df-4978-bf11-b22f19e50240/scratchpad/v219-evidence/fx82i/normal) python3 u16-full-exec-v219e5.py .
  · edge#1[r1 a4b8f74->ed4b3a5 YES->NO]: APPROVAL_MALFORMED(3) — |c_APP|>1 (후보 1 · 대응 1)
  · edge#2[r1 afcf66a->3c3795a YES->NO]: APPROVAL_MALFORMED(3) — |c_APP|>1 (후보 1 · 대응 1)
rules_executed=U-16-a(EDGES);U-16-b(edge_seq 표시용 파생·판정 미소비);U-16-c(c_APP 구조 집합·카디널리티·진 조상);g1;g2;g3;g4;g5;h;g6(C_R blob·∃witness);U-16-a2(∀edge∃row);MALFORMED(orphan/double-cover);U-16-d(선-검사 1~4 → g-단락 5~11 · 전순서 최소)
rules_missing=∅
...
---
## 5. 관측 보고 · 신규 결함 후보 (최종 — 등급 명시)

> **등급 규약(최종 addendum)**: **[fail-open/차단]** = 재개 사유(그 문언대로 구현하면 통과해서는 안 될 것이 통과) · **[문언]**·**[관측]** = 기록만, 재개 사유 아님.
> addendum-4 의 **L-1**(파생 경로 결합 기준)은 에라타 5차가 **E14** 로 처분했고 §3·§4 가 **fail-open 대조군 실증**(U-17 `ACTIVE`/0 · U-16 `NO_ROWS_CLEAR`/0)과 **판정기 차단**(각각 1·2)으로 확인했다.
> **L-2·L-3·L-5** 는 계약 본문에 한 줄씩 성문화됐다(§1 diff).

### **[fail-open/차단] 신규 0건**

- 이번 실행에서 **«통과해서는 안 될 것이 통과»하는 새 구성을 찾지 못했다.** E14 가 닫은 자리(결합 기준)는 §4 에서 대조군/판정기가 **정확히 갈렸고**(0 vs 1·2), 자기신고 축(`--separate-git-dir` 에서의 «이중 결합» 위험)도 실측으로 **없음**이 확인됐다.
- ⇒ **재개 사유 없음.**

### N-1 **[관측]** `--git-path` 의 상대 반환은 «저장소 루트» 기준이며, `--show-toplevel` 결합이 정확했다

- 실측(§4-1·§4-2·§6): 일반 배치에서 `--git-path info/grafts` = `.git/info/grafts` 이고, `git rev-parse --show-toplevel` 과 결합한 값이 **실제 파일을 정확히 가리켰다**(present). `--separate-git-dir` 에서는 파생이 **절대**라 «그대로» 쓰였고, 만약 이중 결합이 났다면 그 경로는 **ABSENT** 였을 것임을 나란히 실측했다(§4-1 [E14]-2).
- **관측**: 계약 E14 는 결합 기준을 «저장소 루트 / `--absolute-git-dir`» 둘로 적는다. 실측상 **`--show-toplevel` 결합이 맞고 `--absolute-git-dir` 결합은 틀린다**(후자는 `<git-dir>` 이므로 `.git/info/grafts` 를 붙이면 `<git-dir>/.git/info/grafts` 가 된다). 문언이 두 기준을 병렬로 적어 두었으나 **상대 파생에 결합할 대상은 «저장소 루트»뿐**이다 — `--absolute-git-dir` 은 «파생을 대체하는 다른 경로 산출법»으로 읽어야 맞다. **극성은 fail-closed**(잘못 결합하면 거짓 ABSENT 가 아니라 거짓 present 도 아닌 «없는 경로» → ABSENT)이므로 **[관측]** 등급이나, 문언을 한 줄 다듬으면 오독이 사라진다.

### N-2 **[관측]** cwd 를 저장소 루트로 두는 소비자는 E14 이전에도 안전했다 — E14 는 «cwd 가정»을 제거한다

- §4-1 «정상 회귀 (b)»: 저장소 루트를 cwd 로 두고 실행하면 결합 유무와 무관하게 상태값이 같다(`PREVENTION_ACTIVE`/0). E14 의 실질은 «cwd 가정을 없앤다»이며, 그 가정이 깨지는 배치(저장소 밖 cwd)에서만 극성이 갈린다 — §4 가 그 경계를 양쪽에서 고정했다.

### N-3 **[관측]** 최종 상태 요약

- **본 저장소**(§6): ㉠ 재파생 == 이력 뷰 · ㉡ `git replace -l` 공집합 ∧ `<결합값> info/grafts` 부재 · ㉢ `--is-shallow-repository=false` ⇒ **[PARENTS-UNTRUSTED] 통과** · U-17 live = **`PREVENTION_ABSENT`**(아티팩트 부재 — 계약 문언 그대로) · 하니스 블록 sha256 `957bf49d…` **5판 연속 byte-동일**.

---
## 6. 사후 검증 원문 (repo 무영향 · HEAD 불변 · S-24 재확인 · 본 저장소 [PARENTS-UNTRUSTED] 통과 · 픽스처 격리)

```text
post_utc=2026-08-19T03:55:09Z
$ git -C <repo> rev-parse HEAD
eddbd241ac1ec2b4ce25c922a879814d0ea3b9bf
$ git -C <repo> status --short
 M uv.lock
?? tools/spikes/
$ git -C <repo> diff --quiet eddbd241 -- <계약> → rc
rc=0
$ git -C <repo> rev-list --count eddbd241..HEAD -- <계약>
0
$ sed -n '4625,4725p' <워킹트리> | shasum -a 256
957bf49da8fc6ae39f97abe679411afeaa5a59f707f35bf3b3a8c6f9de141f0d  -
$ git show eddbd241:<계약> | sed -n '4625,4725p' | shasum -a 256
957bf49da8fc6ae39f97abe679411afeaa5a59f707f35bf3b3a8c6f9de141f0d  -
$ git show db6ce918:<계약> | sed -n '4620,4720p' | shasum -a 256   (직전 판 — byte-동일)
957bf49da8fc6ae39f97abe679411afeaa5a59f707f35bf3b3a8c6f9de141f0d  -
$ git -C <repo> reflog -n 3
eddbd241 HEAD@{0}: commit: docs(tos): phase0 completion contract v2.19 errata #5 (final) — derived-path binding rule (repo-root join / --absolute-git-dir)
4f102c73 HEAD@{1}: commit: docs(tos): record v2.19 errata #4 addendum evidence (S-24 — git-dir-derived paths · ㉢-first jurisdiction · K-4 ancestry residual)
db6ce918 HEAD@{2}: commit: docs(tos): phase0 completion contract v2.19 errata #4 — ㉠/㉢ jurisdiction, git-dir-derived paths (no `.git/` literals)
--- [E14] 본 저장소 파생·결합 실측 ---
  --git-path info/grafts = .git/info/grafts (상대) · --show-toplevel = /Users/harris/Development/private/kis_unified_sts
  [E14] 결합값 = /Users/harris/Development/private/kis_unified_sts/.git/info/grafts → ABSENT
  --git-path shallow = .git/shallow · 결합값 → ABSENT
  git replace -l = '' · --is-shallow-repository = false
  ㉠ 재파생 = 4f102c73cd9dad15b4d70305d741248ee83d9b80  · ㉠ 이력 뷰 %P = 4f102c73cd9dad15b4d70305d741248ee83d9b80
  ⇒ ㉠ 일치 · ㉡ 공집합/부재(결합값 기준) · ㉢ 얕지 않음 — [PARENTS-UNTRUSTED] 통과
--- 픽스처 격리 ---
       6
```

**판독**: HEAD `eddbd241` 불변 · 계약 워킹트리 = 에라타 5차 blob(rc=0) · `eddbd241..HEAD` 계약 커밋 0 · 하니스 블록 `sed -n '4625,4725p'` sha256 이 워킹트리·`eddbd241` 양쪽에서 **`957bf49d…`** 이고
**`db6ce918:4620-4720` 과도 동일**(§1 ④ 와 이중 확인) · 워킹트리 변경은 실행 «전»부터 있던 `uv.lock`·`tools/spikes/` 뿐(본 실행이 만든 것 0 — 선행 증거 6파일은 `90a5ce7d`·`197f4fe4`·`c83e44db`·`d988bd0f`·`4f102c73` 로 이미 커밋됐다) ·
**본 저장소는 [E14] 결합값 기준으로 `info/grafts`·`shallow` 부재 · `git replace -l` 공집합 · `--is-shallow-repository=false` · ㉠ 재파생 == 이력 뷰** ⇒ `[PARENTS-UNTRUSTED]` 통과 ·
`grafts`/`--separate-git-dir` 조작은 **전부 scratchpad 픽스처 안에서만** · addendum-5 픽스처 git 저장소 6개는 전부 scratchpad 하위(`fx84i/*`·`fx82i/*`)다.
